...
## Sinopse
Atualize o ACL de Rede de um serviço de Sinalização.
...
## Descrição
Atualize o ACL de Rede de um serviço de Sinalização, incluindo a ação padrão e os Acls de rede para conexão pública e privada.
...
### Permitir conexão do cliente e conexão de servidor para uma conexão de ponto de extremidade particular
```powershell
...
### Negar a conexão do cliente tanto para a rede pública quanto para uma conexão de ponto de extremidade particular
```powershell
...
### -DefaultAction
Ação padrão de ACLs de rede signalr, permitir ou negar. Ele decide se as ACLs de rede são negadas ou permitem que acLs de rede entre em vigor. Por exemplo, se a ação padrão for permitir, somente as ACLs negadas são importantes.
...
### -PrivateEndpointName
Nome(s) de pontos de extremidade particulares a serem atualizados
