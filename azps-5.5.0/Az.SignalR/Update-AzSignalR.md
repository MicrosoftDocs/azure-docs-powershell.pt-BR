...
## Descrição
Atualizar um serviço de Sinalização.
Os seguintes valores serão usados para os parâmetros se não especificados:
* `ResourceGroupName`: o grupo de recursos padrão definido por `Set-AzDefault -ResourceGroupName` .
* `Sku`: `Standard_S1`

## Exemplos

### Atualizar um serviço de Sinalização específico.
```powershell
...
### -AllowedOrigin
As origens permitidas para o serviço de Sinalização. Para permitir tudo, use "*" e remova todas as outras origens da lista. Barras não são permitidas como parte do domínio ou depois de TLD
...
### -ServiceMode
O modo de serviço do serviço de Sinalização.
...
### -SKU
A SKU do serviço de Sinalização.
...
### -Tag
As marcas do serviço de Sinalização.
...
### -UnitCount
A contagem da unidade de serviço de Sinalização, valor somente de {1, 2, 5, 10, 20, 50, 100}.
Padrão para 1.
