...
## Descrição
O New-AzConsumptionBudget cmdlet cria um orçamento em uma assinatura ou em um grupo de recursos.

## Exemplos

### Exemplo 1: Criar um orçamento de custo com um nome de orçamento no nível da assinatura
```powershell
...
### -ContactEmail
Endereços de email para enviar a notificação de orçamento quando o limite for excedido.
...
### -EndDate
Data de término (AAAA-DD-MM no UTC) do período de tempo de um orçamento.
...
### -MeterFilter
Lista de metros separados por vírgulas para filtrar.
Obrigatório se categoria for o uso.
...
### -NotificationKey
Chave de uma notificação associada a um orçamento, necessária para criar uma notificação com opção habilitada para notificação, limite de notificação, emails de contato, grupos de contatos ou funções de contato.
...
Ela sempre é porcentagem e deve estar entre 0 e 1000.
...
### -Data Inicial
Data de início (AAAA-DD-MM no UTC) do período de tempo de um orçamento.
Não antes do mês atual para o tempo mensal.
Não antes de três meses para o tempo trimestral.
Não antes de doze meses para a hora anual.
Data de início futura, não mais do que três meses.
