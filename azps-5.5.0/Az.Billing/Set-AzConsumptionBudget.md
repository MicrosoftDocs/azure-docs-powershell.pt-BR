...
## Descrição
O Set-AzConsumptionBudget cmdlet atualiza um orçamento em uma assinatura ou em um grupo de recursos.

## Exemplos

### Exemplo 1: Atualizar um orçamento em um novo valor com um nome de orçamento no nível da assinatura
```powershell
...
### Exemplo 3: Atualizar um orçamento em um novo valor com um nome de orçamento no nível do grupo de recursos
```powershell
...
### -ContactGroup
Grupos de ações para enviar a notificação de orçamento quando o limite for excedido.
...
Se não especificado, a notificação será desabilitada por padrão.
