...
## Descrição
Esse comando não registrou um servidor do serviço de sincronização de armazenamento. Aviso: a não registro de um servidor resultará em exclusões em cascata de todos os pontos de extremidade do servidor neste servidor. Ela só deve ser chamada quando você tem certeza de que nenhum caminho neste servidor deve ser sincronizado mais.
...
Esse comando desosterna o servidor, causando exclusões em cascata de todos os pontos de extremidade do servidor neste servidor.
...
### -InputObject
Objeto de Entrada RegisteredServer, normalmente passou pelo pipeline.
