...
## Sinopse
Atualizar um aplicativo de malha de serviço. Isso permite atualizar os parâmetros do aplicativo e/ou atualizar a versão do tipo de aplicativo que disparará uma atualização de aplicativo. Só é compatível com aplicativos implantados pelo ARM.
...
Este exemplo iniciará uma atualização de aplicativo para atualizar a versão do tipo para "v2", que foi criada com **o New-AzServiceFabricApplicationTypeVersion.** Os parâmetros de aplicativo usados devem ser definidos no manifesto do aplicativo.
...
Este exemplo atualizará o número mínimo e máximo de restrição de nós para o aplicativo.
...
Este exemplo iniciará uma atualização de aplicativo para atualizar a versão do tipo para "v2" e também define alguns parâmetros de política de atualização que terão efeito a partir da atualização atual.
...
Indica se um evento de saúde de aviso deve ser tratado como um evento de erro durante a avaliação da saúde.
...
Especifica a porcentagem máxima de partições inhelthy por serviço permitida pela política de saúde para o tipo de serviço padrão a ser usado para a atualização monitorada.
...
Especifica a porcentagem máxima de replicações inhelthy por serviço permitidas pela política de saúde para o tipo de serviço padrão a ser usado para a atualização monitorada.
...
Os valores aceitáveis para este parâmetro são Reativação ou Manual.
...
### -HealthCheckCheckAriaryTimeoutSec
Especifica a duração, em segundos, após a qual a Malha de Serviço recupera a verificação de saúde se a verificação de saúde anterior falhar.
...
Especifica a duração, em segundos, que a Malha de Serviço espera para verificar se o aplicativo está estável antes de passar para o próximo domínio de atualização ou concluir a atualização.
Essa duração de espera evita alterações de saúde descaradas logo após a verificação de saúde ser executada.
...
Especifica a duração, em segundos, de que a Malha de Serviço espera antes de executar a verificação de saúde inicial após concluir a atualização no domínio de atualização.
...
### -MaximumNodeCount
Especifica o número máximo de nós em que um aplicativo deve ser colocado
...
### -MinimumNodeCount
Especifica o número mínimo de nós onde a Malha de Serviço reserva a capacidade para este aplicativo
...
Especifica o mapa da política de saúde a ser usada para diferentes tipos de serviço como uma tabela hash no seguinte formato: @ {"ServiceTypeName" : "MaxPercentUnhealthyPartitionsPerService,MaxPercentUnhealthyReplicasPerPartition,MaxPercentUnhealthyServices"}.
...
Especifica a porcentagem máxima das instâncias de aplicativo implantadas nos nós no cluster que têm um estado de saúde de erro antes que o estado de saúde do aplicativo para o cluster seja um erro.
...
Especifica o tempo máximo, em segundos, que a Malha de Serviço leva para atualizar um único domínio de atualização.
...
Especifica o tempo máximo que o Service Fabric espera para que um serviço seja reconfigurado em um estado seguro, se ainda não estiver em um estado seguro, antes que o Service Fabric prossiga com a atualização.
...
### -UpgradeTimeoutSec
Especifica o tempo máximo, em segundos, que a Malha de Serviço leva para toda a atualização.
Após esse período, a atualização falha.
