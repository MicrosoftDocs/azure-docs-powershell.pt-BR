...
## Descrição
Esse cmdlet permite a criação de serviços sem estado ou estado no aplicativo especificado. O serviço deve sair no manifesto do aplicativo e o tipo deve ser o mesmo que o do manifesto. O nome do aplicativo deve ser um prefixo do nome do serviço.
...
Este exemplo criará um novo serviço sem estado "testApp~testService1" com contagem de instâncias -1 (em todos os nós).
...
### -DefaultMoveCost
Especifique o custo padrão de uma movimentação.
Os custos mais altos fazem com que seja menos provável que o Gerenciador de Recursos de Cluster mova a replica ao tentar equilibrar o cluster
...
### -MinReplicaSetSize
Especificar o tamanho mínimo de conjunto de replicas para o serviço
...
Os serviços que usam esse modelo geralmente têm dados que podem ser bucketed dentro de um conjunto limitado.
...
### -PartitionSchemeSing ltda
Indica que o serviço usa o esquema de partição de singleton.
As partições singleton geralmente são usadas quando o serviço não exige qualquer roteamento adicional.
...
Especificar a duração de espera de perda de quo quorum para o serviço
...
Especificar a duração de espera de reinicialização de replica para o serviço
...
Especificar a duração da replica de stand by para o serviço
...
Especificar o tamanho do conjunto de replicas de destino para o serviço
