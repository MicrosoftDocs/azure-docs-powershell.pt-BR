...
## Descrição
Este cmdlet cria uma nova versão de tipo de aplicativo usando o pacote especificado em -PackageUrl, ele deve estar acessível por meio de um ponto de extremidade REST para que o Gerenciador de Recursos do Azure seja consumido durante a implantação e continha o aplicativo empacotado e cortado com a extensão .sfpkg. Esse comando criará o tipo de aplicativo se ele ainda não existir.
...
Este exemplo criará uma versão de tipo de aplicativo "v1" em tipo "testAppType". A versão no manifesto do aplicativo contida no pacote deve ter a mesma versão especificada em -Version.
...
### -DefaultParameter
Especifique os valores padrão dos parâmetros de aplicativo como pares de chave/valor.
