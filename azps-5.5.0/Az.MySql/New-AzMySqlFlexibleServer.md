...
## Sinopse
Cria um novo servidor flexível MySQL.
...
### Exemplo 1: Criar um novo servidor mySql flexível com argumentos
```powershell
...
### Exemplo 2: Criar um novo servidor flexível MySql com configuração padrão
```powershell
...
Esse cmdlet cria um servidor flexível MySql com valores de parâmetro padrão e provisiona o servidor dentro de uma nova rede virtual e tem uma sub-rede delegada ao servidor.
Os valores padrão de localização são Oeste dos EUA 2, SKU é Standard_B1ms, a camada SKU é imbatível e o tamanho de armazenamento é 10GiB.
...
### Exemplo 3: Criar um novo servidor mySql flexível com rede virtual
```powershell
PS C:\> $Vnet = 'vnetname'
...
Este cmdlet cria um servidor flexível MySql com id da vnet ou nome de vnet fornecido por um usuário.
Se a rede virtual não existir, o cmdlet criará uma.

### Exemplo 4: Criar um novo servidor flexível MySql com nome de rede virtual e sub-rede
```powershell
...
Este cmdlet cria um servidor flexível MySql com nome de vnet, nome da sub-rede, prefixo de vnet e prefixo de sub-rede.
Se a rede virtual e a sub-rede não existirem, o cmdlet criará um.

### Exemplo 7: Criar um novo servidor flexível MySql com acesso público a todos os IPs
```powershell
...
Este cmdlet cria o servidor flexível MySql aberto para todos os endereços IP.

### Exemplo 8: Criar um novo servidor flexível MySql com firewall
```powershell
...
Este cmdlet cria o servidor flexível MySql aberto para endereços IP especificados.
...
Observe que a sub-rede será delegada ao Microsoft.DBforMySQL/flexibleServers.
