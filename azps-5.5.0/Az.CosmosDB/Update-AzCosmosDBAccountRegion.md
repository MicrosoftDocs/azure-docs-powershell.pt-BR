...
## Sinopse
Atualizar Regiões de uma Conta do CosmosDB.
...
## Descrição
Atualizar Regiões de uma Conta do CosmosDB. O local pode ser fornecido como um objeto do tipo PSLocation ou como cadeias de caracteres de Nome de Local ordenadas por prioridade de failover.
O parâmetro LocationObject espera que a lista de locais atuais (priorícios de failover incluídos) acrescentada pelos novos LocationObjects correspondentes a novos locais sejam adicionados.
O parâmetro de localização espera a lista de localização atual (ordenada por prioridade de failover) e os novos locais. Observe que só damos suporte à Adição de Regiões. Forneça Local ou LocalObject.
