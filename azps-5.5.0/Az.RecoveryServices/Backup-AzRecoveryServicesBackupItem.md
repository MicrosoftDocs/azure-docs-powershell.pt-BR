...
O segundo comando obtém o item de Backup correspondente ao contêiner no $NamedContainer e o armazena na variável $Item dados.
O último comando aciona o trabalho de backup do item backup no $Item.
...
Se a ativação da compactação for necessária
...
Especifica um tempo de expiração para o ponto de Recuperação como um objeto DateTime, se nada for dado, ele leva o valor padrão de 30 dias. Aplicável ao VM, SQL (somente para o tipo de backup somente cópia completa), itens de backup AFS.
