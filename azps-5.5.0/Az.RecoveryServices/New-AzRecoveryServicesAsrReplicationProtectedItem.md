...
## Sinopse
Habilita a replicação para um item protegido pelo ASR criando um item protegido por replicação.
...
### Detalhes do AzureToAzureWithoutDiskDetails
```
...
## Descrição
O cmdlet **New-AzRecoveryServicesAsrReplicationProtectedItem** cria um novo item protegido por replicação.
Use este cmdlet para habilitar a replicação de um item protegido por ASR.
...
Inicia a operação de criação de item protegido de replicação para o item protegido asR especificado e retorna o trabalho ASR usado para controlar a operação.
...
Inicia a operação de criação de item protegido de replicação para o item protegido asR especificado e retorna o trabalho ASR usado para controlar a operação(vmWare para o cenário do Azure).
...
Inicia a operação de criação de item protegido de replicação para o item protegido asR especificado e retorna o trabalho ASR usado para controlar a operação (Azure para o cenário do Azure).
...
Inicia a operação de criação de item protegido de replicação para o VmId especificado e retorna o trabalho ASR usado para controlar a operação (Azure para o cenário do Azure).
...
Inicia a operação de criação de item protegido de replicação para o item protegido asR especificado, incluindo discos seletivos e retorna o trabalho ASR usado para controlar a operação(vmWare para o cenário do Azure) com discos selecionados.
...
Inicia a operação de criação de item protegido de replicação para o VmId especificado e retorna o trabalho ASR usado para controlar a operação (Azure para o cenário do Azure). Para os detalhes de VM de failover passados no cmdlet para criptografia serão usados.
...
Inicia a operação de criação de itens protegidos de replicação para uma Máquina Virtual dentro do grupo de posicionamento de Proximidade e retorna o trabalho ASR usado para controlar a operação (Azure para o cenário do Azure).

## Parâmetros

### -Conta
A conta de executar como a ser usada para instalar o serviço de Mobilidade, se necessário. Deve ser uma da lista de contas de executar como na tela ASR.
...
### -AzureToAzure
A opção parâmetro especifica a criação do item replicado no azure para o cenário do azure.
...
Especifica a configuração de disco a ser usada Vm do Azure para o cenário de recuperação de desastres do Azure.
...
### -AzureVmId
Especifica a ID do VM do Azure para a proteção contra recuperação de desastres na região de recuperação.
...
Especifica a ID do cofre secreto de criptografia de disco (criptografia de disco do Azure) a ser usada para recuperar VM após o failover.
...
### -DiskType
Especifica o tipo de disco gerenciado de VM de Recuperação.
...
### -HyperVToAzure
Alternar parâmetro para especificar o item replicado é uma máquina virtual Hiper-V que está sendo replicada para o Azure.
...
### -IncludeDiskId
A lista de discos a ser incluído para replicação. Por padrão, todos os discos são incluídos.
...
Especifica a entrada de configuração de disco para iD de disco vMWare para proteger contra itens protegidos especificados.
...
### -KeyEncryptionKeyUrl
Especifica a URL da chave de criptografia de disco com a versão(criptografia de disco do Azure) a ser usada para recuperação de VM após o failover.
...
Especifica a ID do cofre de chave de criptografia de disco (criptografia de disco do Azure) a ser usada para recuperar VM após o failover.
...
Os valores aceitáveis para este parâmetro são: Windows ou Linux.
...
### -ProcessServer
O Servidor de Processos a ser usado para replicar este computador. Use a lista de servidores de processo na malha ASR correspondente ao servidor de configuração para especificar um.
...
### -ProtectableItem
Especifica o objeto de item protegido asR para o qual a replicação está sendo habilitada.
...
Especifica a zona de disponibilidade do VM de recuperação após o failover.
...
A sub-rede dentro da rede virtual de recuperação do Azure à qual a falha na máquina virtual deve ser anexada no caso de um failover.
...
Especifica a ID de recurso do serviço de nuvem de recuperação para o failover desse computador virtual.
...
Especifique a ID do grupo de posicionamento de proximidade usada pelo Vm de failover na região de recuperação de destino.
...
Especifica o nome do grupo de replicação a ser usado para criar pontos de recuperação consistentes de vários VM. Por padrão, cada item protegido por replicação é criado em um grupo de seus próprios pontos de recuperação consistentes e vários VM não são gerados. Use essa opção somente se você precisar criar pontos de recuperação consistentes de vários VM em um grupo de máquinas protegendo todas as máquinas para o mesmo grupo de replicação. 
...
### -UseManagedDisk
Especifica se a máquina virtual do Azure criada no failover deve usar discos gerenciados. Ele aceita verdadeiro ou falso.
...
### -VmmToVmm
Alternar parâmetro para especificar o item replicado é uma máquina virtual Hyper-V que está sendo replicada entre sites HiperV gerenciados de VMM.
...
### -V FazerAzure
Alternar parâmetro para especificar o item replicado é um servidor físico ou de computador virtual V Ltd Que será replicado para o Azure.
