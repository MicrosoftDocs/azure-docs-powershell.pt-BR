...
Esses comandos criam um modelo de imagem de máquina virtual.
...
Duração máxima para aguardar durante a criação do modelo de imagem.
...
### - Personalizar
Especifica as propriedades usadas para descrever as etapas de personalização da imagem, como Fonte da imagem etc. Para construir, consulte a seção ANOTAÇÕES para personalizar propriedades e criar uma tabela hash.
...
### -DefaultProfile
Região HideParameter As credenciais, a conta, o locatário e a assinatura usadas para comunicação com o Azure.
...
Para construir, confira a seção ANOTAÇÕES para as propriedades DISTRIBUTE e crie uma tabela hash.
...
Hora de término da última executar (UTC).
...
Informações detalhadas sobre o estado da última executar.
...
Hora de início da última executar (UTC).
...
Para construir, consulte a seção ANOTAÇÕES para as propriedades SOURCE e crie uma tabela hash.
...
Omita ou especifique 0 para usar o tamanho de disco padrão do sistema operacional do Azure.
...
Omita ou especifique a cadeia de caracteres vazia para usar o padrão (Standard_D1_v2).
...
CUSTOMIZE <IImageTemplateCus ltd[]>: especifica as propriedades usadas para descrever as etapas de personalização da imagem, como Fonte de Imagem etc.
  - `Type <String>`: o tipo de ferramenta de personalização que você deseja usar na Imagem. Por exemplo, "Shell" pode ser personalizador de shell
  - `[Name <String>]`: Nome amigável para fornecer contexto sobre o que esta etapa de personalização faz

DISTRIBUTE <IImageTemplateDistributor[]>: os destinos de distribuição para os quais a saída da imagem precisa ir.
  - `RunOutputName <String>`: o nome a ser usado para o RunOutput associado.
  - `Type <String>`: Tipo de distribuição.
  - `[ArtifactTag <IImageTemplateDistributorArtifactTags>]`: marcas que serão aplicadas ao artefato depois que ele tiver sido criado/atualizado pelo distribuidor.
    - `[(Any) <String>]`: indica que qualquer propriedade pode ser adicionada a este objeto.

<IImageTemplateSource>ORIGEM: descreve uma fonte de imagem de máquina virtual para criar, personalizar e distribuir.
