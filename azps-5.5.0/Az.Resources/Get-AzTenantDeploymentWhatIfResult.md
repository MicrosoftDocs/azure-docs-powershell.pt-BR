...
## Descrição
O cmdlet **Get-AzTenantDeploymentWhatIfResult** obtém o modelo ARM What-If resultado de uma implantação de modelo no escopo de locatário especificado. Ela retorna uma lista de alterações indicando quais recursos serão atualizados se a implantação for aplicada sem fazer alterações nos recursos reais. Para especificar o formato do resultado de retorno, use o parâmetro *Formatação* De resultado.

## Exemplos

### Exemplo 1: Obter um What-If no escopo do locatário
```powershell
...
Esse comando obtém um What-If no escopo do locatário usando um arquivo de modelo personalizado e um arquivo de parâmetro no disco.
O comando usa o *parâmetro Local* para especificar onde armazenar os dados de implantação.
...
### Exemplo 2: Obter um What-If no escopo do locatário com ResourceIdOnly
```powershell
PS C:\> Get-AzTenantDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -Location "West US" `
...
O comando usa o parâmetro *TemplateFile* para especificar um arquivo de modelo.
O comando usa o parâmetro *TemplateParameterFile* para especificar um arquivo de parâmetro de modelo.
O comando usa o parâmetro *ResultFormat* para definir o What-If para conter somente IDs de recurso.
...
### -ExcludeChangeType
Lista separada por vírgulas dos tipos de alteração de recursos a serem excluídos What-If resultados.
...
### -Nome
O nome da implantação que ele vai criar.
Se não especificado, o padrão é o nome do arquivo de modelo quando um arquivo de modelo é fornecido; padrão para a hora atual quando um objeto de modelo é fornecido, por exemplo, "20131223140835".
