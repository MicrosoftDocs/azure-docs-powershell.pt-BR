...
## Descrição
O cmdlet **Stop-AzDeployment** cancela uma implantação no escopo da assinatura que foi iniciada, mas não concluída.
...
Para criar uma implantação no escopo da assinatura, use o cmdlet New-AzDeployment assinatura.

Este cmdlet interrompe apenas uma implantação em execução. Use o *parâmetro Nome* para interromper uma implantação específica.
...
Esse comando cancela uma "implantação01" de implantação em execução no escopo da assinatura atual.
...
Esse comando obtém a "implantação01" de implantação no escopo da assinatura atual e o cancela. 
