...
Use os parâmetros respectivos para listar negar atribuições a um usuário específico ou para listar negar atribuições em um grupo ou recurso de recursos específicos.
...
## Descrição
Use o comando Get-AzDenyAssignment para listar todas as atribuições negadas que são eficazes em um escopo.
Sem parâmetros, esse comando retorna todas as atribuições de negação feitas sob a assinatura.
Essa lista pode ser filtrada usando parâmetros de filtragem de entidade, negar nome e escopo da atribuição.
...
Isso filtrará as atribuições que são efetivas nesse escopo específico, ou seja, todas negarão atribuições nesse escopo e acima.
B.
ResourceGroupName – Nome de qualquer grupo de recursos sob a assinatura.
Isso filtrará as atribuições efetivas no grupo de recursos especificado, ou seja, todas negarão atribuições nesse escopo e acima.
C.
ResourceName, ResourceType, ResourceGroupName e (opcionalmente) ParentResource – identifica um determinado recurso sob a assinatura e filtrará as atribuições negadas efetivas nesse escopo de recurso.
Para determinar qual acesso foi negado a um determinado usuário na assinatura, use a opção ExpandPrincipalGroups.
...
Obtém todas as atribuições negadas feitas ao john.doe@contoso.com usuário no teste de escopoRG e acima.
...
Obtém atribuições negadas no escopo do site "site1".
...
Se especificado, retornará as atribuições negadas diretamente atribuídas ao usuário e aos grupos dos quais o usuário é membro (transitamente).
...
Os filtros negam todas as atribuições feitas ao diretor especificado.
...
As listas negam atribuições que são eficazes no grupo de recursos especificado.
Quando usadas em conjunto com parâmetros ResourceName, ResourceType e ParentResource, as listas de comandos negam atribuições efetivas nos recursos dentro do grupo de recursos.
...
O comando filtra todas as atribuições negadas que são eficazes nesse escopo.
...
Os filtros negam todas as atribuições feitas ao aplicativo Azure AD especificado.
...
Os filtros negam todas as atribuições feitas ao usuário especificado.
