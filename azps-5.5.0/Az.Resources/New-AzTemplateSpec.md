...
## Descrição
Cria uma nova versão de Especificação de Modelo com o conteúdo de Modelo ARM especificado. O conteúdo pode ser de uma cadeia de caracteres JSON bruto (usando o conjunto de parâmetros **FromJsonStringParameterSet)** ou de um arquivo JSON especificado (usando o conjunto de parâmetros **FromJsonFileParameterSet).**  

Se a Especificação raiz do Modelo ainda não existir, ela será criada juntamente com a versão da Especificação de Modelo. Se uma Especificação de Modelo já existir com o nome determinado, ela e a versão especificada serão atualizadas (quaisquer outras versões existentes serão preservadas).
...
Cria uma nova versão da Especificação de Modelo "v1.0" em uma Especificação de Modelo chamada "myTemplateSpec". A versão especificada terá $templateJson como o conteúdo do modelo ARM da versão.

 **Observação:** O Modelo ARM no exemplo é uma não-operação, pois não contém recursos reais.
...
Cria uma nova versão da Especificação de Modelo "v2.0" em uma Especificação de Modelo chamada "myTemplateSpec". A versão especificada terá o conteúdo do arquivo local "myTemplateContent.jsem" como o conteúdo do Modelo ARM da versão.
...
### -Forçar
Não peça confirmação ao sobrescrever uma versão existente.
...
### -Local
O local da especificação do modelo. Só será necessário se a especificação do modelo ainda não existir.
...
### -TemplateFile
O caminho do arquivo para o arquivo JSON do modelo local do Gerenciador de Recursos do Azure.
