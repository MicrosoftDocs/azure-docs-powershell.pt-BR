...
Aliases de política são usados pela Política do Azure para se referir às propriedades do tipo de recurso.
São fornecidos parâmetros que limitam os itens na listagem, combinando várias propriedades do tipo de recurso ou seus aliases.
Um determinado valor de match corresponde se a cadeia de caracteres de destino contiver-a usando a comparação de maiúsculas e minúsculas.
...
Lista todos os tipos de recursos do provedor cujo namespace corresponde a 'computar' e contém um alias.
...
Lista todos os tipos de recursos do provedor cujo namespace corresponde a "computação" e o tipo de recurso corresponde a "virtual" e contém um alias.
...
Lista todos os tipos de recursos do provedor que contêm um alias 'virtual' correspondente.
...
Lista todos os tipos de recursos do provedor que contêm um alias correspondente "virtual" ou um alias com uma 'rede' correspondente ao caminho.
...
Lista todos os tipos de recursos do provedor com a versão da api alfa ou contendo um alias com uma versão da api alfa.
...
### -ApiVersion
Quando definido, indica a versão da API do provedor de recursos a ser usada. Se não especificado, a versão da API será determinada automaticamente como a mais recente disponível.
...
### -ListAvailable
Inclui os itens correspondentes de saída com e sem aliases.
...
* Para expandir os Aliases ou qualquer outra propriedade, canaliza a saída para `select -ExpandProperty <property>` . Por exemplo: `Get-AzPolicyAlias -NamespaceMatch 'Microsoft.Cache' -ApiVersionMatch 'alpha' | select -ExpandProperty Aliases | select -Property Name -ExpandProperty Paths`

* Propriedades adicionais estão disponíveis na saída e podem ser exibidas por meio da canalização da saída `Format-List` para. Por exemplo: `Get-AzPolicyAlias -NamespaceMatch 'Web' -ResourceTypeMatch site -PathMatch cert | Format-List`
