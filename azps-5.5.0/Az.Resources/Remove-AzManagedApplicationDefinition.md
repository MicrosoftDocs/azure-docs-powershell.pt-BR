...
### Exemplo 1: Remover definição de aplicativo gerenciado por ID de recurso
```
...
O primeiro comando obtém uma definição de aplicativo gerenciado chamada myAppDef usando o cmdlet Get-AzManagedApplicationDefinition usuário.
O comando o armazena na variável $ApplicationDefinition dados.
O segundo comando remove a definição de aplicativo gerenciado identificada pela propriedade **ResourceId** do $ApplicationDefinition.
...
### -ID
A ID de definição de aplicativo gerenciado totalmente qualificada, incluindo a assinatura.
