...
E para especificar um aplicativo Azure AD, use parâmetros ApplicationId ou ObjectId.
...
Conceder acesso do leitor a uma entidade de serviço
...
### -Condição
Condição a ser aplicada à FunçãoAssignação.
...
### -InputFile
Caminho para a tarefa de função json
...
Só deve ser usado em conjunto com parâmetros ResourceGroupName, ResourceType e ResourceName para construir um escopo hierárquico na forma de um URI relativo que identifique um recurso.
...
Cria uma atribuição que é eficaz no grupo de recursos especificado.
...
Só devem ser usados em conjunto com parâmetros ResourceGroupName, ResourceType e (opcionalmente)ParentResource para construir um escopo hierárquico na forma de um URI relativo que identifique um recurso.
...
### -RoleDefinitionId
ID da função RBAC que precisa ser atribuída ao diretor.
...
Se não especificado, criará a atribuição de função no nível da assinatura.
