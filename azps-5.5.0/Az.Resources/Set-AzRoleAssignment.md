...
## Sinopse
Atualizar uma Atribuição de Função existente.
...
Descrições podem ser qualquer cadeia de caracteres válida, use-as para diferentá-la umas das outras.  
se a Condição estiver definida, a versão da condição também deve ser definida, mas se você estiver atualizando uma Condição que não é necessária.
A versão de condição pode ser atualizada de 1.0 para 2.0, mas não pode ser rebaixada. Seja cuidadoso, pois 2.0 não é retrocompatível com 1.0.
...
### -InputFile
Nome de arquivo que contém uma única definição de função.
...
### -PassThru
Se especificado, exibirá a atribuição de função atualizada
