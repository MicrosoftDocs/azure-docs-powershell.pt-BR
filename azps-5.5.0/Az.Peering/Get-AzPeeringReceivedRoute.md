...
## Descrição
Listas de rotas recebidas de um Peering

## Exemplos

### Listar as 100 principais rotas recebidas para um peering
```powershell
PS C:\> Get-AzPeeringReceivedRoute -ResourceGroupName $resourceGroupName -Name $peeringName
```

Lista todas as rotas recebidas para um peering

### Filtrar por caminho AS
```powershell
PS C:\> Get-AzPeeringReceivedRoute -ResourceGroupName $resourceGroupName -Name $peeringName -AsPath "1234 5674 9834"
```

Lista todas as rotas recebidas para um peering com um filtro em AS 

### Filtrar por LTDKIValidationState
```powershell
PS C:\> Get-AzPeeringReceivedRoute -ResourceGroupName $resourceGroupName -Name $peeringName -RPKIValidationState "Valid"
```

Lista todas as rotas recebidas para um peering com um filtro emEIQUIVALidationState
...
Lista todas as rotas recebidas para um peering com um filtro no OriginAsValidationState
...
Filtrar por estado de validação AS de origem.
...
### -EIKIValidationState
Filtrar por estado de validação DE ACORDOKI.
...
### Microsoft.Azure.PowerShell.Cmdlets.Peering.Models.PSPeeringReshellRoute
