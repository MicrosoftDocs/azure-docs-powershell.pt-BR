...
## Descrição
Crie um detalhe de contato PeerAsn na memória.

## Exemplos

### Criar detalhes de contato e adicionar ao PeerAsn
```powershell
...
Função e email são necessários. O telefone é opcional. O telefone dá suporte a +-() ou espaços. Um PeerAsn deve incluir pelo menos um detalhe de contato do tipo "Noc"
...
### -Email
Endereços de email usados para entrar em contato se os problemas normalmente são uma Central de Operações de Rede
...
### -Telefone
Telefone usado para entrar em contato se os problemas normalmente são uma Central de Operações de Rede
...
### -Função
Escolha a função mais adequada às informações de contato.
O Contato NOC é obrigatório.
