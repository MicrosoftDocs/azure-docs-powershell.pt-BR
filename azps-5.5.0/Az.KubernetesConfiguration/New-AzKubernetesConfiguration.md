...
## Sinopse
Crie uma nova Configuração de Controle de Origem das Redes.
...
### Exemplo 1: Criar uma configuração para cluster de soluções de rede
```powershell
...
Esse comando cria uma configuração para cluster de rede de rede.

### Exemplo 1: Criar uma configuração para cluster de soluções com especificar OperadorNamespace de paramter
```powershell
...
Esse comando cria uma configuração no novo espaço de nome de operador para cluster de lança-redes.
Observação: Não é possível criar uma configuração em um namespace de operador existente.
...
### -ClusterScoped
Se aprovado, de acordo com o escopo da Configuração como Cluster (o padrão é nameSpace).
...
### -EnableHelmOperator
Opção para habilitar o Operador de Operador Debilitar para esta configuração de git.
...
### -OperatorChartValues
Os valores substituem o operador Gráfico de Gráfico de Gráfico de Gráficos.
...
Versão do gráfico de gráfico de gráfico de gráfico de gráficos do operador.
...
### -OperatorNamespace
O namespace para o qual este operador está instalado.
Máximo de 253 caracteres alfanuméricos de caso inferior, hífen e somente ponto.
...
### -OperatorParameters
Todos os Parâmetros da instância operador no formato de cadeia de caracteres.
