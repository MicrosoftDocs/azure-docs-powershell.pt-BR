...
## Sinopse
Atualize os campos de mutável da implantação de Borda IoT.
...
Observação: o conteúdo da configuração é imutável. As propriedades de configuração que podem ser atualizadas são "rótulos", "métricas", "prioridade" e "targetCondition".
...
Altere a prioridade da implantação do Edge IoT e atualize sua condição de destino.
...
Atualize as métricas e rótulos da implantação de Borda IoT.
...
### -Forçar
Permite que o objeto de implantação seja substituído mesmo se ele tiver sido atualizado desde que foi recuperado da última vez.
...
### -Métrica
Conjunto de consultas para definição de métricas de implantação de borda IoT.
...
### -TargetCondition
Condição de destino à qual uma implantação de borda se aplica.
