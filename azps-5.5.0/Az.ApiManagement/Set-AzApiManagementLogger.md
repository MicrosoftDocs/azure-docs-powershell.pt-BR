...
## Sinopse
Modifica uma Api Management Lo também.
...
## Descrição
O cmdlet **Set-AzApiManagementLogger** modifica as configurações de um **Loear** de Gerenciamento de API do Azure.

## Exemplos

### Exemplo 1: Modificar loção eventHub
```powershell
...
Esse comando modifica um lago que tem a ID Loção123.

### Exemplo 2

Modifica uma Api Management Lo também. (gerado automaticamente)
...
### -ConnectionString
Especifica uma cadeia de conexão do Hub de Eventos do Azure que inclui os direitos de política Enviar.
...
### -IsBuffered
Especifica que os registros no lo buffer são armazenados em buffer antes da publicação.
...
### -LoggerId
Especifica a ID do lago a ser atualizado.
...
### -PassThru
Indica que esse cmdlet retorna  **o PsApiManagementLogger** que este cmdlet modifica.
