...
O cmdlet **New-AzApiManagementApiRevision** cria uma Revisão da API para uma API existente no contexto de Gerenciamento de API.
...
Esse comando cria uma Revisão da `2` `echo-api` API.

### Exemplo 2: Criar uma Revisão da API a partir de uma Api Existente e copiar Todas as operações, marcas e Políticas
```powershell
...
Esse comando cria uma Revisão da `5` `echo-api` API.
...
### -ServiceUrl
Uma URL do serviço Web que expõe a API no serviço Backend. Essa URL será usada apenas pelo Gerenciamento de API do Azure e não será pública. Deve ter de 1 a 2000 caracteres. Esse parâmetro é necessário.
