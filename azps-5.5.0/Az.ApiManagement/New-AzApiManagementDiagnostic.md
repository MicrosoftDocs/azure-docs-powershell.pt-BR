...
## Descrição
O cmdlet **New-AzApiManagementDiagnostic** cria uma entidade de diagnóstico no escopo Global ou escopo de Api específico.
...
O exemplo acima cria um diagnóstico para que a API registre o Header e `httpbin` 100 Bytes de Corpo `azuremonitor` para loção.

## Parâmetros

### -AlwaysLog
Especifica quais tipos de configurações de amostragem de mensagens não devem ser aplicadas.
...
### -BackendSetting
Configuração de diagnóstico para mensagens http de entrada/saída no Backend. Este parâmetro é opcional.
...
### -LoggerId
Identificador do loção para pressionar o diagnóstico.
Esse parâmetro é necessário.
