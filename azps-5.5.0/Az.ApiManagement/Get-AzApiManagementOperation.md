...
## Descrição
O **Get-AzApiManagementOperation** obtém uma lista ou uma operação de API especificada.
...
Esse comando obtém uma operação de gerenciamento de API pela ID da operação chamada Operation0003.
...
### -ApiRevision
Identificador de Revisão da API. Este parâmetro é opcional. Se não especificado, a operação será recuperada da revisão da api ativa no momento.
