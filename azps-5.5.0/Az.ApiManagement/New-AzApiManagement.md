...
### Exemplo 1: Criar um serviço de Gerenciamento de API de nível de desenvolvedor
```powershell
...
Esse comando cria um serviço de Gerenciamento de API de nível de desenvolvedor.
...
### Exemplo 2: Criar um serviço de camada padrão que tenha três unidades
```powershell
PS C:\>New-AzApiManagement -ResourceGroupName "ContosoGroup02" -Name "ContosoApi" -Location "Central US" -Organization "Contoso" -AdminEmail "admin@contoso.com" -Sku Standard -Capacity 3
```

Esse comando cria um serviço de Gerenciamento de API de nível padrão que tem três unidades.

### Exemplo 3: Criar um serviço de camada consumo
```powershell
...
Esse comando cria um serviço de Gerenciamento de API de nível de consumo com Certificado de Cliente habilitado no oeste da Europa.
...
Esse comando cria um serviço de Gerenciamento de API de nível premium em uma sub-rede de rede virtual do Azure com um ponto de extremidade de gateway voltado para externo com uma região mestra no Oeste dos EUA.
...
Esse comando cria um serviço de Gerenciamento de API de nível premium em uma sub-rede de rede virtual do Azure com um ponto de extremidade de gateway voltado para interno com uma região mestra no Oeste dos EUA.
...
Esse comando cria um serviço de Gerenciamento de Api SKU Padrão e Habilita o TLS 1.0 no cliente Frontend para o cliente ApiManagement Gateway e Backend entre ApiManagement Gateway e Backend.

## Parâmetros

### -AdditionalRegions
Regiões de implantação adicionais do Gerenciamento de API do Azure.
...
### -Capacidade
Especifica a capacidade da SKU do serviço de Gerenciamento de API do Azure.
O padrão é um (1).
...
### -EnableClientCertificate
O sinalizador só deve ser usado para o Serviço de ApiManagement SKU de Consumo. Isso impõe que um certificado de cliente seja apresentado em cada solicitação ao gateway. Isso também permite autenticar o certificado na política no gateway.
...
### -ResourceGroupName
Especifica o nome do grupo de recursos no qual esse cmdlet cria uma implantação de Gerenciamento de API.
...
### -SKU
Especifica o nível do serviço de Gerenciamento de API.
...
Certificados emitidos pela CA interna para serem instalados no serviço. O valor padrão é $null.
...
- "Nenhum" (Valor Padrão. ApiManagement não faz parte de nenhuma Rede Virtual")
- "Externo" (a Implantação apiManagement é configurada dentro de uma Rede Virtual com um Ponto de Extremidade Voltado para a Internet)
- "Interno" (ApiManagement Deployment is setup inside a Virtual Network having an Intranet Facing Endpoint)
...
### System.Nullable'1[[Microsoft.Azure.Commands.ApiManagement.Models.PsApiManagementSku, Microsoft.Azure.PowerShell.Cmdlets.ApiManagement, Version=1.0.0.0,0, Culture=neutral, PublicKeyToken=null]]
