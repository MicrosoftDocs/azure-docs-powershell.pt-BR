...
## Descrição
O **cmdlet Set-AzApiManagementOperation define** os detalhes da operação da API.
...
Esse comando define os detalhes da operação para o gerenciamento de API.

### Exemplo 2

Define detalhes da operação da API. (gerado automaticamente)
...
### -ApiRevision
Identificador de Revisão da API. Este parâmetro é opcional. Se não especificado, a operação será atualizada na revisão da api ativa no momento.
...
### -Respostas
Especifica uma matriz de possíveis respostas de operação.
...
Se você não especificar um valor, um valor padrão será gerado com base na UrlTemplate.
Use o parâmetro para dar mais detalhes sobre parâmetros como descrição, tipo e outros valores possíveis.
