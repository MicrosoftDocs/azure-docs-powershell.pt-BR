...
### Exemplo 2: Criar uma API copiando todas as operações, marcas, produtos e políticas da echo-api e para um ApiVersionSet
```powershell
...
Esse comando cria uma API no ApiVersionSet e copia todas as operações, marcas e `echoapiv3` políticas da Api de `xmsVersionSet` `echo-api` origem. Ele substitui SubscriptionRequired, ServiceUrl, Path, Protocols
...
Api Version Description. Este parâmetro é opcional.
...
Esse é o nome público da API como aparece nos portais de desenvolvedor e administrador.
...
### -Caminho
Especifica o caminho da API da Web, que é a última parte da URL pública da API e corresponde ao campo de sufixo da URL da Web no portal de administração.
...
### -SourceApiRevision
Revisão da API de origem. Este parâmetro é opcional.
