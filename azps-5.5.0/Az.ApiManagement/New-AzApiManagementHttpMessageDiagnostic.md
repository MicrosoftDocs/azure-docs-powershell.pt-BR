...
## Sinopse
Cria uma instância **do PsApiManagementHttpMessageDiagnostic,** que é uma configuração de diagnóstico de mensagem http do Diagnóstico
...
Criar uma configuração de diagnóstico de mensagem http para log `Content-Type` e `User-Agent` cabeçalhos junto com 100 bytes de `body`

## Parâmetros

### -BodyBytesToLog
Número de bytes do corpo de solicitação para log. Este parâmetro é opcional.
...
### -HeadersToLog
A matriz de headers para log. Este parâmetro é opcional.
