...
## Sinopse
Cria uma Api Management Lo também.
...
## Descrição
O cmdlet **New-AzApiManagementLogger cria** um **Logger** de Gerenciamento de API do Azure.

## Exemplos

### Exemplo 1: Criar um loção
```powershell
...
Esse comando cria um loção chamado ContosoSdkEventHub usando a cadeia de conexão especificada.

### Exemplo 2

Cria uma Api Management Lo também. (gerado automaticamente)
...
### -ConnectionString
Especifica uma cadeia de conexão dos Hubs de Eventos do Azure que começa com o seguinte: `Endpoint=endpoint and key from Azure classic portal`
...
### -InstrumentationKey
Chave de Instrumentação do insights do aplicativo. Este parâmetro é opcional.
...
### -IsBuffered
Especifica se os registros no lo buffer são armazenados em buffer antes da publicação.
...
### -LoggerId
Especifica uma ID para o loção.
