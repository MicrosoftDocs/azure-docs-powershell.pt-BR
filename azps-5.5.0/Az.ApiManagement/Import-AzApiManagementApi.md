...
## Descrição
O cmdlet **Import-AzApiManagementApi** importa uma API de gerenciamento de API do Azure de um arquivo ou uma URL no formato DESCRIÇÃO do Aplicativo Web (URL), WSDL (Linguagem de Descrição dos Serviços Web) ou formato Desmancha.

## Exemplos

### Exemplo 1: Importar uma API de um arquivo WADEL
```powershell
...
Esse comando importa uma API do arquivo COTAL especificado.

### Exemplo 2: Importar uma API de um arquivo Descarada
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Esse comando importa uma API do arquivo Descarado especificado.

### Exemplo 3: Importar uma API de um link DESL
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Esse comando importa uma API do link DESL especificado.

### Exemplo 4: Importar uma API de um Link open api
```powershell
...
### Exemplo 5: Importar uma API de um Link da Api Open para um Conjunto de ApiVersion
...
### -ApiRevision
Identificador de Revisão da API. Este parâmetro é opcional. Se não especificado, a importação será feita na revisão ativa no momento ou em uma nova api.
...
### -ApiType
Este parâmetro é opcional com um valor padrão de Http. A opção Soap só é aplicável ao importar o WSDL e criará uma API SOAP Passthrough.
...
### -ApiVersion
Api Version of the Api to create. Este parâmetro é opcional.
...
### -ApiVersionSetId
Um identificador de recurso para o Conjunto de Versão da Api relacionado. Este parâmetro é opcional.
...
Essa URL é usada pelos consumidores da API para enviar solicitações para o serviço Web.
...
### -Protocolo
Protocolos de API da Web (https). Protocolos sobre os quais a API é disponibilizada. Este parâmetro é opcional. Se fornecido, ele substituirá os protocolos especificados no documento de especificações.
...
### -ServiceUrl
Uma URL do serviço Web que expõe a API. Essa URL será usada apenas pelo Gerenciamento de API do Azure e não será pública. Este parâmetro é opcional. Se fornecido, ele substituirá a ServiceUrl especificada no documento Especificações.
...
psdx_paramvalues, Badal, Wsdl e Seta.
...
### -SpecificationUrl
Especifica a URL da especificação.
...
### System.Nullable'1[[Microsoft.Azure.Commands.ApiManagement.ServiceManagement.Models.PsApiManagementApiType, Microsoft.Azure.PowerShell.Cmdlets.ApiManagement.ServiceManagement, Version=1.0.0.0.0, Culture=neutral, PublicKeyToken=null]]
