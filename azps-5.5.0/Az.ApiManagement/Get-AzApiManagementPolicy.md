...
## Sinopse
Obtém a política de escopo especificada.
...
Esse comando obtém uma política de nível de locatário e a salva em um arquivo chamado tenantpolicy.xml.
...
### Exemplo 3: Obter a política de escopo da API
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementPolicy -Context $apimContext -ApiId "9876543210"
```

Esse comando obtém uma política de escopo da API.
...
Esse comando obtém a política de escopo de operação.

### Exemplo 5: Obter a política de escopo locatário no formato RawXml
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Esse comando obtém a política de escopo do locatário no formato de escape não Xml.

## Parâmetros

### -ApiId
Especifica o identificador da API existente.
Se você especificar esse parâmetro, o cmdlet retornará a política de escopo da API.
...
### -ApiRevision
Identificador de Revisão da API. Este parâmetro é opcional. Se não especificado, a política será recuperada da revisão da api ativa no momento.
...
### -Formatar
Especifica o formato da política de gerenciamento da API.
...
### -SalvarAs
Especifica o caminho do arquivo para salvar o resultado.
Se você não especificar esse parâmetro, o resultado será pipelineado como uma ferramenta.
