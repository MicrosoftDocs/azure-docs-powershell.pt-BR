...
## Descrição
O **Get-AzApiManagementDiagnostic** obtém detalhes do diagnóstico configurado no serviço de gerenciamento de Api em um determinado escopo.

## Exemplos

### Exemplo 1: Configure todo o diagnóstico no escopo do locatário.
```powershell
...
Esse comando configura todos os diagnósticos no serviço de Gerenciamento de Api.

### Exemplo 2: Configurar todos os diagnósticos no escopo da Api
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Esse comando configura todos os diagnósticos no escopo `echo-api` da Api
...
Esse comando configura `applicationinsights` o diagnóstico na `echo-api` API.
...
### -ResourceId
Identificador de Recursos Arm de um Diagnóstico ou Diagnóstico de Api. Se especificado, tentará encontrar o diagnóstico pelo identificador. Esse parâmetro é necessário.
