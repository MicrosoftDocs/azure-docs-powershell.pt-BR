...
## Sinopse
Criar ou atualizar um HSM dedicado na assinatura especificada.
...
Esse comando cria um HSM Dedicado em uma rede virtual existente.

**OBSERVAÇÃO:** Atualmente, `New-AzDedicatedHsm` ela tem uma limitação que retorna antes que o HSM seja totalmente provisionado no Azure.
Portanto, depois de criar um novo HSM, consulte seu estado e verifique se `Get-AzDedicatedHsm` ele está antes de `Provisioning State` `Succeeded` usá-lo.
...
Para construir, confira a seção ANOTAÇÕES para propriedades NETWORKINTERFACE e crie uma tabela hash.
...
### -StampId
Esse campo será usado quando NÃO FOR NECESSÁRIO dar suporte a zonas de disponibilidade.
...
NETWORKINTERFACE <INetworkInterface[]>: Especifica a lista de IDs de recursos para as interfaces de rede associadas ao HSM dedicado.
  - `[PrivateIPAddress <String>]`: Endereço Ip particular da interface
