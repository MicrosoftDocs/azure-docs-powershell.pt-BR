...
### Exemplo 1: Atualizar o tamanho da nuvem particular por nome
```powershell
...
IDENTITYSOURCE <IIdentitySource[]>: vCenter Single Sign On Identity Sources
  - `[Alias <String>]`: Nome do NetBIOS do domínio
  - `[BaseGroupDn <String>]`: O nome diferenciado da base para grupos
  - `[BaseUserDn <String>]`: O nome diferenciado da base para os usuários
  ...
  - `[PrimaryServer <String>]`: URL do servidor principal
  - `[SecondaryServer <String>]`: URL do servidor secundário
  - `[Ssl <SslEnum?>]`: Proteger a comunicação LDAP usando certificado SSL (LDAPS)
  - `[Username <String>]`: A ID de um usuário do Active Directory com acesso mínimo somente leitura ao DN Base para usuários e grupos
