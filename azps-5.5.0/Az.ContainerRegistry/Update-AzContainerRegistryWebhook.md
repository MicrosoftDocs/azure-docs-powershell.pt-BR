...
## Descrição
O Update-AzContainerRegistryWebhook cmdlet atualiza um web registry de contêiner.

## Exemplos

### Exemplo 1: Atualizar um webtare de registro de contêiner existente.
```powershell
...
Atualize uma Web de registro de contêiner existente.

## Parâmetros

### -Ação
Lista de ações separadas por espaços que acionam a Webção para postar notificações.
...
### -Header
Espaços separados por headers personalizados no formato 'key \[ \] =value' que serão adicionados às notificações web widget.
...
### -Uri
O URI de serviço do web browser para postar notificações.
...
### -Web browser
Objeto Web browser do Registro de Contêineres.
