...
## Descrição
Remover um diagrama que foi atribuído a uma assinatura.
...
Remova a atribuição de diagrama especificada pelo nome da assinatura especificada. O cmdlet solicitará confirmação antes de executar o comando.
...
### -InputObject
Objeto de atribuição de projeto.
...
### -ManagementGroupId
A ID do grupo de gerenciamento onde a atribuição De plantas é salva.
...
### -PassThru
Quando definido, o cmdlet retornará um objeto que representa a atribuição De diagrama removida. Por padrão, esse cmdlet não gera saída.
...
### -SubscriptionId
A ID da assinatura para a atribuição de planta foi implantada.
