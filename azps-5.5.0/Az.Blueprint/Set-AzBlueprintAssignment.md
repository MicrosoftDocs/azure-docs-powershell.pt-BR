...
Atualize uma atribuição de diagrama existente da definição de diagrama dentro da `$blueprintObject` assinatura especificada, atualizando os parâmetros. Usa a identidade atribuída pelo sistema. O local define a região para a criação da identidade gerenciada.
...
Atualize uma atribuição de plano de trabalho existente da definição de diagrama visando a assinatura especificada dentro do `$blueprintObject` grupo de gerenciamento especificado usando o parâmetro definido.
...
### -ManagementGroupId
A ID do grupo de gerenciamento onde as atribuições do plano de trabalho serão salvas.
...
### -SubscriptionId
SubscriptionId para atribuir o Diagrama.
