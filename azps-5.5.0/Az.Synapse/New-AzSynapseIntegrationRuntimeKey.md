...
## Sinopse
Regenerar a chave de tempo de execução de integração auto-hospedada.

## Sintaxe

### NewByNameParameterSet (Padrão)
```
...
## Descrição
O cmdlet **New-AzSynapseIntegrationRuntimeKey** regenera a tecla de tempo de execução de integração com o nome da chave especificado pelo parâmetro 'KeyName'. A tecla anterior será inválida.
...
O cmdlet regenera a chave "authKey2" para tempo de execução de integração chamado "test-selfhost-ir".
...
### -KeyName
O nome da chave de autenticação do tempo de execução de integração auto-hospedado.
