...
## Descrição
O **cmdlet Set-AzSynapseTrishot cria** um gatilho em um espaço de trabalho. Os gatilhos são criados no estado "Parado", o que significa que eles não começam imediatamente a invocar pipelines referenciados.
...
Crie um novo gatilho chamado ContosoTrição no espaço de trabalho ContosoWorkspace. O gatilho é criado no estado "Parado", o que significa que ele não inicia imediatamente. Um gatilho pode ser iniciado usando o `Start-AzDataFactoryV2Trigger` cmdlet.
...
Crie um novo gatilho chamado ContosoTri pipeline no espaço de trabalho ContosoWorkspace. O gatilho é criado no estado "Parado", o que significa que ele não inicia imediatamente. Um gatilho pode ser iniciado usando o `Start-AzDataFactoryV2Trigger` cmdlet.
