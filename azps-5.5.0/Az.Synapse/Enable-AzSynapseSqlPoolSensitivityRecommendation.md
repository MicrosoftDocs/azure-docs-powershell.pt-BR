...
## Sinopse
Habilita recomendações de sensibilidade em colunas (as recomendações são habilitadas por padrão em todas as colunas) no pool sql.
...
## Descrição
O Enable-AzSynapseSqlPoolSensitivityRecommendation cmdlet permite recomendações de sensibilidade em colunas no pool SQL.

## Exemplos

### Exemplo 1: Habilitar recomendações de sensibilidade em uma determinada coluna em um pool SQL do Azure Synapse.
```powershell
...
### Exemplo 2: Habilitar recomendações de sensibilidade em uma determinada coluna do pool SQL Azure Synapse usando o Piping.
```powershell
