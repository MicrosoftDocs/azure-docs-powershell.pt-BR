...
## Descrição
O **comando Invoke-AzSynapse Pipelineline** inicia uma operação no pipeline especificado e retorna uma ID para essa executar. Esse GUID pode ser passado **para Get-AzSynapse GuidlineRun** ou **Get-AzSynapseActivityRun** para obter mais detalhes sobre essa executar.
...
Esse comando inicia uma operação para o pipeline chamado Contoso Pipeline no espaço de trabalho ContosoWorkspace.
...
Esse comando inicia uma operação para o pipeline chamado Contoso Pipeline no espaço de trabalho ContosoWorkspace por meio do pipeline.
...
Se o modo de recuperação estiver definido como verdadeiro, a operação de pipeline especificada referenciada e a nova executar serão agrupadas sob a mesma ID de grupo.
...
A ID de executar o pipeline para executar de uma nova vez.
Se a ID de executar for especificada, os parâmetros da executar especificada serão usados para criar uma nova executar.
...
### -StartActivityName
No modo de recuperação, a reprise iniciará a partir dessa atividade.
Se não especificado, todas as atividades serão executados.
