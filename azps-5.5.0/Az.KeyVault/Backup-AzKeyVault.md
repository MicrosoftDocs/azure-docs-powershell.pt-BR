...
## Descrição
Faça backup completo de um HSM gerenciado em uma conta de armazenamento.
...
O cmdlet criará uma pasta (normalmente nomeada) no contêiner de armazenamento, armazenará o backup nessa pasta e gerará a saída do `mhsm-{name}-{timestamp}` URI da pasta.
