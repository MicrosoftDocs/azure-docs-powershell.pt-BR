...
## Sinopse
Restaura um certificado em um cofre de chave de um arquivo de backup.
...
## Descrição
O cmdlet **Restore-AzKeyVaultCertificate** cria um certificado no cofre de chave especificado de um arquivo de backup.
Este certificado é uma réplica do certificado em backup no arquivo de entrada e tem o mesmo nome do certificado original.
Se o cofre de chave já contiver um certificado com o mesmo nome, esse cmdlet falhará em vez de sobrescrever o certificado original.
Se o backup contiver várias versões de um certificado, todas as versões serão restauradas.
O cofre de chave no que você restaurar o certificado pode ser diferente do cofre de chave do onde você fez backup do certificado.
...
### Exemplo 1: Restaurar um certificado em backup
```powershell
...
Esse comando restaura um certificado, incluindo todas as suas versões, do arquivo de backup chamado Backup.blob para o cofre de chave chamado MyKeyVault.
