...
## Descrição
Use o `Remove-AzKeyVaultRoleAssignment` cmdlet para revogar o acesso a qualquer entidade em determinado escopo e função. O objeto da atribuição, ou seja, o principal DEVE ser especificado. O principal pode ser um usuário (use parâmetros SignInName ou ObjectId para identificar um usuário), grupo de segurança (usar parâmetro ObjectId para identificar um grupo) ou entidade de serviço (use parâmetros ApplicationId ou ObjectId para identificar um ServicePrincipal. A função à que a entidade deve ser atribuída deve ser especificada usando o parâmetro RoleDefinitionName ou RoleDefinitionId.
...
Este exemplo revoga a função "Administrador de Política HSM Gerenciado" de " no escopo user1@microsoft.com "/teclas".
...
### -RoleDefinitionId
A ID da função à que o diretor está atribuído.
...
### -RoleDefinitionName
Nome da função RBAC para atribuir o diretor.
...
### -Escopo
Escopo no qual a atribuição ou definição da função se aplica, por exemplo, '/' ou '/keys' ou '/keys/{keyName}'.
