...
## Sinopse
Obter ou listar atribuições de função de um HSM gerenciado. Use parâmetros respectivos para listar atribuições para um usuário específico ou uma definição de função.
...
## Descrição
Use o `Get-AzKeyVaultRoleAssignment` comando para listar todas as atribuições de função que são eficazes em um escopo.
Sem parâmetros, esse comando retorna todas as atribuições de função feitas sob o HSM gerenciado.
...
O escopo no qual o acesso está sendo concedido pode ser especificado. Ele é padrão para "/".
...
Este exemplo lista todas as atribuições de função de "myHsm" no escopo "/keys" e filtra o resultado por nome de login do usuário.
