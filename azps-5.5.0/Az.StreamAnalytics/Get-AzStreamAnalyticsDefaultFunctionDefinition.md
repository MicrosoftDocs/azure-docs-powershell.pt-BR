...
Você pode usar a definição padrão e New-AzStreamAnalyticsFunction cmdlet para criar uma função.
Você pode modificar a definição padrão antes de criar uma função.

## Exemplos

### Exemplo 1: Obter a definição padrão de uma função de Análise de Fluxo
```
...
Esse comando obtém a definição padrão da função chamada ScoreTweet usando os parâmetros especificados no RetrieveDefaultDefinitionRequest.jsno arquivo.
...
### -Arquivo
Especifica o caminho de um arquivo .json que contém a representação de JSON (Notação de Objeto JavaScript) do corpo da solicitação para a solicitação de definição de função padrão de recuperação.
...
Esse cmdlet obtém a definição padrão de uma função no trabalho especificado por esse parâmetro.
...
### -Nome
Especifica o nome da função Análise de Fluxo para a qual este cmdlet obtém a definição padrão.
