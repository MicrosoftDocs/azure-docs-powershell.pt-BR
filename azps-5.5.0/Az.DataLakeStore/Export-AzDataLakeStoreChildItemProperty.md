...
## Descrição
A **Propriedade Export-AzDataLakeStoreChildItemProperty** é usada para relatar o uso de espaço ADLS ou/e OCL para o diretório determinado e seus sub-diretórios e arquivos.

## Exemplos

### Exemplo 1: Obter o uso de disco e o uso de ACL para todos os subdiretários e arquivos
```
...
Obter o uso de ACL para todos os subdireários e arquivos em /a. IncludeFile garante que o uso dos arquivos também seja relatado. HideconsistentAcl in this case will show the Acl of /a, not it's children since all of the children has same acl as /a. Esse sinalizador ignora a saída de acl da subárvore se todos os acls são iguais à raiz.

## Parâmetros

### -Conta
A conta do Data Lake Store para executar a operação de sistema de arquivos em
...
### -GetAcl
Recupera o acl iniciando do caminho raiz
...
### -GetDiskUsage
Recupera o uso do disco iniciando do caminho raiz
...
### -HideConsistentAcl
Não mostrar subárvore de diretório se as ACLs são iguais em toda a subárvore. Isso torna mais fácil ver apenas os caminhos até os quais as ACLs são diferentes. Por exemplo, se todos os arquivos e pastas em /a/b são iguais, não mostre a subárvoreunder /a/b e apenas saída /a/b com 'True' na coluna Consistente ACLNão será definido se IncludeFiles não estiver definido, pois o Acl consistente não pode ser determinado sem recuperar acls para os arquivos.
...
### -MaximumDepth
Profundidade máxima do diretório raiz até que o uso de disco ou acl seja exibido
...
### -OutputPath
Caminho para o arquivo de saída. Pode ser um caminho local ou Caminho do Adl. Por padrão, ele é local. Se SalvarToAdl for especificado, ele será um caminho ADL na mesma conta
...
### -Caminho
O caminho na conta especificada do Lago de Dados que deve ser recuperada.
Pode ser um arquivo ou pasta No formato '/pasta/file.txt', onde o primeiro '/' após o DNS indica a raiz do sistema de arquivos.
...
### -SaveToAdl
Se aprovado, salva o arquivo de despejo no ADL.
