...
## Descrição
O cmdlet **Remove-AzDataLakeStoreItemAclEntry** remove uma entrada (ACE) da lista de controles de acesso (ACL) de um arquivo ou pasta na Data Lake Store.
...
Esse comando remove o ACE do usuário para a Elaca Merda da conta contosoADL.
...
### Exemplo 3: remover permissões para um objeto ACE recursivamente usando o objeto Acl
```
...
Esse comando remove o ACE do usuário para o Time Descursado da raiz e recursivamente de todos os seus subdiretivos e arquivos da conta ContosoADL.
...
### -AceType
Especifica o tipo de ACE a ser removido.
...
### -Padrão
Indica que essa operação remove o ACE padrão da ACL especificada.
...
### -ID
Especifica a ID de objeto do usuário, grupo ou entidade de serviço do AzureActive Directory para o qual remover um ACE.
...
### -Caminho
Especifica o caminho do Data Lake Store do item do qual remover um ACE, começando com o diretório raiz (/).
...
### -Recorrência
Indica que o ACL deve ser removido recursivamente para os arquivos e subdiretivos filho
...
### -ShowProgress
Se aprovado, o status do progresso será mostrado. Aplicável somente quando a remoção de Acl recursiva é feita.
