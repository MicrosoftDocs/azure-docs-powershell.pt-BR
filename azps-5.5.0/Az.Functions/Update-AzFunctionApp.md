...
### Exemplo 1: Atualizar o plano de hospedagem de aplicativos de função.
```powershell
...
### Exemplo 2: Definir uma identidade gerenciada atribuída pelo SystemAssigned para um aplicativo de função.
```powershell
PS C:\> Update-AzFunctionApp -Name MyUniqueFunctionAppName -ResourceGroupName MyResourceGroupName -IdentityType SystemAssigned 
```

Esse comando define uma identidade gerenciada atribuída pelo SystemAssigned para um aplicativo de função.

### Exemplo 3: Atualizar insights do aplicativo de função.
```powershell
...
Este comando atualiza o aplicativo Application Insights.
...
Tecla de instrumentação do Insights do Aplicativo a ser adicionada.
...
Nome do projeto Insights do Aplicativo existente a ser adicionado ao aplicativo de função.
...
### -IdentityType
Especifica o tipo de identidade usado para o aplicativo de função.
O tipo "Nenhum" removerá as identidades do aplicativo de função.
...
INPUTOBJECT: <ISite> 
  ...
  - `[Config <ISiteConfig>]`: Configuração do aplicativo.
    ...
    - `[XManagedServiceIdentityId <Int32?>]`: ID de identidade de serviço gerenciado explícita
