...
### Exemplo 2: Atualizar uma extensão com a localização especificada por meio do pipeline
```powershell
...
Atualiza uma extensão específica passada por meio do pipeline.
Aqui estamos usando a extensão passada por meio do pipeline para nos ajudar a identificar em qual extensão desejamos operar e estamos especificando o que desejamos alterar por meio dos parâmetros normais `-Settings` (como)

### Exemplo 3: Atualizar uma extensão com parâmetros de extensão especificados por meio do pipeline
```powershell
PS C:\> $extToUpdate = Get-AzConnectedMachineExtension -ResourceGroupName connectedMachines -MachineName linux-eastus1_1 -Name customScript
...
Atualiza uma extensão específica passada por meio do pipeline.
Aqui estamos usando a extensão passada por meio do pipeline para fornecer as alterações que desejamos fazer na extensão.
O local da extensão não é recuperado pelo pipeline, mas sim pelos parâmetros especificados normalmente (pelo parâmetro splat).

### Exemplo 4: Usar um objeto de extensão como o local e os parâmetros para atualizar
```powershell
PS C:\> $extToUpdate = Get-AzConnectedMachineExtension -ResourceGroupName connectedMachines -MachineName linux-eastus1_1 -Name customScript
PS C:\> # Update the settings on the object that will be used via the pipeline
PS C:\> $extToUpdate.Setting.commandToExecute = "ls -l"
...
Atualiza uma extensão específica passada por meio do pipeline.
Aqui estamos usando a extensão passada por meio do pipeline para nos ajudar a identificar em qual extensão desejamos operar.
...
### -ExtensionParameter
Descreve uma Atualização de Extensão de Máquina.
...
EXTENSIONPARAMETER <IMachineExtensionUpdate> : descreve uma Atualização de Extensão de Máquina.
  ...
  - `[ForceUpdateTag <String>]`: como o manipulador de extensão deve ser obrigado a atualizar mesmo que a configuração de extensão não tenha sido alterada.
  - `[ProtectedSetting <IMachineExtensionUpdatePropertiesProtectedSettings>]`: A extensão pode conterSettings protegidos ou protectedSettingsFromKeyVault ou nenhuma configuração protegida.
  - `[Publisher <String>]`: o nome do editor do manipulador de extensão.
  - `[Setting <IMachineExtensionUpdatePropertiesSettings>]`: Json formatou as configurações públicas para a extensão.
