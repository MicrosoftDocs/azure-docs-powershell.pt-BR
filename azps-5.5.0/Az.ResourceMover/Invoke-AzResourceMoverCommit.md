...
A operação de confirmação é disparada na moveResources no moveState "CommitPending" ou "CommitFailed", em uma conclusão bem-sucedida, o moveResource moveState faz uma transição para Committed.
...
### Exemplo 1: Validar as dependeções antes de confirmar os recursos
```powershell
...
Valide as dependecies antes de confirmar os recursos.

### Exemplo 2: Commit the set of resources in the Move Collection using "MoveResource Name" as input
```powershell
...
### Exemplo 3: Commit the set of resources in the Move Collection using "SourceARMID" as input
```powershell
