...
### -MatchValue
Corresponder valores para corresponder.
O operador será aplicado a cada valor aqui com semântica OU.
Se algum deles corresponder à variável com o operador determinado, essa condição de match será considerada uma combinação.
...
Os valores possíveis são IsMobile, RemoteAddr, RequestMethod, QueryString, PostArg, RequestUri, RequestPath, RequestFileName, RequestfilenameExtension, RequestHeader, RequestFile, RequestScheme
...
### -Operador
Descreve o operador a ser aplicado à condição de combinação.
Os valores possíveis são Any, IPMatch, GeoMatch, Equal, Contains, LessThan, GreaterThan, LessThanOrEqual, GreaterThanOrEqual, BeginsWith, EndsWith.
...
### -Seletor
Nome do seletor no RequestHeader ou RequestHeader para ser corresponder
...
### -Transformar
Lista de quais transformaçãos são aplicadas antes da correspondência. Possíveis valores de transformação individual são Minúsculas, Maiúsculas, Cortar, UrlDecode, UrlEncode, RemoveNulls.
