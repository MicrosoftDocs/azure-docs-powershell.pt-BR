...
### Exemplo 1: Criar uma criação de PSRoutingRuleObject para porta da frente com uma regra de encaminhamento
```powershell
...
### -BackendPoolName
ID do recurso do BackendPool para o qual esta regra é encaminhada
...
### -CustomFragment
Fragmente para adicionar à URL de redirecionamento. Fragmentar é a parte da URL que vem depois de #. Não inclua o #.
...
### -CustomHost
Hospede para redirecionar. Deixe vazia para usar o host de entrada como host de destino.
...
### -CustomPath
O caminho completo para redirecionar. O caminho não pode estar vazio e deve começar com /. Deixe vazia para usar o caminho de entrada como caminho de destino.
...
### -DynamicCompression
Se você quer habilitar a compactação dinâmica para conteúdo armazenado em cache quando o cache estiver habilitado.
...
### -EnableCaching
Se você quer habilitar o cache para essa rota. O valor padrão é falso
...
### -EnabledState
Se você quer habilitar o uso dessa regra.
O valor padrão está habilitado
...
### -ForwardingProtocol
O protocolo que essa regra usará ao encaminhar o tráfego para backends O valor padrão é MatchRequest.
...
Os nomes dos pontos de extremidade Frontend associados a esta regra
...
### -PatternToMatch
Os padrões de rota da regra não devem ter * exceto possivelmente após a final/no final do caminho. O valor padrão é /*
