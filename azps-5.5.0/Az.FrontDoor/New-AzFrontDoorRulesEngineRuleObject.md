...
Use o cmdlet "New-AzFrontDoorRulesEngineActionObject" para criar um objeto PSRulesEngineAction para passar para o parâmetro "-Ação".
...
Espere a saída ao passar em um valor de prioridade inválido.

## Parâmetros

### -Ação
Ações a executar na solicitação e resposta se todas as condições de match são atendidas.
...
### -MatchCondition
Uma lista de condições de combinação que deve atender para que as ações desta regra seja executado. Não ter condições de combinação significa que as ações sempre serão executados.
...
Se essa regra for uma combinação, o mecanismo de regras deverá continuar executando as regras restantes ou parar.
