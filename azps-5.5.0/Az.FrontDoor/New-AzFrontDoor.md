...
## Descrição
O **cmdlet New-AzFrontDoor** cria um novo saldo de carga do Azure Front Door no grupo de recursos especificado sob a assinatura atual
...
Crie uma Porta da Frente com base em parâmetros determinados.
...
Se você deve desabilitar a verificação de nome do certificado em solicitações HTTPS para todos os pools de back-end. Não há efeito em solicitações que não são HTTPS.
...
### -EnabledState
EnabledState do balanceador de carga porta da frente.
...
### -FrontendEndpoint
Pontos de extremidade frontend disponíveis para regra de roteamento.
