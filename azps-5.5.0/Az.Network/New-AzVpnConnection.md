...
## Sinopse
Cria uma conexão IPSec que conecta uma VpnGateway a uma ramificação remota do cliente representada no RM como um VpnSite.

## Sintaxe

### ByVpnGatewayNameByVpnSiteObject (Padrão)
```
...
O acima criará um grupo de recursos, WAN Virtual, Rede Virtual, Hub Virtual e um VpnSite no oeste dos EUA no grupo de recursos "testRG" no Azure. Um gateway VPN será criado posteriormente no Hub Virtual com 2 unidades de escala.

Depois que o gateway for criado, ele será conectado ao VpnSite usando o comando New-AzVpnConnection usuário.
...
O acima criará um grupo de recursos, WAN Virtual, Rede Virtual, Hub Virtual e um VpnSite com 1 VpnSiteLinks no oeste dos EUA no grupo de recursos "testRG" no Azure.
Um gateway VPN será criado posteriormente no Hub Virtual.
Depois que o gateway for criado, ele será conectado ao VpnSite usando o comando New-AzVpnConnection com 1 VpnSiteLinkConnections para o VpnSiteLink do VpnSite.
...
A largura de banda que precisa ser tratada por essa conexão em mbps.
...
### -EnableBgp
Habilitar BGP para esta conexão
...
### -ParentObject
O VpnGateway pai para esta conexão.
...
### -ParentResourceId
A ID do recurso do VpnGateway pai para esta conexão.
...
### -SharedKey
A chave compartilhada necessária para configurar essa conexão.
...
Use o endereço ip local do azure como endereço de origem ao iniciar a conexão.
...
Use seletores de tráfego baseados em política para esta conexão.
...
Protocolo de conexão do gateway:IKEv1/IKEv2
...
### -VpnSite
O site vpn remoto ao qual esta conexão de rede virtual do hub está conectada.
...
A lista de VpnSiteLinkConnections que esta VpnConnection tem.
