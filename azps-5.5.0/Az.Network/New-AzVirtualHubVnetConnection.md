...
## Sinopse
O New-AzVirtualHubVnetConnection cmdlet cria um recurso HubVirtualNetworkConnection que compara uma Rede Virtual ao Hub Virtual do Azure.

## Sintaxe

### ByVirtualHubNameByRemoteVirtualNetworkObject (Padrão)
```
...
O acima criará um grupo de recursos, WAN Virtual, Rede Virtual, Hub Virtual na Central dos EUA nesse grupo de recursos no Azure. Uma Conexão de Rede Virtual será criada a partir daí, que irá fazer a conexão da Rede Virtual com o Hub Virtual.

### Exemplo 2

O New-AzVirtualHubVnetConnection cmdlet cria um recurso HubVirtualNetworkConnection que compara uma Rede Virtual ao Hub Virtual do Azure. (gerado automaticamente)
...
```
O exemplo acima criará uma nova configuração de roteamento e criará rotas estáticas na configuração de roteamento com o próximo salto como um endereço IP especificado. Essa configuração de roteamento pode então ser passada para o comando New-AzVirtualHubVnetConnection como o parâmetro -RoutingConfiguration.
...
A rede virtual remota à qual esta conexão de rede virtual do hub está conectada.
