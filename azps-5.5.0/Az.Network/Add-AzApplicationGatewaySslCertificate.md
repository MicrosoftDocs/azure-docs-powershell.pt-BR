...
Obter o segredo e fazer referência a ele `Add-AzApplicationGatewaySslCertificate` para adicioná-lo ao Gateway de Aplicativos com o `Cert01` nome.
Observação: como a ID sem versão é fornecida aqui, o Gateway de Aplicativo sincroniza o certificado em intervalos regulares com o KeyVault.
...
Obter o segredo e fazer referência a ele `Add-AzApplicationGatewaySslCertificate` para adicioná-lo ao Gateway de Aplicativos com o `Cert01` nome.
Observação: se for necessário que o Gateway de Aplicativo sincronize o certificado com o KeyVault, forneça a ID sem versão.
...
### -KeyVaultSecsecsecid
SecretId (uri) do KeyVault Secret. Use essa opção quando uma versão específica do segredo precisar ser usada.
