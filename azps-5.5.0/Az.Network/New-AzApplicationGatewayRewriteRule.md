...
## Sinopse
Cria uma regra de reescrita para um gateway de aplicativo.
...
## Descrição
O cmdlet **New-AzApplicationGatewayRewriteRule** cria uma regra de reescrita para um gateway de aplicativo do Azure.

## Exemplos

### Exemplo 1: Criar uma regra de reescrita para um gateway de aplicativo
```powershell
PS C:\> $rule = New-AzApplicationGatewayRewriteRule -Name rule1 -ActionSet $action -RuleSequence 101 -Condition $condition
```

Esse comando cria uma regra de reescrita chamada regra1 e armazena o resultado na variável chamada $rule.

## Parâmetros

### -ActionSet
ActionSet da regra de reescrita
...
### -Condição
Condição para que a regra de reescrita seja executada
...
### -RuleSequence
A ordem das regras desta regra de reescrita no conjunto de regras de reescrita
