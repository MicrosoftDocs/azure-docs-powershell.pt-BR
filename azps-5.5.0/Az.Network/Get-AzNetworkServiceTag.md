...
## Sinopse
Obtém a lista de recursos de informações de marca de serviço.
...
## Descrição
O **cmdlet Get-AzNetworkServiceTag** obtém a lista de recursos de informações de marca de serviço.

Observe que as informações da região do Azure especificadas serão usadas como referência para a versão (não como um filtro com base no local). Por exemplo, mesmo que você especifique, receberá a lista de marcas de serviço com detalhes de prefixo em todas as regiões, mas limitado à nuvem à qual sua assinatura `-Location eastus2` pertence (ou seja, Pública, governo dos EUA, China ou Alemanha).
...
O comando obtém a lista de recursos de informações de marca de serviço e o armazena em `serviceTags` variável.

### Exemplo 2: Obter todos os prefixos de endereço do AzureSQL
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
O primeiro comando obtém a lista de recursos de informações de marca de serviço e o armazena em `serviceTags` variável.
O segundo comando filtra a lista para selecionar o recurso de informações do Sql.

### Exemplo 3: Obter o recurso de informações de marca de serviço do Armazenamento para o West US 2
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
O primeiro comando obtém a lista de recursos de informações de marca de serviço e o armazena em `serviceTags` variável.
Os comandos a seguir mostram várias maneiras de filtrar a lista para selecionar informações de marca de serviço para Armazenamento no Oeste dos EUA 2.

### Exemplo 4: Obter todos os recursos de informações de marca de serviço global
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
O primeiro comando obtém a lista de recursos de informações de marca de serviço e o armazena em `serviceTags` variável.
O segundo comando filtra a lista para selecionar somente aqueles sem definir região.
...
### -Local
O local que será usado como referência para a versão (não como um filtro com base na localização, você receberá a lista de marcas de serviço com detalhes de prefixo em todas as regiões, mas limitado à nuvem à onde sua assinatura pertence).
