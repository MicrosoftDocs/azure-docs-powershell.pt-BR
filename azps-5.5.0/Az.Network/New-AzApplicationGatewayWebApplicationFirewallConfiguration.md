...
## Descrição
O cmdlet **New-AzApplicationGatewayWebApplicationFirewallConfiguration** cria uma configuração waf (web application firewall) para um gateway de aplicativo do Azure.
...
O primeiro comando cria uma nova configuração de grupo de regras desabilitada para o grupo de regras chamado "REQUEST-942-APPLICATION-ATTACK-SQLI" com a regra 942130 e a regra 942140 desabilitadas.
O segundo comando cria outra configuração de grupo de regras desabilitada para um grupo de regras chamado "REQUEST-921-PROTOCOL-ATTACK". Nenhuma regra é especificamente aprovada e, portanto, todas as regras do grupo de regras serão desabilitadas.
O último comando cria uma configuração WAF com regras de firewall desabilitadas conforme configuradas no $disabledRuleGroup 1 e $disabledRuleGroup 2. A nova configuração WAF é armazenada na variável $firewallConfig dados.
