...
## Sinopse
Crie uma configuração ip de serviço de link privado.
...
## Descrição
O **cmdlet New-AzPrivateLinkServiceIpConfig** cria uma configuração ip de serviço de link particular. Essa configuração é usada para nat de origem do tráfego de entrada do ponto de extremidade particular. 
...
Este exemplo cria uma configuração ip de serviço de link particular na memória.

### Exemplo 2

Crie uma configuração ip de serviço de link privado. (gerado automaticamente)
...
### -Nome
O nome da Configuração IpConfiguration
...
### -PrivateIpAddress
O endereço ip particular da ipConfiguration se a alocação estática for especificada.
...
A versão ip da configuração ip
...
### -Principal
Indique que a configuração ip atual é primária ou não.
