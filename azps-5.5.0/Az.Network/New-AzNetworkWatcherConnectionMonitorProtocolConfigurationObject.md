...
Porta : 80 Disable Disable DisableRoute : False

### Exemplo 2

Crie uma configuração de protocolo usada para executar a avaliação de teste em TCP, HTTP ou ICMP. (gerado automaticamente)
...
Os valores com suporte não são Nenhum, ListenIfAvailable.
...
### -Caminho
O componente do caminho do URI.
...
### -PreferHTTPS
Valor que indica se HTTPS é preferencial em relação a HTTP em casos em que a opção não é explícita.
...
### -RequestHeader
Os cabeçalhos HTTP para transmitir com a solicitação.
