...
O primeiro comando obtém um gateway de rede virtual chamado Gateway01 que pertence ao grupo de recursos ResourceGroup001 e o armazena na variável chamada $Gateway O segundo comando atualiza o gateway de rede virtual armazenado em uma variável $Gateway.
...
O primeiro comando obtém um gateway de rede virtual chamado Gateway01 que pertence ao grupo de recursos ResourceGroup001 e o armazena na variável chamada $Gateway O segundo comando cria o objeto de política ipsec Vpn de acordo com parâmetros ipsec especificados.
O terceiro comando atualiza o gateway de rede virtual armazenado em dados $Gateway.
...
O primeiro comando obtém um gateway de rede virtual chamado Gateway01 que pertence ao grupo de recursos ResourceGroup001 e o armazena na variável chamada $Gateway O segundo comando atualiza o Gateway de gateway de rede virtual com as marcas @{ testtagKey="SomeTagKey"; testtagValue="SomeKeyValue" }.

### Exemplo 4: Configuração de autenticação AAD adicionar/atualizar para VpnClient de um gateway de rede virtual existente
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
O primeiro comando obtém um gateway de rede virtual chamado Gateway01 que pertence ao grupo de recursos ResourceGroup001 e o armazena na variável chamada $Gateway O segundo comando atribui o valor do gateway de gateway de rede virtual01 IpConfiguration Id em ipconfigurationId1 variável.
...
O quinto comando definiu esse novo PSIpConfigurationBgpPeeringAddress para IpConfigurationBgpPeeringAddresses e atualize o gateway.
...
### -Asn
AsN do gateway de rede virtual, usado para configurar sessões BGP dentro de túnel IPsec
...
### -GatewaySku
A SKU do gateway de rede virtual
...
### -RadiusServerList
Vários servidores De raio externo P2S.
...
Sinalizar para remover a autenticação AAD do cliente P2S do gateway de rede virtual.
...
O objeto virtual do gateway de rede para basear as modificações.
Isso pode ser recuperado usando o Get-AzVirtualNetworkGateway
...
Isso não deve se sobrepor à rede virtual ou aos intervalos locais.
...
### -VpnClientProtocol
Uma lista de protocolos de túnel do cliente P2S VPN
...
Um cliente VPN apresentando um certificado que corresponde a um desses será informado para desaparecer.
...
### -VpnClientRootCertificates
Uma lista de certificados raiz do cliente VPN para usar para autenticação do cliente VPN.
