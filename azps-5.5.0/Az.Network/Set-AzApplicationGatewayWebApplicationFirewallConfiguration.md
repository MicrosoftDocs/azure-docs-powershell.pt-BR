...
## Sinopse
Modifica a configuração WAF de um gateway de aplicativo.
...
## Descrição
O cmdlet **Set-AzApplicationGatewayWebApplicationFirewallConfiguration** modifica a configuração waf (web application firewall) de um gateway de aplicativo.

## Exemplos

### Exemplo 1: Atualizar a configuração de firewall do aplicativo web gateway de aplicativos
```
...
O segundo comando habilita a configuração de firewall para o gateway de aplicativos armazenado no $AppGw e define o modo de firewall como "Detecção", RuleSetType como "OWASP" e RuleSetVersion como "3.0".

## Parâmetros

### -ApplicationGateway
Especifica um objeto de gateway de aplicativo.
Você pode usar o cmdlet Get-AzApplicationGateway para obter um objeto do gateway de aplicativo.
...
### -FileUploadLimitInMb
Limite máximo de carregamento de arquivo em MB.
...
Máximo de solicitação de tamanho do corpo em KB.
...
### -RequestCheck
Se o corpo da solicitação está marcado ou não.
...
### -RuleSetType
O tipo de conjunto de regras de firewall do aplicativo Web. Os valores aceitáveis para este parâmetro são: 
- Owasp
