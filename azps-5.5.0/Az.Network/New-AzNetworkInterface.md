...
Este exemplo cria uma nova interface de rede usando um objeto de configuração IP. O objeto de configuração IP especifica um endereço IPv4 particular estático.
O primeiro comando recupera uma rede virtual especificada existente usada para atribuir a sub-rede no segundo comando.
O segundo comando cria uma configuração IP de interface de rede chamada IPConfig1 e armazena a configuração na variável chamada $IPconfig.
O terceiro comando cria uma interface de rede chamada NetworkInterface1 que usa a configuração IP da interface de rede armazenada na variável chamada $IPconfig.
...
Especifica um conjunto de referências de grupo de segurança de aplicativos às quais a configuração ip da interface de rede deve pertencer.
...
### -Local
Especifica a região de uma interface de rede.
...
### -Nome
Especifica o nome da interface de rede a ser criado.
...
### -PrivateIpAddress
Especifica um endereço IP IPv4 estático para atribuir a esta interface de rede.
...
### -ResourceGroupName
Especifica o nome de um grupo de recursos ao que a interface de rede pertence.
...
Esse cmdlet cria uma interface de rede para a sub-rede especificada por esse parâmetro.
