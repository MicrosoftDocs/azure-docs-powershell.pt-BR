...
Os dois primeiros comandos obterão uma rede virtual chamada myvnet e uma sub-rede chamada mysubnet, respectivamente, que foram criadas anteriormente. Eles são armazenados em $vnet e $Subnet respectivamente. O terceiro comando obtém um endereço IP público criado anteriormente chamado PIP1. O comando forth cria uma nova configuração IP chamada "IPConfig-1" como a configuração IP principal com um endereço IP público associado a ele.
O último comando cria uma interface de rede chamada mynic1 usando essa configuração IP.

### 2: Criar uma configuração IP com um endereço IP particular
```
$vnet = Get-AzVirtualNetwork -Name myvnet -ResourceGroupName myrg
$Subnet = Get-AzVirtualNetworkSubnetConfig -Name mysubnet -VirtualNetwork $vnet
...
Os dois primeiros comandos obterão uma rede virtual chamada myvnet e uma sub-rede chamada mysubnet, respectivamente, que foram criadas anteriormente. Eles são armazenados em $vnet e $Subnet respectivamente. O terceiro comando cria uma nova configuração IP chamada "IPConfig-2" com um endereço IP particular 10.0.0.5 associado a ele.
O último comando cria uma interface de rede chamada mynic1 usando essa configuração IP.
...
Especifica um conjunto de referências nat de entrada do balanceador de carga às quais esta interface de rede IPConfiguration pertence.
