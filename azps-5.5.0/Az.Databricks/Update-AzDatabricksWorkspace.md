...
### Exemplo 1: atualiza as marcas de um espaço de trabalho Databções
```powershell
...
Este comando atualiza as marcas de um espaço de trabalho Datab jerseys.

### Exemplo 2: Habilitar a criptografia em um espaço de trabalho Datab bits
```powershell
...
A habilitação da criptografia em um espaço de trabalho Datab bits requer três etapas:
1.
Atualize o espaço de trabalho `-PrepareEncryption` com (se não tiver sido criado).
1.
Encontre `StorageAccountIdentityPrincipalId` na saída da última etapa.
Conceda permissões de chave ao diretor.
1.
...
### Exemplo 3: Desabilitar a criptografia em um espaço de trabalho Datab firewalls
```powershell
...
### -EncryptionKeyName
O nome da chave do Cofre de Teclas.
...
Valores possíveis (sem maiúsculas de minúsculas): Default, Microsoft.Keyvault
...
### -EncryptionKeyVaultUri
O URI (nome DNS) do Cofre de Teclas.
