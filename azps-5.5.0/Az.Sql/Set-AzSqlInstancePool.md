...
## Sinopse
Define propriedades para um pool de Instância SQL do Azure.

## Sintaxe

### DefaultSetInstancePoolParameterSet (Padrão)
```
...
## Descrição
O cmdlet **Set-AzSqlInstancePool** modifica as propriedades de um pool de Instância SQL do Azure.

## Exemplos

### Exemplo 1: Definir um tipo de licença de pool de instâncias
```powershell
...
Esse comando define o tipo de licença e/ou marcas para um pool de instâncias chamado instancePool0.

### Exemplo 2: Definir um tipo de licença de pool de instâncias usando um objeto de pool de instâncias
```powershell
...
Esse comando define o tipo de licença e/ou marcas para um pool de instâncias usando um objeto de pool de instâncias.

### Exemplo 3: Definir um tipo de licença de pool de instâncias usando uma ID de recurso de pool de instâncias
```powershell
...
### -InputObject
O objeto de entrada do pool de instâncias.
...
### -LicenseType
Determina qual Tipo de Licença usar.
Os valores possíveis são o Preço Base (com desconto AHB) e o LicenseInc inc conforme (sem desconto AHB).
...
### -Tag
As marcas a associar à instância
