...
### Exemplo 3: Habilitar a política de auditoria de armazenamento de blob de um banco de dados SQL do Azure com filtragem usando um predicado T-SQL
```powershell
...
### Exemplo 9: Desabilitar, por meio de pipeline, a política de auditoria de análise de log de um banco de dados SQL do Azure
```powershell
...
### Exemplo 10: Desabilitar o envio de registros de auditoria de um banco de dados SQL do Azure para blob armazenamento e habilitar o envio para análise de log.
```powershell
...
### Exemplo 11: Habilitar o envio de registros de auditoria de um banco de dados SQL do Azure para blob storage, hub de eventos e análise de log.
```powershell
...
Receber  
Referências  
O formulário geral para definir uma ação a ser auditada é: [ação] ON [objeto] BY [principal] Observe que [objeto] no formato acima pode se referir a um objeto como uma tabela, exibição ou procedimento armazenado, ou um banco de dados ou esquema inteiro. Para os últimos casos, os formulários BANCO DE DADOS::[dbname] e ESQUEMA::[nomedo esquema] são usados, respectivamente.
Por exemplo:  
