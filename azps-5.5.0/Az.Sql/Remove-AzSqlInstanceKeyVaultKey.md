...
## Sinopse
Remove uma chave do Cofre de Teclas de uma instância gerenciada pelo SQL
...
## Descrição
O Remove-AzSqlInstanceKeyVaultKey cmdlet remove a tecla Key Vault da Instância Gerenciada especificada. Observe que as permissões da instância gerenciada pelo SQL para o cofre da chave não são alteradas. Para alterar as permissões, use Set-AzKeyVaultAccessPolicy. Observe que esse cmdlet não faz alterações no Cofre de Teclas. Para remover uma chave do Cofre de Teclas, use Remove-AzureKeyVaultKey.
...
Esse comando remove a tecla Key Vault com Id ' https://contoso.vault.azure.net/keys/contosokey/01234567890123456789012345678901 da instância gerenciada especificada. 

### Exemplo 2: Usando objeto de instância gerenciada
```powershell
...
### Exemplo 3: Usando a ID de recurso de instância gerenciada
```powershell
PS C:\> $managedInstance = Get-AzSqlInstance -Name 'ContosoManagedInstanceName' -ResourceGroupName 'ContosoResourceGroup'
...
### Exemplo 4: Usando a canalização
```powershell
PS C:\> $managedInstance = Get-AzSqlInstance -Name 'ContosoManagedInstanceName' -ResourceGroupName 'ContosoResourceGroup'
