...
## Sinopse
Altera as configurações de auditoria de um servidor SQL do Azure.
...
## Descrição
O cmdlet **Set-AzSqlServerAudit** altera as configurações de auditoria de um servidor SQL do Azure.
...
### Exemplo 1: Habilitar a política de auditoria de armazenamento de blob de um servidor SQL do Azure
```powershell
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -BlobStorageTargetState Enabled -StorageAccountResourceId "/subscriptions/7fe3301d-31d3-4668-af5e-211a890ba6e3/resourceGroups/resourcegroup01/providers/Microsoft.Storage/storageAccounts/mystorage"
```

### Exemplo 2: Desabilitar a política de auditoria de armazenamento de blob de um servidor SQL do Azure
```powershell
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -BlobStorageTargetState Disabled
```

### Exemplo 3: Habilitar a política de auditoria de armazenamento de blob de um servidor SQL do Azure com filtragem avançada usando um predicado T-SQL
```powershell
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -BlobStorageTargetState Enabled -StorageAccountResourceId "/subscriptions/7fe3301d-31d3-4668-af5e-211a890ba6e3/resourceGroups/resourcegroup01/providers/Microsoft.Storage/storageAccounts/mystorage" -PredicateExpression "statement <> 'select 1'"
```

### Exemplo 4: Remover a configuração avançada de filtragem da política de auditoria de um servidor SQL do Azure
```powershell
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -PredicateExpression ""
```

### Exemplo 5: Habilitar a política de auditoria do hub de eventos de um servidor SQL do Azure
```powershell
...
### Exemplo 6: Desabilitar a política de auditoria do hub de eventos de um servidor SQL do Azure
```powershell
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -EventHubTargetState Disabled
```

### Exemplo 7: Habilitar a política de auditoria de análise de log de um servidor SQL do Azure
```powershell
...
### Exemplo 8: Desabilitar a política de auditoria de análise de log de um servidor SQL do Azure
```powershell
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -LogAnalyticsTargetState Disabled
```

### Exemplo 9: Desabilitar, por meio de pipeline, a política de auditoria de análise de log de um servidor SQL do Azure
```powershell
...
### Exemplo 10: Desabilitar o envio de registros de auditoria de um servidor SQL do Azure para blob armazenamento e habilitar o envio para análise de log.
```powershell
...
### Exemplo 11: Habilitar o envio de registros de auditoria de um servidor SQL do Azure para blob armazenamento, hub de eventos e análise de log.
```powershell
...
Essa combinação acima também é o conjunto configurado por padrão. Esses grupos abrangem todas as instruções SQL e os procedimentos armazenados executados no banco de dados e não devem ser usados em combinação com outros grupos, pois isso resultará em logs de auditoria duplicados.
...
### -PassThru
Especifica se a política de auditoria será saída no final da execução do cmdlet
