...
## Descrição
O Set-AzEventHubAuthorizationRule cmdlet atualiza a regra de autorização especificada no Hub de Eventos determinado.
...
Atualiza a regra de autorização MyAuthRuleName para conceder direitos de Gerenciamento ao Hub de Eventos \` \` \` MyEventHubName, com escopo pelo \` namespace \` MyNamespaceName. \`
