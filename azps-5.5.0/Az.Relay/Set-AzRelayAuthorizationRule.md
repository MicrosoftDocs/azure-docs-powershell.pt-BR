...
## Sinopse
Atualiza a descrição da regra de autorização especificada para as entidades de Retransmissão determinadas (Namespace/WcfRelay/HybridConnection).
...
## Descrição
O cmdlet **Set-AzRelayAuthorizationRule** atualiza a descrição da regra de autorização especificada das entidades de Relay determinadas (Namespace/WcfRelay/HybridConnection).
...
Adiciona **Enviar** a partir dos direitos de acesso da regra de autorização `AuthoRule1` no `TestNameSpace-Relay1` namespace.

### Exemplo 1.2 - Namespace with Rights parameter
```
...
Adiciona **Enviar** aos direitos de acesso da regra de `AuthoRule1` autorização do WcfRelay. `TestWCFRelay1`

### Exemplo 2.2 - Parâmetro WcfRelay com Direitos
```
...
Adiciona **Enviar** aos direitos de acesso da regra de `AuthoRule1` autorização da HybridConnection. `TestHybridConnection`

### Exemplo 3.2 - Parâmetro HybridConnection with Rights
```
...
### -InputObject
Objeto Relay AuthorizationRule.
...
### -Direitos
Direitos, por exemplo, @("Ouvir";"Enviar";"Gerenciar")
