...
## Sinopse
Cria uma Conexão Híbrida no namespace de Retransmissão especificado.
...
## Descrição
O New-AzRelayHybridConnection cmdlet cria uma Conexão Híbrida no namespace de Retransmissão especificado.
...
Cria um novo HybirdConnection \` TestHybirdConnection2 no \` namespace \` testNameSpace-HybirdConnection de Retransmissão \` especificado.
...
Cria um novo \` TestHybirdConnection HybirdConnection1 no \` namespace \` testNameSpace-HybirdConnection do Relay \` especificado.
...
verdadeiro se a autorização do cliente for necessária para esta Conexão Híbrida; caso contrário, falso
