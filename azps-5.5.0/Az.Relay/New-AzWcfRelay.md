...
## Descrição
O New-AzWcfRelay cmdlet cria um WcfRelay no namespace de Relay especificado.
...
Cria um novo WcfRelay \` TestWCFRelay2 \` no namespace \` testNameSpace-Relay de Relay \` especificado.
...
Cria um novo WcfRelay TestWCFRelay no espaço de nome de \` \` Retransmissão especificado \` TestNameSpace-Relay1. \`
...
verdadeiro se a autorização do cliente for necessária para esta retransmissão; caso contrário, falso
...
### - RequerTransportSecurity
verdadeiro se a segurança do transporte for necessária para esta retransmissão; caso contrário, falso
...
### -UserMetadata
Obtém ou define usermetadata é um espaço reservado para armazenar dados de cadeia de caracteres definidos pelo usuário para o ponto de extremidade HybridConnection.por exemplo. ele pode ser usado para armazenar dados descritivos, como a lista de equipes e suas informações de contato, também podem ser armazenadas configurações definidas pelo usuário.
