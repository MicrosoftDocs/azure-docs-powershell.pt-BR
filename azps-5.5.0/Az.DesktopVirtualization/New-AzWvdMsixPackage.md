...
Este comando adiciona o Pacote MSIX no HostPool especificado
...
### -ImagePath
Caminho de imagem DE SOD/LTD no Compartilhamento de Rede.
...
### -IsActive
Faça com que esta versão do pacote seja a ativa na hostpool.
...
Especifica como registrar Pacote no feed.
...
### -LastUpdated
Pacote de Datas foi atualizado pela última vez, encontrado no appxmanifest.xml.
...
### -PackageAlias
Alias de Pacote da extração de imagem MSIX
...
Para construir, confira a seção ANOTAÇÕES para propriedades PACKAGEAPPLICATION e crie uma tabela hash.
...
Para construir, consulte a seção ANOTAÇÕES para propriedades PACKAGEDEPENDENCY e crie uma tabela hash.
...
Contém o Nome do Pacote e o nome do Publisher.
...
### -PackageName
Nome do pacote do appxmanifest.xml.
...
PACKAGEAPPLICATION <IMsixPackageApplications[]>: Lista de aplicativos de pacote. 
  - `[AppId <String>]`: ID do Aplicativo de Pacote, encontrada no appxmanifest.xml.
  - `[AppUserModelId <String>]`: Usado para ativar o Aplicativo de Pacote. Consiste em Nome do Pacote e ID do Aplicativo. Encontrado no appxmanifest.xml.
  - `[Description <String>]`: Descrição do Aplicativo de Pacote.
  - `[FriendlyName <String>]`: Nome amigável.
  - `[IconImageName <String>]`: Nome amigável.
  - `[RawIcon <Byte[]>]`: o ícone de uma cadeia de 64 bits como uma matriz de byte.
  - `[RawPng <Byte[]>]`: o ícone de uma cadeia de 64 bits como uma matriz de byte.

PACKAGEDEPENDENCY <IMsixPackageDependencies[]>: Lista de dependências de pacotes. 
