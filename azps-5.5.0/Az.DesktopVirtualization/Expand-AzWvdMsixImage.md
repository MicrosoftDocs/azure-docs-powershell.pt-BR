...
### Exemplo 1: expande o Caminho da Imagem especificado e recupera metadados de pacote encontrados no AppxManifest.xml
```powershell
...
Esse comando retorna Metadados do Pacote MSIX encontrados no Caminho da Imagem determinado.
...
### -MsixImageUri
Representa URI que faz referência à imagem MSIX Para construir, confira a seção ANOTAÇÕES para as propriedades MSIXIMAGEURI e crie uma tabela hash.
...
MSIXIMAGEURI <IMsixImageUri> : representa URI que faz referência à imagem MSIX
  - `[Uri <String>]`: URI para Imagem
