...
## Descrição
O **cmdlet Restore-AzDeletedWebApp** restaura um aplicativo Web excluído. O aplicativo Web especificado por TargetResourceGroupName, TargetName e TargetSlot será substituído com o conteúdo e as configurações do aplicativo Web excluído. Se os parâmetros de destino não são especificados, eles serão automaticamente preenchidos com o grupo de recursos, o nome e o slot do aplicativo Web excluído. Se o aplicativo Web de destino não existir, ele será criado automaticamente no plano de serviço de aplicativo especificado pelo TargetAppServicePlanName. O parâmetro de opção RestoreContentOnly pode ser usado para restaurar apenas os arquivos do aplicativo excluído sem as configurações do aplicativo.
...
Restaura o slot de Preparação de um aplicativo excluído chamado ContosoApp pertencente ao grupo de recursos Default-Web-WestUS. O aplicativo Web chamado ContosoRestore pertencente ao grupo de recursos Default-Web-EastUS será substituído. As configurações do aplicativo Web excluídas não serão restauradas.
...
Caso haja dois aplicativos excluídos com o mesmo nome (ContosoApp), obteremos detalhes dos sites e restauraremos o aplicativo chamado ContosoRestore com o aplicativo de nossa escolha ligando para restaurar com a ID.
...
Caso haja dois aplicativos excluídos com o mesmo nome(ContosoApp), obteremos detalhes dos sites e restauraremos o aplicativo chamado ContosoRestore com o aplicativo de nossa escolha, ligando para restaurar com detalhes de InputObject(Deletedsite) 
...
### -DeletedId
A ID do Azure Web App excluído.
...
### -Local
O local do Azure Web App excluído.
...
### -Nome
O nome do Azure Web App excluído.
...
### -ResourceGroupName
O grupo de recursos do Azure Web App excluído.
...
### -Slot
O slot excluído do Azure Web App.
...
### -UseDisasterRecovery
Use para recuperar um aplicativo excluído de uma unidade de escala offline.
