...
## Descrição
O cmdlet **Add-AzVmssNetworkInterfaceConfiguration** adiciona uma configuração de interface de rede ao VMSS (Virtual Machine Scale Set).
...
Lista de endereços IP do servidor DNS para configurações de dns.
...
Especifica se a interface de rede está habilitada para rede acelerada.
...
### -EnableIPForwarding
Especifica se o encaminhamento de IP está habilitado neste NIC.
...
### -Principal
Indica se as interfaces de rede criadas a partir da configuração da interface de rede serão a NIC (central de informações de rede) principal da máquina virtual.
