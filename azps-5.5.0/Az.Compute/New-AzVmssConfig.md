...
## Descrição
O cmdlet **New-AzVmssConfig** cria um objeto local configurável do VMSS (Virtual Manager Scale Set). Outros cmdlets são necessários para configurar o objeto VMSS. Estes cmdlets são:
- Set-AzVmssOsProfile
...
### -BootDiagnostic
Especifica o perfil de diagnóstico de inicialização de conjunto de escala de máquina virtual.
...
Habilita reparos automáticos no conjunto de escala de máquina virtual.
...
### -EnableUltrasSD
Permite que um ou mais discos de dados gerenciados tenham um UltraSSD_LRS de armazenamento no conjunto de escala de máquina virtual.
...
### -EsporáciaPolicy
Especifica a política de desapropriação para as máquinas virtuais no conjunto de escalas.
...
### -Extensão
Especifica o objeto de informações de extensão do VMSS. Você pode usar **o cmdlet Add-AzVmssExtension** para adicionar este objeto.
...
### -HealthProbeId
Especifica a ID de um equilíbrio de carga usado para determinar a saúde de uma instância no conjunto de escala de máquina virtual.
HealthProbeId está na forma de '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Network/loadBalancers/{loadBalancerName}/constraints/{name}'.
...
### -MaxPrice
Especifica o preço máximo que você está disposto a pagar por um VM/VMSS Spot. Este preço está em dólares americanos. Esse preço será comparado com o preço spot atual para o tamanho de VM. Além disso, os preços são comparados no momento da criação/atualização do VM/VMSS Spot, e a operação só será bem-sucedida se o preço máximo for maior do que o preço Spot atual. O maxPrice também será usado para desapropriar um VM/VMSS Spot se o preço spot atual ultrapassar o máximoPrice após a criação do VM/VMSS. Os valores possíveis são: qualquer valor decimal maior que zero. Exemplo: 0,01538.  -1 indica que o VM/VMSS Spot não deve ser despejado por motivos de preço. Além disso, o preço máximo padrão será -1 se ele não for fornecido por você.
...
Especifica o objeto de perfil de rede que contém as propriedades de rede para a configuração VMSS.
...
### -OsProfile
Especifica o objeto de perfil do sistema operacional que contém as propriedades do sistema operacional para a configuração VMSS.
...
### -Prioridade
A prioridade para o virtual que está no conjunto de escalas.  Somente valores com suporte são 'Regular', 'Spot' e 'Low'.
...
Especifica a política de atualização em implantação.
...
### -SkuTier
Especifica o nível de VMSS. Os valores aceitáveis para este parâmetro são:
- Padrão
- Basic
...
### -StorageProfile
Especifica o objeto de perfil de armazenamento que contém as propriedades de disco para a configuração do VMSS.
...
### -Zona
Especifica a lista de zonas para o conjunto de escala de máquina virtual.
...
### -ZoneBalance
Se você quer forçar estritamente até mesmo a distribuição de Máquina Virtual entre zonas x, caso haja uma inotização de zona.
