...
## Descrição
Atualiza o estado de um VMSS.  Por enquanto, a única atualização permitida é adicionar um disco de dados gerenciado.

## Exemplos

### Exemplo 1: Adicionar um disco de dados gerenciado a um VM Vmss usando New-AzVMDataDisk
```powershell
...
### Exemplo 2: Adicionar um disco de dados gerenciado a um VM Vmss usando Add-AzVMDataDisk
```powershell
PS C:\> $disk = Get-AzDisk -ResourceGroupName $rgname -DiskName $diskname0
PS C:\> $VmssVM = Get-AzVmssVM -ResourceGroupName "myrg" -VMScaleSetName "myvmss" -InstanceId 0
...
Atualiza o estado de um VMSS. (gerado automaticamente)
...
### -InstanceId
Especifica a ID de instância de um VMSS VM.
...
### -ProtectFromScaleIn
Indica que o VM definido para escala de máquina virtual não deve ser considerado para exclusão durante uma operação de dimensionamento.
...
Indica que as atualizações ou ações do modelo (incluindo escala) iniciadas no VMSS não devem ser aplicadas ao VMSS VMS.
...
### -ResourceId
A ID do recurso para o VM de escala de máquina virtual definido
...
Objeto VM de conjunto de escala de máquina virtual local
