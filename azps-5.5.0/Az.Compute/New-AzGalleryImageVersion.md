...
Crie uma versão de imagem de galeria com criptografia de imagem em disco.
...
### -DataDiskImage
Imagens de disco de dados.   por exemplo, @{Source = @{Id=<source_id>}; Ltd=1; SizeInGB = 100; HostCaching = "ReadOnly" }
...
A data de fim da vida útil da versão de imagem da galeria.
...
Se estiver definido, as Máquinas Virtuais implantadas a partir da versão mais recente da Definição de Imagem não usarão esta Versão da Imagem.
...
### -SourceImageId
A ID da imagem de origem da qual a Versão da Imagem será criada.
