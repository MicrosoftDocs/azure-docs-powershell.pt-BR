...
## Descrição
O cmdlet **New-AzStorageBtrucSASToken** gera um token SAS (Assinatura de Acesso Compartilhado) para um blob de armazenamento do Azure.

## Exemplos

### Exemplo 1: Gerar um token SAS de blob com permissão de blob completa
```
PS C:\>New-AzStorageBlobSASToken -Container "ContainerName" -Blob "BlobName" -Permission rwd
```

Este exemplo gera um token SAS de blob com permissão total de blob.

### Exemplo 2: Gerar um token SAS de blob com tempo de vida útil
```
...
### Exemplo 3: Gerar um token SAS de identidade de usuário com contexto de armazenamento com base na autenticação OAuth
```
...
Este exemplo gera um token SAS de blob identidade de usuário com contexto de armazenamento com base na autenticação OAuth
...
### -CloudBlab
Especifica o objeto **CloudB ltd.**
Para obter um **objeto CloudBtruc,** use o cmdlet [Get-AzStorageB ltd.](./Get-AzStorageBlob.md)
...
Quando o contexto de armazenamento é baseado na autenticação OAuth, gera um token SAS de blob identidade de usuário.
...
Quando o contexto de armazenamento se baseia na autenticação OAuth, a hora de expiração deve ser em sete dias a partir da hora atual e não deve ser anterior à hora atual.
