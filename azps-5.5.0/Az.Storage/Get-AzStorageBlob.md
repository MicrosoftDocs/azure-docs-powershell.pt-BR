...
## Descrição
O **cmdlet Get-AzStorageB ltd lista** blobs no contêiner especificado em uma conta de armazenamento do Azure.
...
Esse comando usa um nome de blob e um caractere curinga para obter um blob.
...
### Exemplo 4: Blobs de lista em vários lotes
```
...
Os quatro primeiros comandos atribuem valores a variáveis a usar no exemplo.
O quinto comando especifica uma instrução **Do-While** que usa o cmdlet **Get-AzStorageB ltd** para obter blobs.
A instrução inclui o token de continuação armazenado na variável $Token dados.
...
Esse comando obtém todos os blobs em um contêiner incluindo a versão de blob.
...
Esse comando obtém uma única verão de blobs com VersionId.
...
Esse comando obtém um único instantâneo de blobs com InstantâneoTime.

## Parâmetros

### -Blob
Especifica um nome ou padrão de nome, que pode ser usado para uma pesquisa de caracteres curinga.
...
Use este parâmetro e o parâmetro *MaxCount* para listar blobs em vários lotes.
...
### -IncludeDeleted
Incluir Blob Excluído, por padrão, o blob não incluirá blob excluído.
...
### -IncludeVersion
As versões de blob serão listadas somente se esse parâmetro estiver presente, por padrão, o blob não incluirá versões de blob.
...
Isso significa que, se o contêiner tiver apenas blobs chamados "Meu", "MyB ltd1" e "MyBlt2" e você especificar "-Prefixo Meu*", o cmdlet não retornará blobs.
No entanto, se você especificar "-Prefixo Meu", o cmdlet retornará "Meu", "MyB prefix1" e "MyBlt2".
