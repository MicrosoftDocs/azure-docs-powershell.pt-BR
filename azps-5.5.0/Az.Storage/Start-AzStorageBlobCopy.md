...
## Descrição
O cmdlet **Start-AzStorageBcopy começa** a copiar um blob.
...
### Exemplo 2: Obter um contêiner para especificar blobs a copiar
```
C:\PS>Get-AzStorageContainer -Name "ContosoUploads" | Start-AzStorageBlobCopy -SrcBlob "ContosoPlanning2015" -DestContainer "ContosoArchives"
```

Esse comando obtém o contêiner chamado ContosoUploads usando o cmdlet **Get-AzStorageContainer** e passa o contêiner para o cmdlet atual usando o operador de pipeline.
...
Esse comando obtém os blobs no contêiner chamado ContosoUploads, usando o cmdlet **Get-AzStorageB ltd** e, em seguida, passa os resultados para o cmdlet atual usando o operador de pipeline.
...
O comando armazena esse objeto na variável $SrcBlob dados.
O segundo comando obtém o blob chamado ContosoPlanning2015Archived no contêiner chamado ContosoArchives.
O comando armazena esse objeto na variável $DestBlob dados.
O último comando inicia a operação de cópia do contêiner de origem para o contêiner de destino.
O comando usa notação de ponto padrão para especificar os objetos **ICloudBcloud** para $SrcBlob e $DestBlob blobs.
...
Esse comando cria um contexto para a conta chamada ContosoGeneral que usa a chave especificada e armazena essa chave na variável $Context dados.
...
O comando inicia a operação de cópia para o contexto de destino armazenado $Context.
Não há contexto de armazenamento de origem, portanto, o Uri de origem deve ter acesso ao objeto de origem. Por exemplo: se a fonte for um blob do Azure público, o Uri deverá conter um token SAS que tenha acesso de leitura ao blob.

### Exemplo 6: Copiar um blob de bloco para o contêiner de destino com um novo nome de blob e definir o blob de destino StandardBtier como Hot, ReateatePriority como Alto
```
...
Esse comando inicia a operação de cópia de um blob de bloco no contêiner de destino com um novo nome de blob e desfaixa o blob de destino StandardBtier como Hot, ReateatePriority como Alto
...
Este cmdlet copia um blob do contêiner especificado por esse parâmetro.
...
### -DestCloudBcloudBcloud
Especifica um objeto **CloudB ltd de** destino
...
### -ReatarPrioridade
Bloquear Blob ReateatePriority. Indica a prioridade com a qual reatear um blob arquivado. Os valores válidos são Alto/Padrão.
...
### -SrcFile
Especifica um objeto **CloudFile** da biblioteca do Cliente de Armazenamento do Azure.
Você pode criar ou usar Get-AzStorageFile cmdlet.
...
### -SrcShare
Especifica um objeto **CloudFileShare** da biblioteca do Cliente de Armazenamento do Azure.
Você pode criar ou usar Get-AzStorageShare cmdlet.
