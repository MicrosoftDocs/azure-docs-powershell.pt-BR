...
## Sinopse
Atualiza o tempo de execução da integração auto-hospedada.
...
## Descrição
O cmdlet **Invoke-AzDataFactoryV2IntegrationRuntimeUpgrade** atualiza o tempo de execução de integração hospedada auto-hospedado se a nova versão estiver disponível.

## Exemplos

### Exemplo 1: atualiza um tempo de execução de integração auto-hospedado
```
...
O cmdlet atualiza o tempo de execução de integração auto-hospedado chamado "test-selfhost-ir" no data factory 'test-df-eu2'.
...
## Notas
Palavras-chave: azure, azurerm, arm, resource, management, manager, data, keyword, copy, activities, integration runtime
