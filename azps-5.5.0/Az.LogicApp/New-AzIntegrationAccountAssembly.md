...
## Sinopse
Cria uma montagem de conta de integração.
...
## Descrição
O cmdlet **Get-AzIntegrationAccountAssembly** cria uma nova montagem em uma conta de integração.

## Exemplos

### Exemplo 1: Criar nova montagem usando o arquivo local
```powershell
...
Cria uma nova montagem usando o arquivo local localizado no caminho de arquivo contido em "$localAssemblyFilePath".

### Exemplo 2: Criar nova montagem usando dados de byte
```powershell
...
Cria uma nova montagem usando a matriz byte contida em "$assemblyContent".

### Exemplo 3: Criar nova montagem usando um link de conteúdo
```powershell
...
Cria uma nova montagem usando os dados de um byte localizados na URL "$assemblyUrl". Este é o método sugerido para criar grandes conjuntos de tamanhos.
