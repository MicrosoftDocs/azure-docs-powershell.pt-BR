...
### FileEventTrimeterParameterSet (Padrão)
```
...
### PeriodicTimerTrimeterParameterSet
```
New-AzStackEdgeTrigger [-ResourceGroupName] <String> [-DeviceName] <String> [-Name] <String> -RoleName <String>
...
### PeriodicTimerTriourceResourceIdParameterSet
```
...
## Descrição
O **cmdlet New-AzSt stackEdgeTri stack** configura um gatilho no dispositivo Stack Edge. 
...
### -FileEvent
Passe este parâmetro de opção para configurar o Gatilho FileEvent
...
### -PeriodicTimerEvent
Passe este parâmetro de opção para configurar o Gatilho PeriodicTimerEvent
...
### -NomedaEminidade
Calcule a função em relação a quais eventos serão elevados.
...
### -Agendar
Frequência periódica na qual o evento de temporizador precisa ser elevado. Especifique um cronograma em dias (entre 1 e 365), horas (entre 1 e 23) ou minutos (entre 1 e 59).
...
### -ShareId
ID de compartilhamento de arquivos a ser passada no Gatilho FileEvent
...
### -StartTime
A hora do dia que resulta em um gatilho válido. O cronograma é calculado com referência ao tempo especificado em até segundos. Se o tempo não for especificado, o tempo será considerado como no zona de tempo do dispositivo. O valor sempre será retornado como horário UTC.
...
### -Tópico
Tópico em que os eventos periódicos são publicados no dispositivo IoT.
...
### Microsoft.Azure.PowerShell.Cmdlets.StackEdge.Models.PSSt stackEdgeTri meteor
