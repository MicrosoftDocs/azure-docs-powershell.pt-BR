...
## Descrição
O cmdlet **Remove-AzBatchComputeNode** remove nós de computação em lote do Azure de um pool.
...
Esse comando remove o nó de computação que tem a ID especificada do pool que tem o Pool de ID07.
O comando especifica a opção Encerrar transação.
O tempo de tempo de re ressarção é de 10 minutos.
...
Esse comando obtém o nó de computação que tem a ID especificada do pool que tem o Pool de ID07 usando o cmdlet Get-AzBatchComputeNode dados.
...
Esse comando remove dois nós de computação do pool que tem o Pool de ID.
...
### -DeallocationOption
Especifica uma opção de localização para a operação de remoção iniciada por esse cmdlet.
O valor padrão é a sequência.
...
### -Ids
Especifica uma matriz de IDs de nós de computação que este cmdlet remove do pool.
...
### -PoolId
Especifica a ID do pool que contém os nós de computação que este cmdlet remove.
...
### -ResizeTimeout
Especifica o intervalo de tempo para remoção dos nós de computação do pool.
