...
## Descrição
O **cmdlet New-AzBatchPool** cria um pool no serviço Lote do Azure sob a conta especificada pelo parâmetro *BatchContext.*

## Exemplos

### Exemplo 1: Criar um novo pool usando o conjunto de parâmetros TargetDedicated usando CloudServiceConfiguration
```
...
O pool está configurado para usar STANDARD_D1_V2 virtuais com a versão do sistema operacional da família quatro.

### Exemplo 2: Criar um novo pool usando o conjunto de parâmetros TargetDedicated usando VirtualMachineConfiguration
```
...
Esse comando cria um novo pool com o ID MyPool usando o conjunto de parâmetros TargetDedicated.
A alocação de destino é de três nós de computação.
O pool está configurado para usar STANDARD_D1_V2 virtuais com a imagem do sistema operacional Windows-2016-Datacenter.

### Exemplo 3: Criar um novo pool usando o conjunto de parâmetros AutoScale
```
PS C:\>$imageReference = New-Object -TypeName "Microsoft.Azure.Commands.Batch.Models.PSImageReference" -ArgumentList @("WindowsServer", "MicrosoftWindowsServer", "2016-Datacenter", "*")
PS C:\>$configuration = New-Object -TypeName "Microsoft.Azure.Commands.Batch.Models.PSVirtualMachineConfiguration" -ArgumentList @($imageReference, "batch.node.windows amd64")
...
Esse comando cria um novo pool com a ID AutoScalePool usando o conjunto de parâmetros AutoScale.
O pool está configurado para usar STANDARD_D1_V2 virtuais com a imagem do sistema operacional Windows-2016-Datacenter, e o número de destino de nós de computação é determinado pela fórmula de Escala Automática.
...
### -ApplicationLicenses
A lista de licenças de aplicativos que o serviço Lote disponibiliza em cada nó de computação no pool.
...
Especifica a quantidade de tempo, em minutos, que se acumula antes que o tamanho do pool seja automaticamente ajustado de acordo com a fórmula de Escala Automática.
O valor padrão é de 15 minutos e o valor mínimo é de 5 minutos.
...
Especifica os certificados associados ao pool.
...
Especifica configurações de configuração para um pool com base na plataforma de serviço de nuvem do Azure.
...
### -Metadados
Especifica os metadados, como pares de chave/valor, para adicionar ao novo pool.
...
### -MountConfiguration
Uma lista de sistemas de arquivos para montar em cada nó no pool. Isso dá suporte a Arquivos do Azure, NFS, CIFS/SMB e Blobfuse.
...
### -ResizeTimeout
Especifica o tempo de tempo de alocação de nós de computação para o pool.
...
### -StartTask
Especifica a especificação da tarefa inicial do pool.
A tarefa inicial é executado quando um nó de computação ins une o pool ou quando o nó de computação é reinicializado ou reimaginado.
...
Especifica o número de destino de nós de computação dedicados para alocar ao pool.
...
Especifica o número de destino de nós de computação de baixa prioridade para alocar ao pool.
...
Especifica configurações de configuração para um pool na infraestrutura de máquinas virtuais.
...
Para obter mais informações sobre tamanhos de máquina virtuais, consulte Tamanhos para máquinas virtuais ( no site do https://azure.microsoft.com/en-us/documentation/articles/virtual-machines-size-specs/ https://azure.microsoft.com/en-us/documentation/articles/virtual-machines-size-specs/) Microsoft Azure.
