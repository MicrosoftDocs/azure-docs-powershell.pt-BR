...
### Exemplo 1: Atualizar a AzCostManagementExport por escopo e nome
```powershell
...
Atualizar a AzCostManagementExport por Escopo e nome
...
Atualmente, só há suporte para "Diário".
...
### -DefinitionTimeframe
O período de tempo para a coleta de dados para a exportação.
...
Observe que "Uso" é equivalente a "RealCost" e é aplicável às exportações que ainda não fornecem dados para encargos ou depreciação para reservas de serviço.
...
### -ETag
eTag do recurso.
Para lidar com o cenário de atualização simultânea, esse campo será usado para determinar se o usuário está atualizando ou não a versão mais recente.
...
Atualmente, somente 'Csv' é suportado.
