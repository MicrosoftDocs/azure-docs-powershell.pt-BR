...
### Exemplo 1: Criar um armazenamento de configuração de aplicativos
```powershell
...
Confira o exemplo das `Update-AzAppConfigurationStore` etapas a seguir para habilitar o CMK (chave gerenciada por outro usuário).
...
Você pode habilitar a identidade gerenciada atribuída pelo sistema e dar identidades atribuídas pelo usuário ao mesmo tempo.
Confira o exemplo das `Update-AzAppConfigurationStore` etapas a seguir para habilitar o CMK (chave gerenciada por outro usuário).
...
### -SKU
O nome da SKU do armazenamento de configuração.
