...
## Sinopse
Adiciona ou atualiza uma regra de alerta baseada em métrica V2 (não clássica).
...
## Descrição
Adiciona ou atualiza uma regra de alerta baseada em **métrica V2 (não** clássica). A regra adicionada está associada a um grupo de recursos e tem um nome. Esse cmdlet implementa o padrão ShouldProcess, ou seja, ele pode solicitar confirmação do usuário antes de realmente criar, modificar ou remover o recurso.

## Exemplos

### Exemplo 1: Adicionar uma regra de alerta métrica a uma máquina virtual
...
Esse comando cria uma regra de alerta métrico para uma máquina virtual. $condition é a saída do cmdlet [New-AzMetricAlertRuleV2Criteria](https://docs.microsoft.com/en-us/powershell/module/az.monitor/new-azmetricalertrulev2criteria) e $act é a saída do cmdlet [New-AzActionGroup](https://docs.microsoft.com/en-us/powershell/module/az.monitor/new-azactiongroup)

### Exemplo 2: Adicionar uma regra de alerta métrica para todas as máquinas virtuais em uma assinatura
```powershell
...
Esse comando cria uma regra de alerta métrico para todas as máquinas virtuais na assinatura que estão em eastus

### Exemplo 3: Desabilitar uma regra de alerta métrico
```powershell
...
Para criar uma regra de alerta métrica mais complexa, como aquelas que envolvem a seleção de valores de dimensão ou que têm vários critérios, você pode usar os cmdlets auxiliares [New-AzMetricAlertRuleV2DimensionSelection](https://docs.microsoft.com/en-us/powershell/module/az.monitor/new-azmetricalertrulev2dimensionselection) [e New-AzMetricAlertRuleV2Criteria.](https://docs.microsoft.com/en-us/powershell/module/az.monitor/new-azmetricalertrulev2criteria)

O conjunto acima de cmdlets criará uma regra de alerta métrica com dimensões.

## Parâmetros

### -ActionGroup
O Grupo de Ações para regra
...
### -ActionGroupId
A ID do Grupo de Ações para regra
...
### -Descrição
A descrição da regra de alerta métrico
...
### -Gravidade
A gravidade da regra de alerta métrico
...
### -TargetResourceId
A ID do recurso de destino para regra
...
### -TargetResourceRegion
A região de recurso de destino da regra
...
### -TargetResourceScope
O escopo do recurso de destino para a regra
...
### -TargetResourceType
O tipo de recurso de destino para regra
