...
## Descrição
O **cmdlet New-AzDataCollectionRule** cria uma regra de coleta de dados.
...
Para usar o parâmetro -RuleFile, construa um arquivo json contendo três propriedades: dataSources, destinos, dataFlows (consulte Exemplo #1)

Você pode encontrar aqui os detalhes [do esquema.](https://docs.microsoft.com/en-us/rest/api/monitor/datacollectionrules/create)

A saída de um DCR serializado com o cmdlet ConvertTo-Json também tem suporte (consulte Exemplo #2).

## Exemplos

### Exemplo 1: Criar regra de coleta de dados, JSON a partir da API rest
```powershell
...
Esse comando cria regras de coleta de dados para a assinatura atual.

### Exemplo 2: Criar regra de coleta de dados, JSON a partir de PSDataCollectionRuleResource
```powershell
PS C:\>New-AzDataCollectionRule -Location 'East US 2 EUAP' -ResourceGroupName 'testdcr'
