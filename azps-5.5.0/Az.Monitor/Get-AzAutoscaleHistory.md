...
## Sinopse
Obtém o histórico de Escala Automática.
...
## Descrição
O **cmdlet Get-AzAutoscaleHistory** obtém o histórico de eventos relacionados a uma configuração de Escala Automática.

## Exemplos

### Exemplo 1: Obter todos os eventos associados a uma assinatura
```
PS C:\>Get-AzAutoscaleHistory -StartTime 2015-02-09T18:35:00 -EndTime 2015-02-09T18:40:00 -DetailedOutput
```

Esse comando obtém todos os eventos relacionados à escala automática associados à assinatura atual.

### Exemplo 2: GetAutoscaleHistory para um determinado recurso
```
...
Esse comando obtém todos os eventos relacionados à escala automática associados a um recurso específico identificado pela ID do recurso (essencialmente, o ResourceUri).
...
### -DetailedOutput
Indica que essa operação incluiu saída detalhada.
...
### -ResourceId
Especifica a ID do recurso à qual a configuração de escala automática está associada.
...
### -Status
Especifica um filtro por status.
Este parâmetro é opcional.
A falha é uma cadeia de caracteres vazia (ou seja, sem filtro)
