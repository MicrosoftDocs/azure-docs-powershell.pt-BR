...
## Sinopse
Cria uma configuração de Escala Automática.
...
## Descrição
O **cmdlet Add-AzAutoscaleSetting** cria uma configuração de Escala Automática.
...
### Exemplo 1: Criar uma configuração de Escala Automática
```
...
Os dois primeiros comandos usam [o New-AzAutoscaleRule](https://docs.microsoft.com/en-us/powershell/module/az.monitor/new-azautoscalerule) para criar duas regras de Escala automática, $Rule 1 e $Rule 2.
O terceiro e o quarto comandos usam o [New-AzAutoscaleProfile](https://docs.microsoft.com/en-us/powershell/module/az.monitor/new-azautoscaleprofile) para criar perfis de Escala Automática, $Profile 1 e $Profile 2, usando $Rule 1 e $Rule 2.
O comando final cria uma configuração de Escala Automática usando os perfis $Profile 1 e $Profile 2.

## Parâmetros

### -AutoscaleProfile
Especifica uma lista de perfis para adicionar à configuração de Escala Automática ou $Null adicionar nenhum perfil.
...
### -DisableSetting
Desabilita uma configuração de Escala automática existente.
...
### -InputObject
A especificação completa de uma AutoscaleSetting
...
### -Local
Especifica o local da configuração de Escala Automática.
...
### -Nome
Especifica o nome da configuração de Escala Automática a ser criado.
...
### -Notificação
Especifica uma lista de notificações separadas por vírgulas.
...
### -ResourceGroupName
Especifica o nome do grupo de recursos do recurso associado à configuração de Escala Automática.
...
### -TargetResourceId
Especifica a ID do recurso para escala automática.
