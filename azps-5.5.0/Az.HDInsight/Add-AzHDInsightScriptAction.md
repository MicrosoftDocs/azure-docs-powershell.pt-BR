...
## Descrição
O cmdlet **Add-AzHDInsightScriptAction** adiciona ações de script ao objeto de configuração HDInsight criado pelo cmdlet New-AzHDInsightClusterConfig.
As ações de script fornecem funcionalidades que são usadas para instalar software adicional ou para alterar a configuração de aplicativos que são executados em um cluster Hadoop usando scripts Do Windows PowerShell ou Bash (para clusters do Windows ou linux, respectivamente).
Uma ação de script é executado nos nós de cluster quando clusters HDInsight são implantados e são executados após nós na configuração HDInsight completa do cluster.
A ação de script é executado sob privilégios de conta de administrador do sistema e fornece direitos de acesso total aos nós de cluster.
Você pode fornecer a cada cluster uma lista de ações de script para executar em uma sequência especificada.
...
Esse comando adiciona uma ação de script para os nós De Cabeça e Trabalhador do cluster seu-hadoop-001, a ser executado no final da criação de cluster.
