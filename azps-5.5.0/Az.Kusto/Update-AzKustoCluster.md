...
O comando acima atualiza a sKU do cluster de Kusto "testnewkustocluster" encontrada no grupo de recursos "testrg".
...
Um valor boolano que indica se os discos do cluster são criptografados.
...
Um valor boolano que indica se a criptografia dupla está habilitada.
...
### -EnablePurge
Um valor boolano que indica se as operações de limpeza estão habilitadas.
...
Um valor boolano que indica se a ingestão de streaming está habilitada.
...
A lista de identidades de usuário associadas ao cluster de Kusto.
As referências de chave do dicionário de identidade do usuário serão IDs de recurso ARM no formulário: '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName}'.
...
A identidade atribuída pelo usuário (ID de recurso ARM) que tem acesso à chave.
...
Para construir, confira a seção ANOTAÇÕES das propriedades LANGUAGEEXTENSIONVALUE e crie uma tabela hash.
...
Um valor boolano que indica se o recurso de escala automática otimizada está habilitado ou não.
...
Para construir, confira a seção ANOTAÇÕES das propriedades TRUSTEDEXTERNALTENANT e crie uma tabela hash.
...
TRUSTEDEXTERNALTENANT <ITrustedExternalTenant[]>: os locatários externos do cluster.
  - `[Value <String>]`: GUID representando um locatário externo.
