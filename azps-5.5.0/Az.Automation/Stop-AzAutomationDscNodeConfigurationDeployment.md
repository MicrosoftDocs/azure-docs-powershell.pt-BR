...
## Sinopse
Interrompe uma implantação de configuração de nó DSC em Automação. Ele apenas interrompe o trabalho de implantação atual, mas não desaigna configurações de nó já atribuídas.
...
## Descrição
O cmdlet **Stop-AzAutomationDscNodeConfigurationDeployment** interrompe uma implantação de uma configuração de nó DSC (Configuração de Estado Desejada) no Azure Automation. Ela interrompe a atribuição de configuração de nó para grupos de nós, se ainda falta algum para ser atribuído, mas não desatribuição de nós já atribuídos. Para não registro de um trabalho agendado, use o [Unregister-AzAutomationScheduledRunbook](./Unregister-AzAutomationScheduledRunbook.md) com o JobScheduleId para desmarcar um trabalho agendado existente.
...
O comando acima interrompe o trabalho de implantação de configuração do nó DSC com a jobId passada.

## Parâmetros

### -AutomationAccountName
Especifica o nome da conta automação que contém a configuração DSC compilada por este cmdlet
