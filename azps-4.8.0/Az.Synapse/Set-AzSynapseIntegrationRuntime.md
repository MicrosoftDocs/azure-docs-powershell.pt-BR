...
### SetByIntegrationRuntimeName (padrão)
```
...
## DESCRITIVO
O cmdlet **set-AzSynapseIntegrationRuntime** atualiza um tempo de execução de integração com parâmetros específicos.
...
O cmdlet adiciona o espaço de trabalho para usar o tempo de execução de integração compartilhada. Ao usar `-SharedIntegrationRuntimeResourceId` o parâmetro, o `-Type` também deve ser incluído. Observe que o espaço de trabalho precisa ser concedido permissão para usar o tempo de execução de integração antes de executar o cmdlet.
...
### -DataFlowComputeType
Tipo de computação do cluster de fluxo de dados que executará o trabalho de fluxo de dados.
...
### -DataFlowCoreCount
Contagem central do cluster de fluxo de dados que executará o trabalho de fluxo de dados.
...
### -DataFlowTimeToLive
Vida útil (em minutos) da configuração do cluster de fluxo de dados que executará o trabalho de fluxo de dados.
...
O nome do tempo de execução de integração do Self-Hosted que é usado como um proxy.
...
O nome do serviço vinculado do armazenamento do blob do Azure que faz referência ao repositório de dados de preparação a ser usado ao mover dados entre Self-Hosted e o tempo de execução de integração do Azure-SSIS.
...
O caminho no repositório de dados de preparo a ser usado ao mover dados entre Self-Hosted e os tempos de execução de integração do Azure-SSIS, um contêiner padrão será usado se não for especificado.
...
### -ExpressCustomSetup
A configuração personalizada expressa para o tempo de execução de integração do SSIS que pode ser usada para configurar configurações e componentes de terceiros sem o script de instalação personalizada.
...
### -LicenseType
O tipo de licença que você deseja selecionar para o SSIS IV.
Há dois tipos: LicenseIncluded ou BasePrice.
Se você estiver qualificado para o preço do Azure Hybrid use beneficie-se (AHUB), selecione BasePrice.
Caso contrário, selecione LicenseIncluded.
...
### -PublicIP
Os endereços IP públicos estáticos que serão usados pelo tempo de execução de integração.
...
A ID do recurso do tempo de execução de integração de hospedagem automática compartilhada.
...
### -VNetId
A ID da VNet na qual o tempo de execução de integração entrará.
