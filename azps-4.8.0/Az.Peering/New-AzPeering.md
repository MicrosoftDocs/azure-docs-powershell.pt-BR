...
Crie uma nova correspondência direta com uma única conexão na instalação de Seattle usando o PeerAsn 65000

### Criar nova troca de tráfego do Exchange
```powershell
#Gets the ASN
PS C:> $asn = Get-AzPeerAsn -PeerName Contoso
...
Criar um novo emparelhamento do Exchange

### Converter emparelhamento herdado para emparelhamento por braço
```powershell
#Gets the ASN
PS C:> $asn = Get-AzPeerAsn -PeerName Contoso
...
### -DirectConnection
Crie uma nova conexão direta usando o New-AzExchangePeeringConnection e o pipe para esse comando.
...
### -ExchangeConnection
Crie uma nova conexão do Exchange usando o New-AzExchangePeeringConnection e o pipe para esse comando.
...
### -InputObject
Use Get-AzLegacyPeering para recuperar esse objeto.
...
### -MicrosoftNetwork
Selecione a rede da Microsoft à qual você deseja usar o computador.
...
### -PeerAsnResourceId
A ID do recurso ASN de par. Use Get-AzPeerAsn para recuperar a ID.
...
### -PeeringLocation
A localização física é diferente da região do Azure.
Use Get-AzPeeringLocation \<kind\> usar o nome da cidade como chave.
...
### -SKU
Selecione Basic_Direct_Free ou Premium_Direct_Free, a menos que explicitamente solicitado a selecionar outra opção.
...
### -Marca
As marcas a serem associadas ao serviço de emparelhamento da Microsoft.
