...
Criando um cofre de chaves e especifica regras de rede para permitir o acesso ao endereço IP especificado da rede virtual identificada por $myNetworkResId. Consulte `New-AzKeyVaultNetworkRuleSetObject` para obter mais informações.
...
### -DisableSoftDelete
Se especificado, a funcionalidade ' exclusão reversível ' é desativada para este cofre de chaves.
...
Se especificado, a proteção contra exclusão imediata estará habilitada para este cofre; requer a exclusão reversível para ser habilitada também.
...
Se especificado, habilita a autorização de ações de dados pelo controle de acesso baseado em função (RBAC) e, em seguida, as políticas de acesso especificadas nas propriedades do cofre serão ignoradas. Observe que as ações de gerenciamento sempre são autorizadas com RBAC.
...
### -Local
Especifica a região do Azure na qual criar o cofre de chaves. Use o comando [Get-AzLocation](https://docs.microsoft.com/powershell/module/Azure/Get-AzLocation) para ver suas opções.
...
### -NetworkRuleSet
Especifica o conjunto de regras de rede do cofre. Ele rege a acessibilidade do cofre de chaves de locais de rede específicos. Criado por `New-AzKeyVaultNetworkRuleSetObject` .
