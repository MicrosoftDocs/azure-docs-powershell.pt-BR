...
## DESCRITIVO
O cmdlet New-AzEventHub cria um novo hub de eventos do Azure.
Para criar o Eventhub com as propriedades de descrição de captura, siga as etapas abaixo (exemplos 2). 

## EXEMPLOS

### Exemplo 1:-criar novo EventHub
```powershell
PS C:\> New-AzEventHub -ResourceGroupName MyResourceGroupName -NamespaceName MyNamespaceName -Name MyEventHubName
```

Cria um hub de eventos chamado \` MyEventHubName \` com um período de retenção de 3 dias e duas partições, no \` local westus \` , com o grupo de recursos \` MyResourceGroupName \` .

### Exemplo 2: atualizar o Eventhub com "CaptureDescription"
```powershell
...
Cria um hub de eventos chamado \` MyEventHubName \` com um período de retenção de 3 dias, 2 partições e propriedades CaptureDescription no \` local da westus \` , com MyResourceGroupName de grupo de recursos \` \` .
...
Retenção de mensagem do Eventhub em dias
