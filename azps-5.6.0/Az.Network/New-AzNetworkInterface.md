...
Este exemplo cria uma nova interface de rede usando um objeto de configuração IP. O objeto de configuração IP especifica um endereço IPv4 privado estático.
...
O terceiro comando cria uma interface de rede chamada NetworkInterface1 que usa a configuração IP da interface de rede armazenada na variável denominada $IPconfig.
...
Especifica uma coleção de referências de grupo de segurança de aplicativos à qual a configuração IP da interface de rede deve pertencer.
...
Este cmdlet cria uma interface de rede para a sub-rede especificada por esse parâmetro.
