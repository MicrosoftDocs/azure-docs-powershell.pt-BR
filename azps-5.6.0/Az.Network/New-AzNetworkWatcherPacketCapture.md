...
## SYNOPSIS
Cria um novo recurso de captura de pacote e inicia uma sessão de captura de pacote em uma VM.
...
## DESCRIPTION
O New-AzNetworkWatcherPacketCapture cmdlet cria um novo recurso de captura de pacote e inicia uma sessão de captura de pacote em uma VM.
O comprimento das sessões de Captura de Pacotes pode ser configurado por meio de uma restrição de tempo ou uma restrição de tamanho. A quantidade de dados capturados para cada pacote também pode ser configurada.
Os filtros podem ser aplicados a uma determinada sessão de captura de pacote, permitindo que você personalize o tipo de pacotes capturados. Os filtros podem restringir pacotes em endereços IP locais e remotos & intervalos de endereços, portas locais e remotas & intervalos de portas e o protocolo de nível de sessão a ser capturado. Os filtros são composíveis e vários filtros podem ser aplicados para fornecer granularidade de captura.

## EXEMPLOS

### Exemplo 1: Criar uma Captura de Pacote com vários filtros
```
...
Neste exemplo, criamos uma captura de pacote chamada "PacketCaptureTest" com vários filtros e um limite de tempo. Depois que a sessão for concluída, ela será salva na conta de armazenamento especificada. Observação: a extensão do Azure Network Watcher deve ser instalada na máquina virtual de destino para criar capturas de pacotes.
...
### -Filter
Filtra a sessão de captura de pacotes.
