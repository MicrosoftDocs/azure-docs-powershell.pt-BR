...
## SYNOPSIS
Atualiza as configurações HTTP back-end para um gateway de aplicativo.
...
## DESCRIPTION
O Set-AzApplicationGatewayBackendHttpSetting cmdlet atualiza as configurações http (Protocolo de Transferência de Hipertexto) back-end para um gateway de aplicativo do Azure.
As configurações http back-end são aplicadas a todos os servidores back-end em um pool.
...
O segundo comando atualiza as configurações HTTP do gateway de aplicativo na variável $AppGw para usar a porta 88, o protocolo HTTP e habilita afinidade baseada em cookie.
...
### -ApplicationGateway
Especifica um objeto gateway de aplicativo com o qual esse cmdlet associa configurações HTTP back-end.
...
Os valores aceitáveis para este parâmetro são: Desabilitado ou Habilitado.
...
### -Probe
Especifica uma sonda a ser associada às configurações HTTP de back-end.
...
### -ProbeId
Especifica a ID da sonda a ser associada às configurações HTTP de back-end.
...
### -Protocol
Especifica o protocolo a ser usado para comunicação entre o gateway de aplicativos e servidores back-end.
Os valores aceitáveis para este parâmetro são: Http e Https.
Esse parâmetro é sensível a minúsculas.
