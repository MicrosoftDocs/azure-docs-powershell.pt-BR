...
## SYNOPSIS
Adiciona uma configuração de peering a uma conexão cruzada ExpressRoute.
...
## DESCRIPTION
O cmdlet **Add-AzExpressRouteCrossConnectionPeering** adiciona uma configuração de peering a uma conexão cruzada expressRoute. Observe que, depois de executar **Add-AzExpressRouteCrossConnectionPeering,** você deve chamar o cmdlet Set-AzExpressRouteCrossConnection para ativar a configuração.

## EXEMPLOS

### Exemplo 1: Adicionar um par a uma conexão cruzada ExpressRoute existente
```
...
A conexão cruzada ExpressRoute sendo modificada. Este é o objeto Azure retornado pelo cmdlet **Get-AzExpressRouteCrossConnection.**
...
### -Name
O nome da relação de paração a ser adicionada.
...
### -PeerASN
O número AS de sua conexão cruzada ExpressRoute. Deve ser uma ASN Pública quando PeeringType for AzurePublicPeering.
