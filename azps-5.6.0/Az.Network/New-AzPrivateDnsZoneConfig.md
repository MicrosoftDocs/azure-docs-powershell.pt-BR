...
## SYNOPSIS
Cria a configuração de zona DNS do grupo de zonas dns privada.
...
## DESCRIPTION
O cmdlet **New-AzPrivateDnsZoneConfig** permite que você crie um novo objeto de configuração de zona DNS que será definido no grupo de zonas DNS.

## EXEMPLOS

### Cria configuração de zona DNS
```powershell
PS C:\> $dnsZone = New-AzPrivateDnsZone -ResourceGroupName "rg" -Name "test.vault.azure.com"
PS C:\> $config = New-AzPrivateDnsZoneConfig -Name "test-vault-azure-com" -PrivateDnsZoneId $dnsZone.ResourceId
```

O exemplo acima cria zona DNS e, em seguida, cria a configuração de zona DNS. `New-AzPrivateDnsZone` cmdlet é comprovado pelo módulo Az.PrivateDns.
...
### -Name
Nome do recurso exclusivo em um grupo de recursos.
...
### -PrivateDnsZoneId
A id de recurso da zona dns privada.
