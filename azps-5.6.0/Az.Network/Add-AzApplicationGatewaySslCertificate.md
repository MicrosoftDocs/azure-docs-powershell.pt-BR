...
Esse comando obtém um gateway de aplicativo chamado ApplicationGateway01 e adiciona um certificado SSL chamado Cert01 a ele.

### Exemplo 2: Adicione um certificado SSL usando KeyVault Secret (secretId sem versão) a um gateway de aplicativo.
```
PS C:\> $AppGW = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
...
Obter o segredo e fazer referência a ele no `Add-AzApplicationGatewaySslCertificate` para adicioná-lo ao Gateway de Aplicativos com o nome `Cert01` .
Observação: como secretId sem versão é fornecido aqui, o Gateway de Aplicativo sincroniza o certificado em intervalos regulares com o KeyVault.

### Exemplo 3: Adicione um certificado SSL usando KeyVault Secret (secretId com versão) a um gateway de aplicativo.
```
PS C:\> $AppGW = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
PS C:\> $secret = Get-AzKeyVaultCertificate -VaultName "keyvault01" -Name "sslCert01"
...
Obter o segredo e fazer referência a ele no `Add-AzApplicationGatewaySslCertificate` para adicioná-lo ao Gateway de Aplicativos com o nome `Cert01` .
Observação: se for necessário que o Gateway de Aplicativo sincronize o certificado com o KeyVault, forneça o secretId sem versão.

## PARÂMETROS

### -ApplicationGateway
Especifica o nome do gateway de aplicativo ao qual este cmdlet adiciona um certificado SSL.
