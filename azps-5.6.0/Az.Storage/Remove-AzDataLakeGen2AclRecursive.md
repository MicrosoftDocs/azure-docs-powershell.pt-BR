...
## DESCRIPTION
O cmdlet **Remove-AzDataLakeGen2AclRecursive** remove a ACL recursivamente no caminho especificado. As entradas ACL no ACL original, que tem o mesmo AccessControlType, DefaultScope e EntityId com entradas ACL de entrada (mesmo com permissão diferente) serão removidas.

## EXEMPLOS

### Exemplo 1: Remover recursivamente a ACL em uma directiria raiz do sistema de arquivos
```
...
Este comando primeiro cria um objeto ACL com 2 entradas de acl e remove recursivamente a ACL em um diretório raiz de um sistema de arquivos.

### Exemplo 2: Remover recursivamente a ACL em um diretório
```
...
Este comando primeiro remove a ACL recursivamente em um diretório e falha e, em seguida, retoma com ContinuationToken após o usuário corrigir o arquivo com falha.

### Exemplo 3: Remover parte recursiva de ACL por parte
```
$token = $null
...
Este script removerá a ACL rescursivamente em parte de diretório por parte, com tamanho de parte como BatchSize * MaxBatchCount. O tamanho da parte é 50000 neste script.

### Exemplo 4: Remover recursivamente a ACL em um diretório e ContinuarOnFailure e, em seguida, retomar de falhas uma a uma
```
...
Este comando primeiro remove a ACL recursivamente para um diretório com ContinueOnFailure e alguns itens falharam e, em seguida, retoma os itens com falha um por um.
...
### -ContinueOnFailure
De definir esse parâmetro para ignorar falhas e continuar a proceeing com a operação em outras sub-entidades do diretório. Por padrão, a operação terminará rapidamente ao encontrar falhas.
...
### -MaxBatchCount
Número máximo de lotes que a operação de Controle de Acesso de alteração única pode ser executada. Se o tamanho do conjunto de dados exceder MaxBatchCount multiplique BatchSize, o token de continuação será retornado.
...
### -Path
O caminho no FileSystem especificado que para alterar a Acl recursivamente.
Pode ser um arquivo ou diretório.
No formato 'directory/file.txt' ou 'directory1/directory2/'.
Ignore definir esse parâmetro para alterar recursivamente a Acl do diretório raiz do Sistema de Arquivos.
