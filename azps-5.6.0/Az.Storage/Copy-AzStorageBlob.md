...
## SYNOPSIS
Copie um blob de forma síncrona.
...
## DESCRIPTION
O cmdlet **Copy-AzStorageBlob** copia um blob de forma síncrona, atualmente só suporta blob de bloqueio.

## EXEMPLOS

### Exemplo 1: copiar um blob nomeado para outro
```
...
Este comando copia um blob do contêiner de origem para o contêiner de destino com um novo nome de blob. 

### Exemplo 2: Copiar blob de um objeto blob
```
...
Este comando copia um blob do objeto blob de origem para o contêiner de destino com um novo nome de blob.

### Exemplo 3: Copiar blob de um Uri blob
```
...
O primeiro comando cria um Uri blob do blob de origem, com sas token de permissão "rt". O segundo comando copia do blob de origem Uri para o blob de destino.

### Exemplo 4: atualizar um escopo de criptografia de blob bloqueado
```
...
Este comando atualiza um escopo de criptografia blob de bloqueio copiando-o para si mesmo com um novo escopo de criptografia.

## PARÂMETROS

### -AbsoluteUri
uri de blob de origem
...
### -Context
Objeto de contexto de armazenamento de origem do Azure
...
### -DestContext
Objeto de contexto de armazenamento de destino
...
### -EncryptionScope
Escopo de criptografia a ser usado ao fazer solicitações ao blob dest.
...
### -Force
Forçar a substituir o blob ou o arquivo existente
...
### -RehydratePriority
Bloquear Blob RehydratePriority.
Indica a prioridade com a qual reidratar um blob arquivado.
Os valores válidos são High/Standard.
...
Confira detalhes em https://docs.microsoft.com/en-us/azure/storage/blobs/storage-blob-storage-tiers
