...
## SYNOPSIS
Crie ou atualize um HSM dedicado na assinatura especificada.
...
### Exemplo 1: Criar um HSM dedicado em uma rede virtual existente
```powershell
...
Este comando cria um HSM dedicado em uma rede virtual existente.

**OBSERVAÇÃO:** Atualmente, `New-AzDedicatedHsm` tem uma limitação que retorna antes que o HSM seja totalmente provisionado no Azure.
Portanto, depois de criar um novo HSM, consulte seu estado e verifique se está antes `Get-AzDedicatedHsm` `Provisioning State` de `Succeeded` usá-lo.
...
### -Location
O local do Azure com suporte onde o HSM dedicado deve ser criado.
...
Para construir, consulte a seção NOTES para propriedades NETWORKINTERFACE e crie uma tabela de hash.
...
### -StampId
Esse campo será usado quando o RP não suportar zonas de disponibilidade.
...
### -SubnetId
A ARM de recurso no formato /subscriptions/{SubscriptionId}/resourceGroups/{ResourceGroupName}/...
