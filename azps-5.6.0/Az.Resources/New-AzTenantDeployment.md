...
## DESCRIPTION
O cmdlet **New-AzTenantDeployment** adiciona uma implantação no escopo de locatário atual.
...
Para adicionar recursos em um grupo de gerenciamento, use **o New-AzManagementGroupDeployment** que cria uma implantação em um grupo de gerenciamento.
...
Este comando cria uma nova implantação no escopo de locatário atual usando um modelo personalizado e um arquivo de modelo em disco, com parâmetro de marcas definidas.
...
Este comando cria uma nova implantação no locatário atual usando um modelo personalizado e um arquivo de modelo no disco que foi convertido em uma hashtable na memória.
