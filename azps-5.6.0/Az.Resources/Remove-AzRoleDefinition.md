...
A exclusão falhará se houver atribuições de função existentes feitas à função personalizada.
...
## DESCRIPTION
O cmdlet Remove-AzRoleDefinition exclui uma função personalizada no Azure Role-Based Access Control.
Forneça o parâmetro Id de uma função personalizada existente para excluir essa função personalizada.
Por padrão, Remove-AzRoleDefinition solicita a confirmação.
Para suprimir o prompt, use o parâmetro Force.
Se houver atribuições de função existentes feitas à função personalizada a serem excluídas, a exclusão falhará.
