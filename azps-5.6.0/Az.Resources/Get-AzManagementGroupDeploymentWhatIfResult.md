...
## SYNOPSIS
Obtém um modelo What-If resultado de uma implantação no escopo do grupo de gerenciamento. 
...
## DESCRIPTION
O cmdlet **Get-AzManagementGroupDeploymentWhatIfResult** obtém o resultado do What-If modelo ARM para uma implantação de modelo no escopo do grupo de gerenciamento especificado. Ele retorna uma lista de alterações que indicam quais recursos serão atualizados se a implantação for aplicada sem fazer alterações nos recursos reais. Para especificar o formato do resultado de retorno, use o *parâmetro ResultFormat.*

## EXEMPLOS

### Exemplo 1: obter um What-If no escopo do grupo de gerenciamento
```powershell
...
Esse comando obtém um What-If no escopo do grupo de gerenciamento usando um arquivo de modelo personalizado e um arquivo de parâmetro no disco.
O comando usa o *parâmetro Location* para especificar onde armazenar os dados de implantação.
O comando usa o *parâmetro ManagementGroupId* para especificar o grupo de gerenciamento no qual o modelo será implantado.
...
O comando usa o *parâmetro ResultFormat* para definir o What-If para incluir cargas completas de recursos.

### Exemplo 2: obter um What-If no escopo do grupo de gerenciamento com ResourceIdOnly
```powershell
PS C:\> Get-AzManagementGroupDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -Location "West US" `
...
O comando usa o *parâmetro TemplateFile* para especificar um arquivo de modelo.
O comando usa o *parâmetro TemplateParameterFile* para especificar um arquivo de parâmetro de modelo.
O comando usa o *parâmetro ResultFormat* para definir o What-If result para conter apenas IDs de recurso.
...
### -ExcludeChangeType
Lista separada por vírgulas dos tipos de alteração de recursos a serem excluídos dos What-If resultados.
...
### -Name
O nome da implantação que ele criará.
Se não for especificado, o padrão é o nome do arquivo de modelo quando um arquivo de modelo é fornecido; padrão para a hora atual em que um objeto template é fornecido, por exemplo, "20131223140835".
