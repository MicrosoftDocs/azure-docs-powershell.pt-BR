...
## DESCRIPTION
Exporta uma versão específica da Especificação de Modelo para o sistema de arquivos local.
...
Exporta a versão 'v1.0' da Especificação de Modelo denominada 'MyTemplateSpec' dentro do grupo de recursos 'myRG' para a pasta de saída local "v1".

### Exemplo 2: Exportar por id de recurso
```powershell
...
Exporta a versão 'v1.0' da Especificação de Modelo denominada 'MyTemplateSpec' dentro do grupo de recursos 'myRG' de subId de assinatura para a pasta de saída \{ \} local "v1".
...
### -OutputFolder
O caminho para a pasta para a qual a versão de especificação do modelo será saída.
