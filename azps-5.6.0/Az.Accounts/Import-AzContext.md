...
## DESCRIPTION
O Import-AzContext cmdlet carrega informações de autenticação de um arquivo para definir o ambiente e o contexto do Azure.
Cmdlets executados na sessão atual usam essas informações para autenticar solicitações ao Gerenciador de Recursos do Azure.
...
### Exemplo 2: Importando um contexto de um arquivo JSON
```
...
Este exemplo seleciona um contexto de um arquivo JSON que é passado para o cmdlet. Esse arquivo JSON pode ser criado a partir do Save-AzContext.
...
### -Path
Especifica o caminho para informações de contexto salvas usando Save-AzContext.
