...
Para criar Eventhub com propriedades de descrição de Captura, siga as etapas abaixo (Exemplos 2). 
...
Cria um Hub de Eventos chamado MyEventHubName com um período de retenção de mensagens de 3 dias e duas partições, no local do WestUS, com o grupo de recursos \` \` \` \` \` MyResourceGroupName \` .

### Exemplo 2: Atualizar Eventhub com 'CaptureDescription'
```powershell
...
Cria um Hub de Eventos chamado MyEventHubName com um período de retenção de mensagens de 3 dias, 2 partições e propriedades CaptureDescription no local do WestUS, com o grupo de recursos \` \` \` \` \` MyResourceGroupName \` .
