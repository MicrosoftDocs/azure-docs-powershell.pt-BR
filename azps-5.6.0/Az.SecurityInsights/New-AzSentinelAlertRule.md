...
Você deve especificar um dos três parâmetros, *Fusion*, *Scheduled* ou *MicrosoftSecurityIncidentCreation*, para especificar o tipo de regra de Alerta a ser criado.  Cada Tipo tem diferentes paramaters obrigatórios.
...
Este exemplo cria um **AlertRule** do tipo *Fusion* com base no Modelo para Detecção Avançada de Ataques de *Vários* Estágios e o armazena na variável $AlertRule avançada.
...
Este exemplo cria um **AlertRule** do tipo *MicrosoftSecurityIncidentCreation* com base no modelo para Criar incidentes com base no Centro de Segurança do *Azure para alertas de IoT* e o armazena no $AlertRule varaible.
...
Este exemplo cria um **DataConnector** do tipo *Scheduled* e o armazena no $AlertRule varaible.
...
Nomes de exibição de regra de alerta excluem filtro.
...
### -DisplayNamesFilter
Filtro de Nomes de Exibição de Regra de Alerta.
...
### -QueryPeriod
Período de consulta de regra de alerta.
...
### -SuppressionDuration
Duração da Supressão de Regra de Alerta.
