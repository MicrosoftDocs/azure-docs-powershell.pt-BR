...
Se você não especificar uma definição de função ou um nome de entidade de usuário, esse cmdlet obtém todas as atribuições de função.
...
Este comando obtém uma atribuição de função no espaço de trabalho ContosoWorkspace com nome de função ContosoRole e nome principal do usuário ContosoName.
