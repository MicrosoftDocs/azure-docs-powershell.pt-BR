...
O cmdlet adiciona o espaço de trabalho para usar o tempo de execução de integração compartilhada. Ao usar `-SharedIntegrationRuntimeResourceId` o parâmetro, `-Type` o também deve ser incluído. Observe que o espaço de trabalho precisa ter permissão para usar o tempo de execução de integração antes de executar o cmdlet.
...
### -ExpressCustomSetup
A configuração personalizada expressa para o tempo de execução de integração do SSIS que poderia ser usada para configurações de configurações e componentes de terceiros sem script de configuração personalizado.
...
Se você estiver qualificado para o preço do Azure Hybrid Use Benefit (AHUB), selecione BasePrice.
Caso não seja, selecione LicenseIncluded.
...
### -VNetId
A ID da VNet à qual o tempo de execução de integração ingressará.
