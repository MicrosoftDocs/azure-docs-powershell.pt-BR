...
### Exemplo 1: Criar um novo servidor flexível MySql com argumentos
```powershell
...
Este cmdlet cria um servidor flexível MySql com valores de parâmetro padrão e provisiona o servidor dentro de uma nova rede virtual e tem uma sub-rede delegada ao servidor.
Os valores padrão de localização são West US 2, Sku é Standard_B1ms, A camada Sku é Burstable e o tamanho de armazenamento é 10GiB.
...
### Exemplo 3: Criar um novo servidor flexível MySql com rede virtual
```powershell
PS C:\> $Vnet = 'vnetname'
...
Este cmdlet cria um servidor flexível MySql com id de vnet ou nome de vnet fornecido por um usuário.
Se a rede virtual não existir, o cmdlet criará uma.

### Exemplo 4: Criar um novo servidor flexível MySql com rede virtual e nome de sub-rede
```powershell
...
Este cmdlet cria um servidor flexível MySql com nome de vnet, nome de sub-rede, prefixo de vnet e prefixo de sub-rede.
...
Este cmdlet cria um servidor flexível MySql aberto para todos os endereços IP.
...
Insira um único ou intervalo de endereços IP a serem incluídos na lista de IPs permitidos.
Os intervalos de endereços IP devem ser separados por traços e não conter espaços.
A especificação de 0.0.0.0 permite o acesso público de todos os recursos implantados no Azure para acessar seu servidor.
A especificação de nenhum endereço IP define o servidor no modo de acesso público, mas não cria uma regra de firewall.
...
### -Sku
O nome do sku, normalmente, camada + família + núcleos, por exemplo, Standard_B1ms, Standard_D2ds_v4.
...
### -Sub-rede
O Nome ou A ID de uma Sub-rede existente ou o nome de uma nova a ser criado.
Observe que a sub-rede será delegada ao Microsoft.DBforMySQL/flexibleServers.
Após a delegação, essa sub-rede não pode ser usada para qualquer outro tipo de recursos do Azure.
...
### -Vnet
O Nome ou ID de uma rede virtual existente ou o nome de uma nova a ser criado.
O nome deve ter entre 2 e 64 caracteres.
O nome deve começar com uma letra ou número, terminar com uma letra, número ou sublinhado, e pode conter apenas letras, números, sublinhados, períodos ou hífens.
