...
## SYNOPSIS
Crie uma nova Configuração de Controle de Origem do Kubernetes.
...
### Exemplo 1: Criar uma configuração para cluster kubernetes
```powershell
...
Este comando cria uma configuração para cluster kubernetes.

### Exemplo 1: Criar uma configuração para cluster kubernetes com especificar o operatornamespace do paramter
```powershell
...
Este comando cria uma configuração no novo namespace do operador para cluster kubernetes.
...
### -ClusterScoped
Se aprovado definir o escopo da Configuração como Cluster (o padrão é nameSpace).
...
### -EnableHelmOperator
Opção para habilitar o Operador de Helm para essa configuração do git.
...
Os valores substituem o gráfico do operador Helm.
...
Versão do gráfico do operador Helm.
