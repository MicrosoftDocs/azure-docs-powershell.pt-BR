...
## SYNOPSIS
Obtém detalhes da Configuração de Controle de Origem.
...
Este comando obtém todas as configurações do cluster kubernetes.

### Exemplo 2: Obter uma configuração do cluster kubernetes pelo nome
```powershell
...
Este comando obtém uma configuração do cluster kubernetes pelo nome.

### Exemplo 3: Obter uma configuração de cluster kubernetes por objeto
```powershell
...
Este comando obtém uma configuração de cluster kubernetes por objeto.
...
Este comando obtém uma configuração de cluster kubernetes por pipeline.
...
### -ClusterRp
O RP do cluster Kubernetes - Microsoft.ContainerService (para clusters AKS) ou Microsoft.Kubernetes (para clusters OnPrem K8S).
