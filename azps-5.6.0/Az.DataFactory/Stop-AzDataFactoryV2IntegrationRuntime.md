...
## DESCRIPTION
O cmdlet **Stop-AzDataFactoryV2IntegrationRuntime** interrompe um tempo de execução de integração dedicado gerenciado no estado 'Iniciado', iniciado pelo cmdlet Start-AzDataFactoryV2IntegrationRuntime. Os recursos são liberados e o estado é transferido para 'Parado'.

## EXEMPLOS

### Exemplo 1: pare um tempo de execução de integração gerenciada que está no estado 'Iniciado'.
```
...
O tempo de execução de integração gerenciada 'test-reserlved-ir' está no estado 'Iniciado'. Depois de executar Stop-AzDataFactoryV2IntegrationRuntime cmdlet, os recursos são liberados e o estado é transferido para 'Stopped'.
