...
Se você usar o cmdlet Set-AzDataFactorySliceStatus para modificar o status das fatias para um conjuntos de dados, certifique-se de que a hora de início e a hora de término de uma fatia estão no período ativo do pipeline.
Depois de criar um pipeline, você pode especificar o período em que ocorre o processamento de dados.
A especificação do período ativo de um pipeline define a duração do tempo em que as fatias de dados são processadas com base nas propriedades **de** disponibilidade que foram definidas para cada conjuntos de dados da Fábrica de Dados.
...
Esse comando configura o período ativo para as fatias de dados que o pipeline chamado DPWikisample processa.
O comando fornece pontos de início e fim para as fatias de dados como valores.
...
Este cmdlet modifica o período ativo de um pipeline que pertence ao fábrica de dados especificado por esse parâmetro.
...
O processamento de dados ocorre ou as fatias de dados são processadas nesse período.
...
### -ForceRecalculate
Indica que esse cmdlet usa recálculação de força.
...
Este cmdlet modifica o período ativo de um pipeline que pertence ao grupo especificado por esse parâmetro.
