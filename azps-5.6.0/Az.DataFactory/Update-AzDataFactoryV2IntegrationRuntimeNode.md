...
## DESCRIPTION
O cmdlet **Update-AzDataFactoryV2IntegrationRuntimeNode** atualiza as propriedades do nó de tempo de execução de integração auto-hospedado em um fábrica de dados. Atualmente, só dá suporte à atualização de 'ConcurrentJobsLimit'.
...
O cmdlet atualiza 'ConcurrentJobsLimit' como 3 para o nó 'Node_1' no tempo de execução de integração auto-hospedado 'test-selfhost-ir'.

## PARÂMETROS

### -ConcurrentJobsLimit
O número de trabalhos simultâneos permitidos para execução no nó tempo de execução de integração.
Os valores entre 1 e maxConcurrentJobs são permitidos.
