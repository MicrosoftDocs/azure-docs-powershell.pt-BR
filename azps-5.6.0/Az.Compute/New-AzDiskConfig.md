...
O primeiro comando cria um objeto de disco vazio local com tamanho de 5 GB Standard_LRS de conta de armazenamento. Ele também define o tipo de sistema operacional do Windows e habilita as configurações de criptografia. O segundo e o terceiro comandos definirão as configurações de chave de criptografia de disco e chave de criptografia para o objeto de disco. O último comando pega o objeto de disco e cria um disco com o nome 'Disk01' no grupo de recursos 'ResourceGroup01'.
...
O primeiro comando cria um objeto de disco local para Upload.
O segundo comando pega o objeto de disco e cria um disco com o nome 'Disk01' no grupo de recursos 'ResourceGroup01'.
O terceiro comando obtém a Url do SAS para o disco.
O quarto comando obtém o estado do disco.
Se o estado do disco for 'ReadyToUpload', um usuário poderá carregar um disco do armazenamento blob na Url do SAS de disco usando o AzCopy.
Durante o carregamento, o estado do disco é alterado para 'ActiveUpload'.
O último comando revoga o acesso ao disco para a Url do SAS.
...
Crie um disco a partir de uma Versão de Imagem de Galeria Compartilhada.  Id é a id da versão da imagem da galeria compartilhada. Lun só será necessário se a origem for um disco de dados.

## PARÂMETROS

### -BurstingEnabled
Habilita o estouro além do destino de desempenho provisionado do disco. O estouro está desabilitado por padrão. Não se aplica a discos Ultra.
...
### -DiskEncryptionKey
Especifica o objeto chave de criptografia de disco em um disco.
...
### -DiskIOPSReadOnly
O número total de IOPS que será permitido em todas as VMs que montam o disco compartilhado como ReadOnly. Uma operação pode transferir entre 4k e 256 mil bytes.
...
### -DiskIOPSReadWrite
O número de IOPS permitidos para este disco; somente settable para discos UltraSSD. Uma operação pode transferir entre 4k e 256 mil bytes.
...
### -DiskMBpsReadOnly
"description": "A produtividade total (MBps) que será permitida em todas as VMs que montam o disco compartilhado como ReadOnly. MBps significa milhões de bytes por segundo - MB aqui usa a notação ISO, de potências de 10.
...
### -DiskMBpsReadWrite
A largura de banda permitida para esse disco; somente settable para discos UltraSSD. MBps significa milhões de bytes por segundo - MB aqui usa a notação ISO, de potências de 10.
...
O objeto GalleryImageReference.  Obrigatório se estiver criando a partir de uma Imagem de Galeria. A id será a ARM id da versão da imagem de galés compartilhada da qual criar um disco.
Um lun é necessário se a origem da cópia for um dos discos de dados na imagem da galeria; se nulo, o disco do sistema operacional da imagem será copiado.
...
### -KeyEncryptionKey
Especifica a chave de criptografia Key em um disco.
...
### -SkuName
Especifica o nome Sku da conta de armazenamento.  Os valores disponíveis são Standard_LRS, Premium_LRS, StandardSSD_LRS e UltraSSD_LRS.  UltraSSD_LRS pode ser usado apenas com valor vazio para o parâmetro CreateOption.
