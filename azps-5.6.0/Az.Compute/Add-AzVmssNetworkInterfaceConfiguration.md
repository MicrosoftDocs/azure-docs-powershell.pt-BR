...
## DESCRIPTION
O cmdlet **Add-AzVmssNetworkInterfaceConfiguration** adiciona uma configuração de interface de rede ao Conjunto de Escala de Máquina Virtual (VMSS).
...
Este comando adiciona uma configuração de interface de rede ao VMSS.
...
Lista de endereços IP do servidor dns para configurações dns.
...
Especifica se a interface de rede está acelerada habilitada para rede.
...
### -EnableIPForwarding
Especifica se o encaminhamento de IP habilitado nesta NIC.
...
### -Primary
Indica se as interfaces de rede criadas a partir da configuração da interface de rede serão o centro de informações de rede principal (NIC) da máquina virtual.
