...
## DESCRIPTION
O New-AzRelayHybridConnection cmdlet cria um HybridConnection no namespace Relay especificado.
...
Cria um novo \` TestHybirdConnection HybirdConnection2 no namespace De retransmissão \` \` especificado TestNameSpace-HybirdConnection \` .
...
Cria um novo \` TestHybirdConnection HybirdConnection1 no namespace De retransmissão \` \` especificado TestNameSpace-HybirdConnection \` .
...
true se a autorização do cliente for necessária para esse HybridConnections; caso contrário, false
