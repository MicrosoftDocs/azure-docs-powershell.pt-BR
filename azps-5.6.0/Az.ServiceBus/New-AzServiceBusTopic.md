...
Cria um novo tópico de Barramento de `SB-Topic_exampl1` Serviço no namespace de Barramento de Serviço `SB-Example1` especificado.
...
Especifica a duração após a qual a mensagem expira, começando a partir de quando a mensagem é enviada para o Barramento de Serviço.
...
Especifica a estrutura [TimeSpan](https://msdn.microsoft.com/library/system.timespan.aspx) que define a duração do histórico de detecção duplicado. O valor padrão é 10 minutos.
...
### -EnableExpress
Indica se as Entidades Expressas estão habilitadas. Uma fila expressa mantém uma mensagem na memória temporariamente antes de escrevê-la no armazenamento persistente.
...
### -EnablePartitioning
Especifica se o tópico deve ser particionado entre vários operadores de mensagens. 
