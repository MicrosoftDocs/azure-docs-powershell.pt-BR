...
### Exemplo 1: Obter uma lista de recursos dependentes não resolvidos para uma coleção Move.
```powershell
...
Obter uma lista de recursos dependentes não resolvidos para uma Coleção Move.
...
Por exemplo, $apply=filter(count eq 2).
...
Por exemplo, você pode usar $orderby=Count desc.
