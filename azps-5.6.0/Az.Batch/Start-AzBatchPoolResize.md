...
## DESCRIPTION
O cmdlet **Start-AzBatchPoolResize** inicia uma operação de resize do Lote do Azure em um pool.

## EXEMPLOS

### Exemplo 1: Resize um pool para 12 nós
```
PS C:\>Start-AzBatchPoolResize -Id "ContosoPool06" -TargetDedicatedComputeNodes 12 -BatchContext $Context
```

Este comando inicia uma operação de resize no pool que tem a ID ContosoPool06.
...
### Exemplo 2: Resize um pool usando uma opção de localização
```
...
O comando passa esse objeto pool para o cmdlet atual usando o operador de pipeline.
...
O comando especifica uma opção de localização de acordo de Terminate.
...
Especifica uma opção de localização para a operação de ressalto que este cmdlet inicia.
...
### -ResizeTimeout
Especifica um período de tempo de tempo para a operação de ressarção.
Se o pool não atingir o tamanho de destino até esse momento, a operação de resize será interrompida.
