...
## DESCRIPTION
O cmdlet **Remove-AzBatchComputeNode** remove nós de computação do Azure Batch de um pool.
...
Este comando remove o nó de computação que tem a ID especificada do pool que tem a ID Pool07.
O comando especifica a opção Encerrar alocação.
O tempo de tempo de ressarção é de 10 minutos.
...
Este comando obtém o nó de computação que tem a ID especificada do pool que tem a ID Pool07 usando o cmdlet Get-AzBatchComputeNode.
...
Este comando remove dois nós de computação do pool que tem a ID Pool07.
O comando não solicita a confirmação.
...
### -DeallocationOption
Especifica uma opção de localização para a operação de remoção iniciada pelo cmdlet.
