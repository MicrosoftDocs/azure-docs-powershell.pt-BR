...
### Exemplo 1: atualizar o protocolo do cliente
```powershell
...
Este comando atualiza o protocolo de cliente do banco de dados para o Cache da Empresa Redis chamado MyCache.

### Exemplo 2: atualizar o protocolo do cliente e a política de despejo
```powershell
...
Este comando atualiza o protocolo de cliente e a política de despejo do banco de dados do Cache Corporativo Redis chamado MyCache.
...
### -ClientProtocol
Especifica se os clientes redis podem se conectar usando protocolos de redis criptografados por TLS ou plaintext.
...
### -ClusteringPolicy
Política de clustering - o padrão é OSSCluster.
...
### -EvictionPolicy
Política de despejo de redis - o padrão é VolatileLRU
...
Para construir, consulte a seção NOTES para propriedades MODULE e crie uma tabela de hash.
...
Especificado no momento da criação.
Padrão para uma porta disponível.
...
INPUTOBJECT <IRedisEnterpriseCacheIdentity> : Parâmetro Identity
  ...
  - `[Location <String>]`: A região em que a operação está.
  - `[OperationId <String>]`: Identificador exclusivo da operação.
...
MODULE <IModule[]>: conjunto opcional de módulos redis para habilitar neste banco de dados - os módulos só podem ser adicionados no momento da criação.
