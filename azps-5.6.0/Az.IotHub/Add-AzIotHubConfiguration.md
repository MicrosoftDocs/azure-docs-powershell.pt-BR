...
## DESCRIPTION
O conteúdo de configuração é json e varia ligeiramente com base na intenção de dispositivo ou módulo. As configurações de dispositivo estão na forma de {"deviceContent":{...}}
As configurações do módulo estão na forma de {"moduleContent":{...}}
As configurações podem ser definidas com métricas fornecidas pelo usuário para avaliação sob demanda.
As métricas do usuário são json e na forma de {"consultas":{...}} ou {"metrics":{"queries":{...}}}}

Observação: a condição de destino para módulos deve começar com "de devices.modules where". Confira https://docs.microsoft.com/azure/iot-hub/iot-hub-automatic-device-management mais informações.
...
Crie uma configuração de dispositivo com prioridade 3 que se aplique na condição quando um dispositivo é marcado no edifício 9 e o ambiente é 'test'.
