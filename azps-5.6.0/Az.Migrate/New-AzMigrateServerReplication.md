...
Para construir, consulte a seção NOTES para propriedades DISKTOINCLUDE e crie uma tabela de hash.
...
O objeto server pode ser recuperado usando o cmdlet Get-AzMigrateServer servidor.
...
### -TargetSubnetName
Especifica o nome da Sub-rede no destino Virtual Netowk para o qual o servidor precisa ser migrado.
...
### -TargetVMSize
Especifica a SKU da VM do Azure a ser criada.
...
DISKTOINCLUDE <IVMwareCbtDiskInput[]>: especifica os discos no servidor de origem a serem incluídos para replicação.
  ...
  - `LogStorageAccountId <String>`: A conta de armazenamento de log ARM Id.
  - `LogStorageAccountSasSecretName <String>`: O nome secreto do cofre de chaves da conta de armazenamento de log.
  - `[DiskEncryptionSetId <String>]`: A ID DiskEncryptionSet ARM.
  - `[DiskType <DiskAccountType?>]`: O tipo de disco.

INPUTOBJECT <IVMwareMachine> : Especifica o servidor descoberto a ser migrado. O objeto server pode ser recuperado usando o cmdlet Get-AzMigrateServer servidor.
