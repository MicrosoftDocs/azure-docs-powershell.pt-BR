...
## DESCRIPTION
O cmdlet **Import-AzAutomationDscConfiguration** importa uma configuração de Estado Desejado (DSC) do APS para a Automação do Azure.
Especifique o caminho de um script APS que contém uma única configuração de DSC.
...
Esse comando importa a configuração do DSC no arquivo chamado client.ps1 para a conta de automação chamada Contoso17. O comando especifica o parâmetro *Force.* Se houver uma configuração DSC existente, esse comando a substituirá.
...
### -Description
Especifica uma descrição da configuração que esse cmdlet importa.
...
### -Force
Indica que esse cmdlet substitui uma configuração de DSC existente na Automação.
...
### -LogVerbose
Especifica se esse cmdlet liga ou desliga o log detalhado para trabalhos de compilação dessa configuração do DSC. Especifique um valor de $True para ativar o log detalhado ou $False para a desativar.
...
### -Published
Indica que esse cmdlet importa a configuração do DSC no estado publicado.
...
### -SourcePath
Especifica o caminho do script wps_2 que contém a configuração DSC que esse cmdlet importa.
