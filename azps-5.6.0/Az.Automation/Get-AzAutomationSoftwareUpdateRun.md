...
## SYNOPSIS
Obtém uma lista de atualizações de software de automação do azure executados.
...
## DESCRIPTION
O Get-AzAutomationSoftwareUpdateRun cmdlet retorna uma lista de executações de atualização de software. Como uma configuração de atualização de software tem uma agenda associada, ela pode ser disparada várias vezes. Cada vez que um agendamento for disparado, uma atualização será criada. A atualização é um agregado do resultado de todas as executações de máquina. Você pode obter execução para configuração de atualização de software específica passando o parâmetro SoftwareUpdateConfigurationName. Para obter uma correção específica, você precisa passar o parâmetro name. Você também pode listar executados com status específico, executando o sistema operacional específico ou executados iniciados após um horário específico passando o parâmetro apropriado.

## EXEMPLOS

### Exemplo 1
Este exemplo lista todas as atualizações disparadas por uma configuração de atualização de software específica.
...
A configuração de atualização de software disparou a execução.
...
O nome da configuração de atualização de software disparou a execução.
