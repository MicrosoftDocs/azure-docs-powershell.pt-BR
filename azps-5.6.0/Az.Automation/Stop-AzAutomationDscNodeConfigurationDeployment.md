...
## SYNOPSIS
Interrompe uma implantação de configuração de Nó DSC na Automação. Ele apenas interrompe o trabalho de implantação atual, mas não desaigna configurações de nó já atribuídas.
...
## DESCRIPTION
O cmdlet **Stop-AzAutomationDscNodeConfigurationDeployment** interrompe uma implantação de uma configuração de nó DSC (Configuração de Estado Desejado) na Automação do Azure. Ele interrompe a atribuição de configuração de nó para grupos de nós, se algum estiver sendo atribuído, mas não desaigna nós já atribuídos. Para desmarcar um trabalho agendado, use [o Unregister-AzAutomationScheduledRunbook](./Unregister-AzAutomationScheduledRunbook.md) com o JobScheduleId para desmarcar um trabalho agendado existente.

## EXEMPLOS

### Exemplo 1: Implantar uma configuração de nó do Azure DSC na Automação
```
...
O comando acima interrompe o trabalho de implantação de configuração do nó DSC com o jobId passado.

## PARÂMETROS

### -AutomationAccountName
Especifica o nome da conta de automação que contém a configuração DSC compilada por esse cmdlet
...
### -JobId
Especifica a ID de trabalho de um trabalho de implantação existente.
