...
Este comando obtém as versões atuais de todos os segredos no cofre de chaves chamado Contoso.
...
Este comando obtém a versão atual do segredo chamado secret1 no cofre de chaves chamado Contoso.
...
Este comando obtém uma versão específica do segredo chamado secret1 no cofre de chaves chamado Contoso.

### Exemplo 5: Obter o valor de texto simples da versão atual de um segredo específico
```powershell
...
Este comando obtém todos os segredos que foram excluídos anteriormente, mas não limpos, no cofre de chaves chamado Contoso.

### Exemplo 7: obtém o ITSecret secreto que foi excluído, mas não limpo para esse cofre de chaves.
```powershell
...
Este comando obtém o segredo 'secret1' que foi excluído anteriormente, mas não limpo, no cofre de chaves chamado Contoso.
Este comando retornará metadados, como a data de exclusão e a data de exclusão agendada desse segredo excluído.
...
Este comando obtém as versões atuais de todos os segredos no cofre de chaves chamado Contoso que começam com "segredo".
...
### -InRemovedState
Especifica se é preciso mostrar os segredos excluídos anteriormente na saída
...
Este cmdlet constrói o FQDN (nome de domínio totalmente qualificado) de um cofre de chaves com base no nome especificado por esse parâmetro e no seu ambiente atual.
...
Este cmdlet constrói o FQDN de um segredo com base no nome do cofre de chaves, seu ambiente selecionado no momento, o nome secreto e a versão secreta.
