...
## DESCRIPTION
Use o `Remove-AzKeyVaultRoleAssignment` cmdlet para revogar o acesso a qualquer entidade em determinado escopo e função. O objeto da atribuição, ou seja, a entidade deve ser especificada. A entidade pode ser um usuário (use parâmetros SignInName ou ObjectId para identificar um usuário), grupo de segurança (use o parâmetro ObjectId para identificar um grupo) ou entidade de serviço (use parâmetros ApplicationId ou ObjectId para identificar um ServicePrincipal. A função à que a entidade é atribuída DEVE ser especificada usando o parâmetro RoleDefinitionName ou RoleDefinitionId.
...
Este exemplo revoga a função "Administrador de Política HSM Gerenciada" de " " no escopo user1@microsoft.com "/keys".
...
### -RoleDefinitionId
ID da função à que a entidade é atribuída.
...
### -RoleDefinitionName
Nome da função RBAC com a que atribuir a entidade.
