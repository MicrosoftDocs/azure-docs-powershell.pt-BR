...
Este comando cria um ambiente de insights da série de tempo Gen1.

### Exemplo 2: Criar um ambiente de insights da série de tempo Gen2
```powershell
...
Este comando cria um ambiente de insights da série de tempo Gen2.
...
A lista de propriedades de evento que serão usadas para particionar dados no ambiente.
Para construir, consulte a seção NOTES para propriedades PARTITIONKEYPROPERTY e crie uma tabela de hash.
...
### -StorageAccountKey
O valor da chave de gerenciamento que concede acesso de gravação de gravação do serviço Insights de Séries Temporárias à conta de armazenamento.
...
O comportamento que o serviço Insights da Série de Tempo deve ter quando a capacidade do ambiente tiver sido excedida
...
A lista de propriedades de evento que serão usadas para definir a ID da série de tempo do ambiente. Para construir, consulte a seção NOTES para propriedades TIMESERIESIDPROPERTY e crie uma tabela de hash.
...
Timespan ISO8601 especificando o número de dias em que os eventos do ambiente estarão disponíveis para consulta no armazenamento morno.
...
TIMESERIESIDPROPERTY <ITimeSeriesIdProperty[]>: a lista de propriedades de evento que serão usadas para definir a id da série de tempo do ambiente.
  - `[Name <String>]`: O nome da propriedade.
  - `[Type <PropertyType?>]`: O tipo da propriedade.
