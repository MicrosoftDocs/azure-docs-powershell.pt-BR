...
O cmdlet atualiza `2` a revisão da API com uma nova `echo-api` descrição, protocolo e caminho.
...
### -ApiRevision
Identificador da Revisão de API existente. Esse parâmetro é necessário.
...
Essa URL será usada pelos consumidores de API para o envio de solicitações para o serviço Web.
...
### -Protocolos
Protocolos de API Da Web (http, https).
Protocolos sobre qual API é disponibilizada.
Esse parâmetro é necessário.
O valor padrão é $null.
...
### -ServiceUrl
Uma URL do serviço Web expondo a API.
