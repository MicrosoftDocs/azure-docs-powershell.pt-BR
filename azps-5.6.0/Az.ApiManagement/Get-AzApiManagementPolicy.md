...
Este comando obtém a política de nível de locatário e a salva em um arquivo chamado tenantpolicy.xml.
...
Este comando obtém a política de escopo da API.
...
Este comando obtém a política de escopo de operação.

### Exemplo 5: Obter a política de escopo tenant no formato RawXml
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Este comando obtém a política de escopo de locatário no formato de escape Não Xml.
...
### -ApiRevision
Identificador da Revisão da API. Esse parâmetro é opcional. Se não for especificada, a política será recuperada da revisão da api ativa no momento.
...
Se você não especificar esse parâmetro, o resultado será canalizou como uma arraia.
