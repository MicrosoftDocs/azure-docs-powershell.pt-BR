...
Este comando define a política de nível de locatário de um arquivo chamado tenantpolicy.xml.
...
Este comando define a política de escopo do produto para Gerenciamento de API.
...
Este comando define a política de escopo da API para Gerenciamento de API.
...
### -ApiRevision
Identificador da Revisão da API. Esse parâmetro é opcional. Se não for especificada, a política será atualizada na revisão da api ativa no momento.
...
### -Format
Especifica o formato da política. Ao usar `application/vnd.ms-azure-apim.policy+xml` , as expressões contidas na política devem ser de escape XML. Ao `application/vnd.ms-azure-apim.policy.raw+xml` usá-la, **não** é necessário que a política escape de XML.
...
Esse parâmetro será necessário se *PolicyFilePath* não for especificado.
...
Esse parâmetro é necessário se o *parâmetro Policy* não for especificado.
