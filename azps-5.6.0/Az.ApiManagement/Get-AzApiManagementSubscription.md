...
As chaves não serão incluídas nos detalhes do resultado. Para obter chaves, use **Get-AzApiManagementSubscriptionKey**.
...
Esse comando obtém assinaturas de um usuário.
...
Este comando obtém todas as assinaturas configuradas para escopo de api global

### Exemplo 6: Obter todas as assinaturas de um produto e escopo de usuário
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
Se especificado, este cmdlet localiza todas as assinaturas pelo identificador do produto.
...
### -Scope
Identificador de escopo. O Escopo da Assinatura, seja Escopo da Api /apis/{apiId} ou Escopo do Produto /products/{productId} ou Escopo da API Global /apis ou escopo global /.
...
Se especificado, este cmdlet localiza a assinatura pelo identificador.
...
Se especificado, esse cmdlet localiza todas as assinaturas pelo identificador do usuário.
