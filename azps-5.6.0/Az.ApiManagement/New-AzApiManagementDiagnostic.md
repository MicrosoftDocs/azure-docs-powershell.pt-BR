...
## SYNOPSIS
Cria um novo diagnóstico no escopo Global ou no Escopo da Api.
...
## DESCRIPTION
O cmdlet **New-AzApiManagementDiagnostic** cria uma entidade de diagnóstico no escopo Global ou no escopo de Api específico.
...
O exemplo acima cria um diagnóstico para a API registrar o `httpbin` Header e 100 Bytes de Corpo no `azuremonitor` log.
...
Se especificado, definirá a política de escopo da API.
...
### -BackendSetting
Configuração de diagnóstico para mensagens Http de entrada/saída para o Backend. Esse parâmetro é opcional.
...
### -FrontEndSetting
Configuração de diagnóstico para mensagens Http de entrada/saída para o Gateway. Esse parâmetro é opcional.
...
### -LoggerId
Identificador do madeireiro para o que fazer o diagnóstico por push.
Esse parâmetro é necessário.
