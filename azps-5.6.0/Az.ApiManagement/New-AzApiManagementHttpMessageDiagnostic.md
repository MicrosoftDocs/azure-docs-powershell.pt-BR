...
Criar uma configuração de diagnóstico de mensagem http para log `Content-Type` `User-Agent` e cabeçalhos, juntamente com 100 bytes de `body`

## PARÂMETROS

### -BodyBytesToLog
Número de bytes do corpo da solicitação para fazer o log. Esse parâmetro é opcional.
...
### -HeadersToLog
A matriz de headers a ser log. Esse parâmetro é opcional.
