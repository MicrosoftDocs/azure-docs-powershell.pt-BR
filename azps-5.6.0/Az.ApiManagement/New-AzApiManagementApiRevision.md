...
Este comando cria uma Revisão de API `2` da `echo-api` API.
...
Este comando cria uma Revisão de API `5` da `echo-api` API.
...
### -ApiRevisionDescription
Api Revision Description. Esse parâmetro é opcional.
...
### -ServiceUrl
Uma URL do serviço Web expondo a API no serviço Backend. Essa URL será usada apenas pelo Gerenciamento de API do Azure e não será pública. Deve ter de 1 a 2000 caracteres. Esse parâmetro é necessário.
...
### -SourceApiRevision
Identificador de Revisão de Api da API de origem. Esse parâmetro é opcional.
