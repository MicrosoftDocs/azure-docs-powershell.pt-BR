...
## SYNOPSIS
Obter detalhes do Diagnóstico configurado no nível de serviço ou no Nível da Api. Os diagnósticos são usados para registrar solicitações/respostas do gateway de Gerenciamento de Api.
...
## DESCRIPTION
O **Get-AzApiManagementDiagnostic** obtém detalhes dos diagnósticos configurados no serviço de gerenciamento de Api em um determinado escopo.

## EXEMPLOS

### Exemplo 1: Obter todo o diagnóstico configurado no escopo do locatário.
```powershell
...
Esse comando obtém todos os diagnósticos configurados no escopo `echo-api` da Api

### Exemplo 3: Obter o diagnóstico de escopo da API especificado por uma ID
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Este comando obtém `applicationinsights` os diagnósticos configurados na api `echo-api` .

## PARÂMETROS

### -ApiId
Identificador da API existente.
Se especificado, retornará o diagnóstico de escopo da API.
...
### -DiagnosticId
Identificador do diagnóstico existente.
...
### -ResourceId
Identificador de Recurso arm de um Diagnóstico ou Diagnóstico de Api. Se especificado, tentará encontrar o diagnóstico pelo identificador. Esse parâmetro é necessário.
