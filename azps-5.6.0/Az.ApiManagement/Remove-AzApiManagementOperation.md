...
## DESCRIPTION
O cmdlet **Remove-AzApiManagementOperation** remove uma operação existente.
...
Este comando remove uma operação de API existente.
...
### -ApiRevision
Identificador da Revisão da API. Esse parâmetro é opcional. Se não for especificada, a operação será removida da revisão da api ativa no momento.
...
### -OperationId
Especifica o identificador da operação da API.
