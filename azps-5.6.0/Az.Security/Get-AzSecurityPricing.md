...
Obtém detalhes de preços da ID de recurso específica. Onde ResourceId é uma das IDs retornadas por `Get-AzSecurityPricing` .
...
Obtém detalhes de preços do plano chamado Azure Defender. Onde `name` está um dos nomes retornados por `Get-AzSecurityPricing` .
