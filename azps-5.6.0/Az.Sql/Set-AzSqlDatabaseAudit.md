...
## SYNOPSIS
Altera as configurações de auditoria para um banco de dados SQL Azure.
...
## DESCRIPTION
O cmdlet **Set-AzSqlDatabaseAudit** altera as configurações de auditoria de um banco de dados do Azure SQL.
Para usar o cmdlet, use os *parâmetros ResourceGroupName,* *ServerName* e *DatabaseName* para identificar o banco de dados.
Quando o armazenamento de blob é um destino para logs de auditoria, especifique o parâmetro *StorageAccountResourceId* para determinar a conta de armazenamento dos logs de auditoria e o parâmetro *StorageKeyType* para definir as chaves de armazenamento. Você também pode definir a retenção para os logs de auditoria definindo o valor do parâmetro *RetentionInDays* para definir o período dos logs de auditoria.

## EXEMPLOS

### Exemplo 1: Habilitar a política de auditoria de armazenamento de blob de um banco de dados SQL Azure
```powershell
...
### Exemplo 2: desabilitar a política de auditoria de armazenamento de blob de um banco de dados SQL Azure
```powershell
...
### Exemplo 3: Habilitar a política de auditoria de armazenamento de blob de um banco de dados do Azure SQL com filtragem usando um predicado T-SQL
```powershell
...
### Exemplo 4: Remover a filtragem da política de auditoria de um banco de dados SQL Azure
```powershell
...
### Exemplo 5: Habilitar a política de auditoria do hub de eventos de um banco de dados SQL Azure
```powershell
...
### Exemplo 6: desabilitar a política de auditoria do hub de eventos de um banco de dados SQL Azure
```powershell
...
### Exemplo 7: Habilitar a política de auditoria de análise de log de um banco de dados SQL Azure
```powershell
...
### Exemplo 8: Desabilitar a política de auditoria de análise de log de um banco de dados SQL Azure
```powershell
...
### Exemplo 9: Desabilitar, por meio de pipeline, a política de auditoria de análise de log de um banco de dados do Azure SQL banco de dados
```powershell
...
### Exemplo 10: desabilite o envio de registros de auditoria de um banco de dados do Azure SQL blob storage e habilita o envio para análise de log.
```powershell
...
### Exemplo 11: Habilitar o envio de registros de auditoria de um banco de dados do Azure SQL blob storage, event hub e log analytics.
```powershell
...
RECEIVE  
REFERÊNCIAS  
O formulário geral para definir uma ação a ser auditada é: [action] ON [object] BY [principal] Observe que [objeto] no formato acima pode se referir a um objeto como uma tabela, modo de exibição ou procedimento armazenado, ou um banco de dados ou esquema inteiro. Para os últimos casos, os formulários DATABASE:::[dbname] e SCHEMA:::[schemaname] são usados, respectivamente.
Por exemplo:  
SELECT em dbo.myTable por público  
SELECT on DATABASE::myDatabase by public  
SELECT on SCHEMA::mySchema by public  
Para obter mais informações, consulte https://docs.microsoft.com/sql/relational-databases/security/auditing/sql-server-audit-action-groups-and-actions#database-level-audit-actions .
...
Essa combinação acima também é o conjunto configurado por padrão. Esses grupos abrangem todas as SQL e procedimentos armazenados executados no banco de dados e não devem ser usados em combinação com outros grupos, pois isso resultará em logs de auditoria duplicados.
Para obter mais informações, consulte https://docs.microsoft.com/sql/relational-databases/security/auditing/sql-server-audit-action-groups-and-actions#database-level-audit-action-groups .
...
### -DatabaseObject
O objeto database para gerenciar sua política de auditoria.
...
### -PassThru
Especifica se a política de auditoria deve ser saída no final da execução do cmdlet
...
A ID do recurso de conta de armazenamento
...
### -StorageKeyType
Especifica qual das teclas de acesso de armazenamento usar.
