...
### Exemplo 1: Atualizar um cluster existente pelo nome
```powershell
...
### Exemplo 2: Atualizar um cluster existente pelo nome
```powershell
...
Um valor booleano que indica se os discos do cluster são criptografados.
...
As referências de chave do dicionário de identidade do usuário serão ARM ids de recurso no formato: '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName}'.
...
O usuário atribuído a identidade (ARM id de recurso) que tem acesso à chave.
...
Para construir, consulte a seção NOTES para propriedades LANGUAGEEXTENSIONVALUE e crie uma tabela de hash.
...
Um valor booleano que indica se o recurso de escala automática otimizada está habilitado ou não.
...
Para construir, consulte a seção NOTES para propriedades TRUSTEDEXTERNALTENANT e crie uma tabela de hash.
...
ID do recurso de endereço IP público do gerenciamento de dados.
