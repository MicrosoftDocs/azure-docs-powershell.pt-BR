...
Este comando atualiza as marcas de um espaço de trabalho Databricks.
...
Habilenciar a criptografia em um espaço de trabalho Databricks leva três etapas:
1.
Atualize o espaço de trabalho `-PrepareEncryption` com (se não foi criado assim).
1.
Encontre `StorageAccountIdentityPrincipalId` na saída da última etapa.
Conceda permissões de chave à entidade principal.
1.
Atualize o espaço de trabalho novamente para preencher informações sobre a chave de criptografia:
    - `-EncryptionKeySource`
...
### -EncryptionKeySource
A chave de criptografiaSource (provedor).
Valores possíveis (sem maiúsculas de minúsculas): Padrão, Microsoft.Keyvault
