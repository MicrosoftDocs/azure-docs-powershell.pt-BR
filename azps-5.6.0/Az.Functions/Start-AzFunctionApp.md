...
### Exemplo 1: Obter um aplicativo de função pelo nome e in-locar.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Start-AzFunctionApp
```

Esse comando obtém um aplicativo de função pelo nome e o inicia.

### Exemplo 2: Iniciar um aplicativo de função pelo nome.
```powershell
PS C:\> Start-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName
```

Este comando inicia um aplicativo de função por nome.
