...
## DESCRIPTION
O cmdlet **New-AzDataCollectionRuleAssociation** cria uma associação de regras de coleta de dados (DCRA).
...
Este comando cria uma associação de regra de coleta de dados para determinada ID de recurso de destino e regra.

### Exemplo 2: Criar associação de regra de coleta de dados a partir de um objeto DCR
```
PS C:\>$dcr = Get-AzDataCollectionRule -ResourceGroupName $rg -RuleName $dcrName
PS C:\>$vmId = '/subscriptions/{subId}/resourceGroups/{resourcegroup}/providers/Microsoft.Compute/virtualMachines/{vmName}'
