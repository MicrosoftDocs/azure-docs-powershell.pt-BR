...
Criar um novo Direct Peering com uma única conexão nas instalações de Seattle usando PeerAsn 65000

### Criar Novo Paring do Exchange
```powershell
#Gets the ASN
PS C:> $asn = Get-AzPeerAsn -PeerName Contoso
...
Criar um novo par de exchange

### Converter o paramento herddo em ARM paração
```powershell
#Gets the ASN
PS C:> $asn = Get-AzPeerAsn -PeerName Contoso
...
### -DirectConnection
Crie uma nova conexão Direta usando o New-AzExchangePeeringConnection e canal para esse comando.
...
### -ExchangeConnection
Crie uma nova conexão do Exchange usando o New-AzExchangePeeringConnection e canal para este comando.
...
### -MicrosoftNetwork
Selecione a rede da Microsoft com a que você deseja fazer par.
...
### -PeerAsnResourceId
A ID de Recurso de Asn Par. Use Get-AzPeerAsn recuperar a Id.
...
Use Get-AzPeeringLocation -Kind \<kind\> use City name como chave.
...
### -Tag
As marcas a ser associadas ao Serviço de Peering da Microsoft.
