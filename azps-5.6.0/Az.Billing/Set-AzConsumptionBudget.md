...
## DESCRIPTION
O Set-AzConsumptionBudget atualiza um orçamento em uma assinatura ou em um grupo de recursos.
...
### Exemplo 2: atualizar um orçamento com uma notificação quando o custo ou uso atingir um limite de 90% do valor no nível da assinatura
```powershell
...
### -MeterFilter
Lista separada por vírgulas de metros para filtrar.
Obrigatório se categoria for uso.
...
### -NotificationKey
Chave de uma notificação associada a um orçamento, necessária para criar uma notificação com a opção habilitada para notificação, limite de notificação, emails de contato, grupos de contatos ou funções de contato.
...
É sempre por cento e deve estar entre 0 e 1000.
...
### -ResourceFilter
Lista separada por vírgulas de instâncias de recurso a ser filtrada.
...
### -StartDate
Data de início (YYYY-MM-DD em UTC) do período de tempo de um orçamento.
Não antes do mês atual para o grain de hora mensal.
Não antes de três meses para o granulado trimestral.
Não antes de doze meses para o granulado anual.
Data de início futura não superior a três meses.
