...
## SYNOPSIS
Habilita a replicação para um item protegido por ASR criando um item protegido por replicação.
...
Use este cmdlet para habilitar a replicação para um item protegido por ASR.
...
Inicia a operação de criação de item protegido de replicação para o item protegido ASR especificado e retorna o trabalho ASR usado para rastrear a operação.
...
Inicia a operação de criação de item protegido de replicação para o item protegido ASR especificado e retorna o trabalho ASR usado para rastrear o cenário operation(vmWare para o Azure).
...
Inicia a operação de criação de item protegido de replicação para o item protegido ASR especificado e retorna o trabalho ASR usado para rastrear a operação (cenário do Azure para o Azure).
...
Inicia a operação de criação de item protegido de replicação para a VmId especificada e retorna o trabalho ASR usado para rastrear a operação (cenário do Azure para o Azure).
...
Inicia a operação de criação de item protegido de replicação para o item protegido ASR especificado, incluindo discos seletivos e retorna o trabalho ASR usado para rastrear o cenário operation(vmWare para o Azure) com discos selecionados.
...
Inicia a operação de criação de item protegido de replicação para a VmId especificada e retorna o trabalho ASR usado para rastrear a operação (cenário do Azure para o Azure). Para os detalhes da VM de failover passados no cmdlet para criptografia serão usados .
...
Inicia a operação de criação de item protegido de replicação para uma Máquina Virtual dentro do grupo de posicionamento de Proximidade e retorna o trabalho ASR usado para rastrear a operação (cenário do Azure para o Azure).

## PARÂMETROS

### -Account
A conta de executar como a ser usada para pressionar a instalação do serviço mobility, se necessário. Deve ser um da lista de contas de executar como no tecido ASR.
...
### -AzureToAzure
O parâmetro Switch especifica a criação do item replicado no azure para o cenário do azure.
...
Especifica a configuração de disco a ser usada do Vm para o Azure para o cenário de recuperação de desastres do Azure.
...
Especifica a URL secreta de criptografia de disco com a versão(criptografia de disco do Azure) a ser usada como VM de recuperação após o failover.
...
### -DiskEncryptionSetId
Especifica a ID de recurso do conjunto de criptografia de disco, a ser usado para a criptografia dos discos gerenciados.
...
Especifica a ID do cofre secreto de criptografia de disco (criptografia de disco do Azure) a ser usada como VM de recuperação após o failover.
...
### -IncludeDiskId
A lista de discos a incluir para replicação. Por padrão, todos os discos estão incluídos.
...
Especifica a entrada de configuração de disco para id de disco vMWare para proteger contra item protegido especificado.
...
### -KeyEncryptionKeyUrl
Especifica a URL da chave de criptografia de disco com a versão(criptografia de disco do Azure) a ser usada como VM de recuperação após o failover.
...
Especifica a ID do cofre de chaves de criptografia de disco (criptografia de disco do Azure) a ser usada como VM de recuperação após o failover.
...
### -ProcessServer
O Servidor de Processo a ser usado para replicar esse computador. Use a lista de servidores de processo na malha ASR correspondente ao servidor de Configuração para especificar um.
...
A sub-rede dentro da rede virtual de recuperação do Azure à qual a máquina virtual com falha deve ser anexada no caso de um failover.
...
Especifica a conta de armazenamento para diagnósticos de inicialização para a VM do azure de recuperação.
...
Especifica a ID de recurso do serviço de nuvem de recuperação para o failover dessa máquina virtual.
...
Especifique a ID do grupo de posicionamento de proximidade usada pela Vm de failover na região de recuperação de destino.
...
Especifica o ARM do grupo de recursos no qual a máquina virtual será criada no caso de um failover.
...
Especifica o nome do grupo de replicação a ser usado para criar pontos de recuperação consistentes de várias VM. Por padrão, cada item protegido de replicação é criado em um grupo de seus próprios pontos de recuperação consistentes e várias VM não são gerados. Use essa opção somente se você precisar criar pontos de recuperação consistentes de várias VM em um grupo de máquinas protegendo todas as máquinas para o mesmo grupo de replicação. 
...
### -UseManagedDisk
Especifica se a máquina virtual do Azure criada no failover deve usar discos gerenciados. Ele aceita True ou False.
...
### -VmmToVmm
O parâmetro Switch para especificar o item replicado é uma máquina virtual hyper-V que está sendo replicada entre sites do Hyper-V gerenciados VMM.
