...
## SYNOPSIS
Este comando constrói a configuração de recuperação de um item com backup, como SQL DB. O objeto de configuração armazena todos os detalhes, como o modo de recuperação, destinos de destino para os parâmetros específicos de restauração e aplicativo, como caminhos físicos de destino para SQL.
...
O terceiro cmdlet THe cria um plano de recuperação para uma restauração de local alternativo.

### Exemplo 2

Este comando constrói a configuração de recuperação de um item com backup, como SQL DB. (gerado automaticamente)
...
Especifica que o DB com backup deve ser restaurado para outro servidor selecionado.
...
### -FilePath
Especifica o filepath usado para a operação de restauração.
...
Especifica que o DB com backup deve ser substituído com as informações db presentes no ponto de recuperação.
...
### -RecoveryPoint
Objeto ponto de recuperação a ser restaurado
...
### -TargetItem
Especifica o destino no qual o DB precisa ser restaurado. Para SQL restaurações, ele precisa ser apenas do tipo de item protegido SQLInstance.
