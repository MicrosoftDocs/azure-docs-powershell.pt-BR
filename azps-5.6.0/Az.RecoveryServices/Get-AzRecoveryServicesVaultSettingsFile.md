...
### Exemplo 1: Registrar um computador do Windows Server ou DPM para Backup do Azure
```powershell
...
O último comando obtém o arquivo de credenciais do cofre $Vault 01 usando as credenciais no $CredsPath backup do Azure.
...
O comando obtém o arquivo de credenciais do cofre para $Vault 01 do tipo de cofre siteRecovery.

### Exemplo 3: Registrar um computador do Windows Server ou DPM para Backup do Azure
```powershell
PS C:\> $Credsfilename = Get-AzRecoveryServicesVaultSettingsFile -SiteIdentifier -Vault $Vault01
```
...
Você pode baixar esse arquivo no portal do cofre de Recuperação de Site do Azure e armazená-lo localmente.
...
Use este parâmetro se você estiver baixando as credenciais do cofre para um site do Hyper-V.
...
### -SiteIdentifier
Especifica o identificador de site.
Use este parâmetro se você estiver baixando as credenciais do cofre para um site do Hyper-V.
