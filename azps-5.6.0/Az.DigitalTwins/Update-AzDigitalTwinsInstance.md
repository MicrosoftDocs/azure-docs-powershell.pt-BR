...
## SYNOPSIS
Atualizar metadados de DigitalTwinsInstance.
...
### Exemplo 1: UpdateExpanded (Padrão)
```powershell
...
Atualizar o DigitalTwinsInstance especificado por ResourceGroupName

### Exemplo 2: atualizar o AzDigitalTwinsInstance por outro AzDigitalTwinsInstance
```powershell
...
Atualizar o AzDigitalTwinsInstance por outro AzDigitalTwinsInstance
...
Para construir, consulte a seção NOTES para propriedades DIGITALTWINSPATCHDESCRIPTION e crie uma tabela de hash.
...
### -ResourceGroupName
O nome do grupo de recursos que contém DigitalTwinsInstance.
...
DIGITALTWINSPATCHDESCRIPTION <IDigitalTwinsPatchDescription> : A descrição do serviço DigitalTwins.
  - `[Tag <IDigitalTwinsPatchDescriptionTags>]`: Marcas de instância
    - `[(Any) <String>]`: Isso indica que qualquer propriedade pode ser adicionada a esse objeto.

INPUTOBJECT <IDigitalTwinsIdentity> : Parâmetro Identity
  - `[EndpointName <String>]`: Nome do Recurso do Ponto de Extremidade.
  - `[Id <String>]`: Caminho da identidade do recurso
  - `[Location <String>]`: Local de DigitalTwinsInstance.
  - `[ResourceGroupName <String>]`: O nome do grupo de recursos que contém DigitalTwinsInstance.
