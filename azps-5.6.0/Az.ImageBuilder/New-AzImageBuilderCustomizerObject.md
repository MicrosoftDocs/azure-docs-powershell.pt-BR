...
Este comando cria um personalizador de atualização do Windows.
...
Este comando cria um personalizador de arquivos.
...
Este comando cria um personalizador do PowerShell.
...
Este comando cria um personalizador de reinicialização.
...
Este comando cria um personalizador de shell.
...
### -Destination
O caminho absoluto para um arquivo (com estruturas de diretório aninhadas já criadas) para o qual o arquivo (de sourceUri) será carregado na VM.
...
Corresponde ao provisionador de arquivos do Empacotador.
...
Omitir ou especificar matriz vazia para usar o padrão (sem filtro).
Consulte o link acima para ver exemplos e descrição detalhada deste campo.
...
Corresponde ao provisionador do PowerShell do Empacotador.
...
### -RestartCommand
Comando para executar a reinicialização [Padrão: 'desligamento /r /f /t 0 /c reinício do empacotador']
...
### -RestartCustomizer
Reinicia uma VM e aguarda que ela volte online (Windows).
Corresponde ao provisionador de reinicialização do windows do Empacotador.
...
### -RestartTimeout
Tempo de reinicialização especificado como uma cadeia de caracteres de magnitude e unidade, por exemplo, '5m' (5 minutos) ou '2h' (2 horas) [Padrão: '5m'].
...
Pode ser um link github, URI SAS para Armazenamento do Azure, etc.
...
Corresponde ao provisionador de shell do Empacotador.
É possível especificar exatamente um de 'scriptUri' ou 'em linha'.
...
Corresponde ao Provisionador de Atualizações do Windows do Empacotador ( https://github.com/rgl/packer-provisioner-windows-update) .
