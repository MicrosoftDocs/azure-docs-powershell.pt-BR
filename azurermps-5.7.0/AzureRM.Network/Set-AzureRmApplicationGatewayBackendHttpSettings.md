...
## DESCRITIVO
O cmdlet Set-AzureRmApplicationGatewayBackendHttpSettings atualiza as configurações de protocolo de transferência de hipertexto (HTTP) back-end para um gateway do aplicativo do Azure.
As configurações HTTP de back-end são aplicadas a todos os servidores back-end em um pool.

## EXEMPLOS

### Exemplo 1: atualizar as configurações HTTP de back-end para um Application Gateway
```
...
O segundo comando atualiza as configurações de HTTP do gateway do aplicativo na variável $AppGw para usar a porta 88, o protocolo HTTP e habilita a afinidade baseada em cookies.
...
### -ApplicationGateway
Especifica um objeto do aplicativo gateway com o qual esse cmdlet associa as configurações de HTTP back-end.
...
Os valores aceitáveis para esse parâmetro são: Disabled ou Enabled.
...
### -Nome
Especifica o nome do objeto de configurações HTTP back-end.
...
### -Teste
Especifica um teste para associar às configurações HTTP back-end.
...
### -ProbeEnabled
Sinalizar se o teste deve ser habilitado.
...
### -Probeid
Especifica a ID do teste a ser associada às configurações HTTP back-end.
...
### -Protocolo
Especifica o protocolo a ser usado para comunicação entre os servidores de gateway e de back-end do aplicativo.
