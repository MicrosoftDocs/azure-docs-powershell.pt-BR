...
## DESCRITIVO
O cmdlet **set-AzureRmKeyVaultAccessPolicy** concede ou modifica permissões existentes para um usuário, aplicativo ou grupo de segurança para executar as operações especificadas com um cofre de chaves. Ele não modifica as permissões que outros usuários, aplicativos ou grupos de segurança têm no cofre de chaves.
...
O segundo comando modifica as permissões que foram concedidas ao PattiFuller@contoso.com primeiro comando, a fim de permitir a obtenção de segredos, além de definir e excluir essas permissões. As permissões para operações de chave permanecem inalteradas após esse comando. O parâmetro *PassThru* resulta no objeto atualizado sendo retornado pelo cmdlet.

O último comando modifica as permissões existentes para PattiFuller@contoso.com remover todas as permissões para operações de chave. As permissões para operações secretas permanecem inalteradas após esse comando. O parâmetro *PassThru* resulta no objeto atualizado sendo retornado pelo cmdlet.
...
O parâmetro *servicePrincipalName* especifica o aplicativo. O aplicativo deve ser registrado no Azure Active Directory. *O valor do parâmetro servicePrincipalName* deve ser o nome principal do aplicativo ou o GUID da ID do aplicativo.
...
O primeiro comando usa o cmdlet Get-AzureRmADGroup para obter todos os grupos do Active Directory. Na saída, você verá 3 grupos retornados, chamados **grupo1** , **group2** e **Group3**. Vários grupos podem ter o mesmo nome, mas sempre têm um ObjectId exclusivo. Quando mais de um grupo com o mesmo nome for retornado, use o ObjectId na saída para identificar o que você deseja usar.

Em seguida, você usa a saída desse comando com Set-AzureRmKeyVaultAccessPolicy para conceder permissões a group2 para o seu cofre de chaves, chamado **myownvault**. Este exemplo enumera os grupos chamados "group2" embutidos na mesma linha de comando.
