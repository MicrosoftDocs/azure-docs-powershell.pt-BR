...
## Sinopse
Obtém nós de computação em lotes de um pool.
...
## DESCRITIVO
O cmdlet **Get-AzureBatchComputeNode** Obtém os nós de computação em lotes do Azure a partir de um pool.
Especifique o *poolid* ou o parâmetro do *pool* .
Especifique o parâmetro *ID* para obter um único nó de computação.
Especifique o parâmetro *Filter* para obter os nós de computação correspondentes a um filtro do Open Data Protocol (OData).

## EXEMPLOS

### Exemplo 1: obter um nó de computação por ID
```
...
Esse comando obtém o nó de computação que tem a ID TVM-2316545714_1-20150725t213220z do pool que tem a ID Pool06.
Use o cmdlet Get-AzureRmBatchAccountKeys para atribuir um contexto para a variável $Context.

### Exemplo 2: obter todos os nós de computação ociosa de um pool
```
...
Esse comando obtém todos os nós de computação ociosos contidos no pool que tem a ID Pool06.
O comando especifica o estado Idle usando o parâmetro *Filter* .

### Exemplo 3: obter todos os nós de computação em um pool especificado
```
...
Esse comando obtém o pool com a ID Pool07 usando o cmdlet Get-AzureBatchPool.
O comando passa o pool para o cmdlet atual usando o operador pipeline.
Esse cmdlet obtém todos os nós de computação desse pool.
...
Esse cmdlet retorna nós de computação que correspondem ao filtro que esse parâmetro especifica.
Se você não especificar um filtro, esse cmdlet retornará todos os nós de computação do pool.
...
### -ID
Especifica a ID do nó de computação que esse cmdlet obtém do pool.
...
### -MaxCount
Especifica o número máximo de nós de computação a serem retornados.
...
### -Pool
Especifica o pool, como um objeto **PSCloudPool** , que contém os nós de computação.
Para obter um objeto **PSCloudPool** , use o cmdlet Get-AzureBatchPool.
...
### -Poolid
Especifica a ID do pool que contém os nós de computação.
...
### PSCloudPool
O parâmetro ' Pool ' aceita o valor do tipo ' PSCloudPool ' da pipeline
