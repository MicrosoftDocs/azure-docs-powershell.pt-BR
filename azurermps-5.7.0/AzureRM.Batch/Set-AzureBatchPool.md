...
## DESCRITIVO
O cmdlet **set-AzureBatchPool** atualiza as propriedades de um pool no serviço de lote do Azure.
Use o cmdlet Get-AzureBatchPool para obter um objeto **PSCloudPool** .
Modifique as propriedades desse objeto e, em seguida, use o cmdlet atual para confirmar suas alterações no serviço de lote.

## EXEMPLOS

### Exemplo 1: atualizar um pool
```
...
O primeiro comando obtém um pool usando **Get-AzureBatchPool** e, em seguida, armazena-o na variável $pool.

Os próximos três comandos modificam a especificação Start Task no objeto $Pool.

O comando final atualiza o serviço em lotes para corresponder ao objeto local em $Pool.
...
### -Pool
Especifica o **PSCloudPool** para o qual esse cmdlet atualiza o serviço em lotes.
