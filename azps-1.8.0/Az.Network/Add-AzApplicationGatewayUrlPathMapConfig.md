...
## DESCRITIVO
O cmdlet **Add-AzApplicationGatewayUrlPathMapConfig** adiciona uma matriz de mapeamentos de caminho de URL a um pool de servidores back-end.

## EXEMPLOS

### Exemplo 1: adicionar um mapeamento de caminho de URL a um gateway de aplicativo.
```
...
O segundo comando obtém o pool de endereços de back-end e o armazena em $pool variável.
O terceiro comando obtém as configurações de back-end http e as armazena em $poolSettings variável.
O quarto comando cria uma nova configuração de regra de caminho chamada rule01 e a armazena em $pathRule variável.
O quinto comando adiciona a configuração de mapeamento de caminho de URL chamada url01 ao gateway do aplicativo.
O sexto comando atualiza o Application Gateway.

## OS

### -ApplicationGateway
Especifica o gateway do aplicativo para o qual esse cmdlet adiciona uma configuração de mapa de caminho de URL.
...
### -Nome
Especifica o nome do mapa de caminho de URL que este cmdlet adiciona ao pool do servidor back-end.
...
As regras de caminho são sigilosas de ordem, elas são aplicadas na ordem em que são especificadas.
