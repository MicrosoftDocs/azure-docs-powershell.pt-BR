...
## Descrição
O cmdlet **Set-AzSqlServerThreatDetectionPolicy** define uma política de detecção de ameaças em um servidor SQL do Azure.
Para habilitar a detecção de ameaças em um servidor, uma política de auditoria deve ser habilitada nesse servidor.
...
Esse comando define a política de detecção de ameaças para um servidor chamado Server01.
...
### -EmailAdmins
Especifica se a política de detecção de ameaças contata os administradores usando o email.
...
Especifica uma matriz de tipos de detecção a ser excluída da política.
...
Especifica uma lista separada por ponto e vírgula dos endereços de email para os quais a política envia alertas.
...
### -StorageAccountName
Especifica o nome da conta de armazenamento a ser usada. Caracteres curinga não são permitidos. Este parâmetro não é necessário. Quando esse parâmetro não for fornecido, o cmdlet usará a conta de armazenamento que foi definida anteriormente como parte da política de detecção de ameaças do banco de dados. Se esta for a primeira vez que uma política de detecção de banco de dados theat é definida e esse parâmetro não é fornecido, o cmdlet falhará.
