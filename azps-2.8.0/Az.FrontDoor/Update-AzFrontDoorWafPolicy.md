...
## DESCRITIVO
O cmdlet **Update-AzFrontDoorWafPolicy** atualiza uma política WAF existente. Se os parâmetros de entrada não forem fornecidos, os parâmetros antigos da política WAF existente serão usados.
...
Atualize um código de status personalizado de política de WAF existente.
...
Atualizar um modo de política WAF existente.
...
Atualizar um estado e modo habilitados para política do WAF existente.
...
Atualizar todas as políticas do WAF no $resourceGroupName
...
Código de status de resposta personalizado
...
### -Enabledstate
Se a política está no estado habilitado ou desabilitado.
Os valores possíveis incluem: ' disabled ', ' Enabled '
...
### -InputObject
O objeto FireWallPolicy a ser atualizado.
...
### -ManagedRule
Regras gerenciadas dentro da política
...
### -Mode
Descreve se ele está no modo de detecção ou no modo de prevenção em nível de política.
Os valores possíveis incluem: ' Prevention ', ' detecção '
...
### -Nome
O nome do FireWallPolicy a ser atualizado.
...
### -ResourceGroupName
O grupo de recursos ao qual o FireWallPolicy pertence.
...
### -ResourceId
ID do recurso do FireWallPolicy para atualizar
