...
## DESCRITIVO
O cmdlet **Get-AzTenantDeploymentWhatIfResult** Obtém o modelo ARM What-If resultado para uma implantação de modelo no escopo do locatário especificado. Ele retorna uma lista de alterações indicando quais recursos serão atualizados se a implantação for aplicada sem fazer alterações em recursos reais. Para especificar o formato do resultado retornado, use o parâmetro *ResultFormat* .

## EXEMPLOS

### Exemplo 1: obter um resultado de What-If no escopo do locatário
```powershell
...
Esse comando obtém um resultado What-If em escopo do locatário usando um arquivo de modelo personalizado e um arquivo de parâmetro no disco.
...
### Exemplo 2: obter um resultado What-If no escopo do locatário com ResourceIdOnly
```powershell
PS C:\> Get-AzTenantDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -Location "West US" `
