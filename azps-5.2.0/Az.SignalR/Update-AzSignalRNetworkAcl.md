...
## Sinopse
Atualize a ACL de rede de um serviço de sinal de acesso.
...
## DESCRITIVO
Atualize a ACL de rede de um serviço de sinal, incluindo a ação padrão e as ACLs de rede para conexão pública e privada.

## EXEMPLOS

### Permitir o RESTAPI, o ClientConnection para redes públicas e definir a ação padrão para recusar
```powershell
...
### Permitir conexão do cliente e conexão do servidor para uma conexão de ponto de extremidade particular
```powershell
...
### Negar conexão do cliente para a rede pública e uma conexão de ponto de extremidade particular
```powershell
...
### -DefaultAction
A ação padrão das ACLs de rede Signalr, permitir ou recusar. Ele decide se a negação de ACLs de rede ou permite que ACLs de rede entrem em vigor. Por exemplo, se a ação padrão for permitir, somente as ACLs de negação serão importantes.
...
### -PrivateEndpointName
Nome (s) de ponto (s) particular (is) a ser atualizado (s)
