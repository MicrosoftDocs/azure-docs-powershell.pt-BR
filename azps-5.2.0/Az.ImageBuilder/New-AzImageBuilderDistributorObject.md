...
### ManagedImageDistributor (padrão)
```
...
### Exemplo 1: criar um distribuidor de imagens gerenciadas
```powershell
...
Esse comando cria um distribuidor de imagens gerenciadas.

### Exemplo 2: criar um distribuidor VHD
```powershell
...
Esse comando cria um distribuidor VHD.

### Exemplo 3: criar um distribuidor de imagens compartilhadas
```powershell
...
Esse comando cria um distribuidor de imagens compartilhadas.

## OS

### -ArtifactTag
Marcas que serão aplicadas ao artefato após a criação/atualização do distribuidor.
...
### -ExcludeFromLatest
Sinalizador que indica se a versão da imagem criada deve ser excluída do mais recente.
Omita para usar o padrão (falso).
...
### -GalleryImageId
ID do recurso da imagem da Galeria de imagens compartilhada.
...
### -Local
O Azure Location para a imagem deve corresponder se a imagem já existe.
...
Distribuir como uma imagem de disco gerenciado.
...
### -ReplicationRegion
Uma lista de regiões para as quais a imagem será replicada.
...
### -SharedImageDistributor
Distribuir via Galeria de imagens compartilhada.
...
### -StorageAccountType
Tipo de conta de armazenamento a ser usado para armazenar a imagem compartilhada.
Omita para usar o padrão (Standard_LRS).
...
### -VhdDistributor
Distribuir via VHD em uma conta de armazenamento.
