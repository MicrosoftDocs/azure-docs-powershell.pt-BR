...
## DESCRITIVO
O cmdlet New-AzNetworkWatcherConnectionMonitorTestConfigurationObject cria uma configuração de teste de monitor de conexão.
...
Nome: httpTC TestFrequencySec: 120 PreferredIPVersion: ProtocolConfiguration: {"porta": 443, "método": "GET", "GET", [{"Name": "Allow", "value": "GET"}], "ValidStatusCodeRanges": ["2xx", "300-308"], "PreferHTTPS": verdadeiro} SuccessThreshold: {"ChecksFailedPercent": 20, "RoundTripTimeMs": 30} 
...
### -Nome
O nome da configuração de teste do monitor de conexão.
...
### -PreferredIPVersion
A versão de IP preferencial a ser usada na avaliação de teste. O monitor de conexão pode optar por usar uma versão diferente, dependendo dos outros parâmetros.
...
Os parâmetros usados para executar a avaliação de teste em um protocolo.
...
A porcentagem máxima de verificações com falha permitidas para um teste ser avaliada como bem-sucedida.
...
O tempo máximo de ida e volta em milissegundos permitido para um teste avaliar como bem-sucedido.
...
### -TestFrequencySec
A frequência da avaliação do teste, em segundos.
