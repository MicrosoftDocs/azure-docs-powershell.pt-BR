...
### Exemplo 1: adicionar um certificado SSL usando pfx a um gateway de aplicativo.
```
...
### Exemplo 2: adicionar um certificado SSL usando o segredo do keyvault (versão menos secreta) para um gateway de aplicativo.
```
PS C:\> $AppGW = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
...
Obtenha o segredo e faça referência a ele no `Add-AzApplicationGatewaySslCertificate` para adicioná-lo ao gateway do aplicativo com o nome `Cert01` .
Observação: como a versão-menos secreta é fornecida aqui, o Application Gateway sincronizará o certificado em intervalos regulares com o cofre de chave.

### Exemplo 3: adicionar um certificado SSL usando o segredo do keyvault (com versão secretaid) em um gateway de aplicativo.
```
PS C:\> $AppGW = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
PS C:\> $secret = Get-AzKeyVaultCertificate -VaultName "keyvault01" -Name "sslCert01"
...
Obtenha o segredo e faça referência a ele no `Add-AzApplicationGatewaySslCertificate` para adicioná-lo ao gateway do aplicativo com o nome `Cert01` .
Observação: se for necessário que o Application Gateway Sincronize o certificado com o keyvault, forneça o nome secreto sem versão.
...
### -KeyVaultSecretId
Secretid (URI) do segredo do keyvault. Use esta opção quando for preciso usar uma versão específica do segredo.
