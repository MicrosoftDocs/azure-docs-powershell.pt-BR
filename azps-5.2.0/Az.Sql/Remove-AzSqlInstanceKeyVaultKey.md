...
## DESCRITIVO
O cmdlet Remove-AzSqlInstanceKeyVaultKey remove a chave do cofre de chaves da instância gerenciada especificada. Observe que as permissões da instância gerenciada SQL para o cofre da chave não são alteradas. Para alterar permissões, use set-AzKeyVaultAccessPolicy. Observe que esse cmdlet não faz alterações no cofre de chaves. Para remover uma chave do cofre de chaves, use remove-AzureKeyVaultKey.
...
Este comando Remove a chave do cofre de chaves com ID ' https://contoso.vault.azure.net/keys/contosokey/01234567890123456789012345678901 ' da instância gerenciada especificada. 
