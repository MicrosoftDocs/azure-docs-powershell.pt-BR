...
## DESCRITIVO
O cmdlet **New-AzSqlInstance** cria uma instância gerenciada do banco de dados SQL do Azure.
...
Esse comando cria uma nova instância usando o parâmetro SkuName.
...
### Exemplo 3: criar uma nova instância em um pool de instâncias usando um objeto de pool de ocorrências
```powershell
...
Esse comando cria uma nova instância em um pool de instâncias usando um objeto de pool de instâncias.

### Exemplo 4: criar uma nova instância em um pool de instâncias usando um identificador de recursos de pool de instâncias
```powershell
...
Esse comando cria uma nova instância em um pool de instâncias usando o identificador de recursos do pool de instâncias.

### Exemplo 5: criar uma nova instância em um pool de instâncias
```powershell
...
Esse comando cria uma nova instância em um pool de instâncias com o nome instancePool0
...
A redundância do armazenamento de backup usada para armazenar backups para a instância gerenciada do SQL Azure. Opções são: local, região e geo
...
### -Agrupamento
O agrupamento da instância gerenciada do SQL do Azure a ser usada.
...
### -DnsZonePartner
A ID do recurso do servidor gerenciado por parceiro para herdar a propriedade DnsZone da criação da instância gerenciada
...
### -InstancePool
O objeto pai do pool de instâncias.
...
### -InstancePoolName
O pool de instâncias no qual colocar esta instância.
...
A ID de recurso do pool de instâncias.
...
### -LicenseType
Determina qual tipo de licença usar. Os valores possíveis são:
- BasePrice-o benefício híbrido do Azure (AHB) foi aplicado o preço com desconto para os proprietários de licenças existentes do SQL Server. O preço do serviço de instância gerenciado será descontado para os proprietários de licença existentes do SQL Server.
- LicenseIncluded-o AHB (benefício híbrido do Azure) não é aplicado com o preço de desconto dos proprietários de licenças existentes do SQL Server. O preço do serviço de instância gerenciada incluirá um novo custo de licença do SQL Server.
...
A ID de configuração de manutenção para a instância gerenciada do SQL Azure.
...
### -MinimalTlsVersion
A versão do TLS mínima a ser aplicada para a instância gerenciada 
...
### -ProxyOverride
O tipo de conexão usado para conexão com a instância.
...
Se o ponto de extremidade de dados públicos está ou não habilitado para a instância.
...
### -TimeZoneID
A ID de fuso horário da instância a ser definida. Uma lista de IDs de fuso horário é exposta por meio do modo de exibição do sys.time_zone_info (Transact-SQL).
