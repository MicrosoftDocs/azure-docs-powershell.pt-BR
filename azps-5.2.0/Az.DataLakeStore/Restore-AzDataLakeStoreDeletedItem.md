...
## Sinopse
Restaurar um arquivo ou uma pasta excluída no Azure data Lake.
...
## DESCRITIVO
O cmdlet **Restore-AzDataLakeStoreDeletedItem** restaura um arquivo ou uma pasta excluída no repositório data Lake. Requer o caminho do item excluído no lixo retornado por Get-AzDataLakeStoreDeletedItem.
Cuidado: não excluir arquivos é uma melhor operação de esforço. Não há garantias de que um arquivo possa ser restaurado depois que ele é excluído. O uso dessa API é habilitado por meio da lista branca. Se a sua conta do ADL não for listada, o uso dessa API irá gerar uma exceção não implementada. Para obter mais informações e assistência, entre em contato com o suporte da Microsoft.

## EXEMPLOS

### Exemplo 1: restaurar um arquivo do data Lake Store usando a opção-Force
```
...
### -Destino
O caminho de destino para o qual o arquivo ou a pasta excluída deve ser restaurado. 
...
### -PassThru
Retorna booliano true no sucesso.
...
### -Caminho
O caminho do arquivo ou da pasta excluída no lixo.
...
### -Restoreaction
Ação a ser tomada em conflito com o nome do destino-"cópia" ou "substituir"
...
### -Digite
O tipo de entrada sendo restaurado-"arquivo" ou "pasta"
