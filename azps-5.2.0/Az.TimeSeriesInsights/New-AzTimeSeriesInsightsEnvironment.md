...
### Exemplo 1: criar um ambiente do insights série de tempo do Gen1
```powershell
...
Esse comando cria um ambiente do insights série tempo do Gen1.

### Exemplo 2: criar um ambiente do insights série de tempo do Gen2
```powershell
...
Esse comando cria um ambiente do insights série tempo do Gen2.
...
### -Capacidade
A capacidade da SKU.
Para ambientes Gen1, esse valor pode ser alterado para dar suporte ao dimensionamento de ambientes após a criação.
...
A lista de propriedades de evento que será usada para particionar dados no ambiente.
Para construir, consulte a seção notas para propriedades PARTITIONKEYPROPERTY e crie uma tabela de hash.
...
### -StorageAccountKey
O valor da chave de gerenciamento que concede o acesso de gravação do serviço de insights série de tempo à conta de armazenamento.
...
### -StorageAccountName
O nome da conta de armazenamento que manterá os dados de longo prazo do ambiente.
...
O comportamento que o serviço insights de série de tempo deve levar quando a capacidade do ambiente foi excedida
...
A lista de propriedades do evento que será usada para definir a ID da série temporal do ambiente. Para construir, consulte a seção notas para propriedades TIMESERIESIDPROPERTY e crie uma tabela de hash.
...
PARTITIONKEYPROPERTY <ITimeSeriesIdProperty [] >: a lista de propriedades do evento que será usada para particionar dados no ambiente.
  - `[Name <String>]`: O nome da propriedade.
  - `[Type <PropertyType?>]`: O tipo da propriedade.

TIMESERIESIDPROPERTY <ITimeSeriesIdProperty [] >: a lista de propriedades do evento que será usada para definir a ID da série temporal do ambiente.
  - `[Name <String>]`: O nome da propriedade.
  - `[Type <PropertyType?>]`: O tipo da propriedade.
