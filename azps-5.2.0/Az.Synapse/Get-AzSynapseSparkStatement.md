...
### GetSparkStatementsByIdParameterSet (padrão)
```
...
## DESCRITIVO
O cmdlet **Get-AzSynapseSparkStatement** Obtém informações sobre uma declaração Spark do Azure Synapse Analytics.
...
Esse comando obtém todas as instruções do Spark em uma sessão do Spark.
...
Esse comando obtém a instrução Spark com a ID Livy especificada.
...
Esse comando obtém todas as instruções do Spark em uma sessão do Spark por meio do pipeline.
