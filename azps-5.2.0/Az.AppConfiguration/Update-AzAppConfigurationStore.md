...
### Exemplo 1: habilitar a criptografia de dados do repositório de configuração do aplicativo por identidade gerenciada atribuída pelo sistema
```powershell
...
Esse comando habilita a criptografia de dados por uma chave armazenada no cofre de chaves do Azure usando uma identidade gerenciada atribuída pelo sistema.
O cofre deve ter uma exclusão de exclusão flexível e proteção de limpeza, e a identidade gerenciada deve ter essas permissões de chave: Get, wrapKey, unwrapKey.

### Exemplo 2: habilitar a criptografia de dados do repositório de configuração de aplicativo por identidade gerenciada atribuída pelo usuário
```powershell
PS C:\> $key = Add-AzKeyVaultKey -VaultName kv-Name -Name key-Name -Destination 'Software'
...
Esse comando habilita a criptografia de dados por uma chave armazenada no cofre de chaves do Azure usando uma identidade gerenciada atribuída pelo usuário.
A identidade atribuída pelo usuário deve ter sido definida com `-UserAssignedIdentity` .
O cofre deve ter uma exclusão de exclusão flexível e proteção de limpeza, e a identidade gerenciada deve ter essas permissões de chave: Get, wrapKey, unwrapKey.

### Exemplo 3: desabilitar a criptografia de um repositório de configuração de aplicativo.
```powershell
...
Esse comando desabilita a criptografia de um repositório de configuração de aplicativo.

### Exemplo 3: atualizar SKU e marcar um repositório de configuração de aplicativo por pipeline.
```powershell
...
Esse comando atualiza a SKU e a marca de um repositório de configuração de aplicativo por pipeline.
...
O tipo ' nenhum ' removerá todas as identidades.
...
A ID do cliente da identidade que será usada para acessar o cofre de chaves.
...
### -SKU
O nome do SKU do repositório de configuração.
...
As chaves de dicionário de identidade atribuídas pelo usuário serão as IDs de recurso ARM no formato: '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName} '.
