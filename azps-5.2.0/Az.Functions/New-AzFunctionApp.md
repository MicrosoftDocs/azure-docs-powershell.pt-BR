...
### Exemplo 1: criar um aplicativo de função de consumo do PowerShell nos EUA.
```powershell
...
Esse comando cria um aplicativo de função de consumo do PowerShell nos EUA.

### Exemplo 2: criar um aplicativo de função do PowerShell que será hospedado em um plano de serviço.
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
### Exemplo 3: criar um aplicativo de função usando uma imagem de ACR particular.
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
Esse comando cria um aplicativo de função usando uma imagem de ACR particular.

## OS

### -ApplicationInsightsKey
Chave de instrumentação de ideias do aplicativo a ser adicionada.
...
Nome do projeto insights do App existente a ser adicionado ao aplicativo function.
...
Desabilitar a criação do recurso de insights do aplicativo durante a criação da função do aplicativo.
Nenhum registro estará disponível.
...
Obrigatório para registros particulares.
...
Os valores aceitáveis para esse parâmetro são:-SystemAssigned-userassigned
...
Para determinar se a operação foi concluída com êxito, use outro mecanismo.
