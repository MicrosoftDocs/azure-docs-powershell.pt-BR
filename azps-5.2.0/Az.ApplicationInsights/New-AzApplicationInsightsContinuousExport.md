...
### Exemplo 1 criar uma nova configuração de exportação contínua para um recurso do Application insights
```
...
Criar uma nova configuração de exportação contínua do Application insights para exportar tipos de documento "solicitação" e "rastreamento" para armazenamento contêm "testcontainer" na conta de armazenamento "teststorageaccount" no grupo de recursos "grupo de teste". O token SAS tem que ser válido e ter permissão de gravação para o contêiner; caso contrário, o recurso de exportação contínua não funcionará. Se o token SAS venceu, o recurso de exportação contínua deixará de funcionar.
