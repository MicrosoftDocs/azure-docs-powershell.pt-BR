...
## DESCRITIVO
O cmdlet **New-AzApiManagementProduct** cria um produto de gerenciamento de API.

## EXEMPLOS

### Exemplo 1: criar um produto que não exija uma assinatura
```
...
Esse comando cria um produto de gerenciamento de API.
Não é preciso ter uma assinatura.

### Exemplo 2: criar um produto que exija uma assinatura e uma aprovação
```
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Esse comando cria um produto.
Uma assinatura e aprovação são necessárias.
Esse comando define o período de notificação como 10 dias.
A duração da assinatura é definida como um ano.

## OS

### -ApprovalRequired
Indica se a assinatura do produto requer aprovação ou não.
Por padrão, esse parâmetro é **$false**.
...
### -ProductId
Especifica o identificador do novo produto.
Se você não especificar esse parâmetro, um novo produto será gerado.
...
psdx_paramvalues
- Não publicado
- Publicado o valor padrão é não publicado.
...
### -Título
Especifica o título do produto.
...
### System. Nullable ' 1 [[Microsoft. Azure. Commands. ApiManagement. Management. Models. PsApiManagementProductState, Microsoft. Azure. PowerShell. cmdlets. ApiManagement. immanagement, Version = 1.0.0.0, Culture = neutral, PublicKeyToken = null]]
