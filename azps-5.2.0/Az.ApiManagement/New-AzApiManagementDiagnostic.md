...
## DESCRITIVO
O cmdlet **New-AzApiManagementDiagnostic** cria uma entidade de diagnóstico em escopo global ou escopo de API específico.

## EXEMPLOS

### Exemplo 1: criar um novo diagnóstico de escopo global
```powershell
...
Este exemplo cria uma entidade de diagnóstico no escopo global.

### Exemplo 2: criar um diagnóstico no escopo da API
```powershell
PS D:\github\azure-powershell> $context = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
O exemplo acima cria um diagnóstico para a API `httpbin` para registrar o cabeçalho e 100 bytes de corpo no `azuremonitor` Logger.
...
Se especificado, definirá a política de escopo de API.
...
### -Diagnosticid
Identificador da entidade de diagnóstico.
Este parâmetro é opcional.
