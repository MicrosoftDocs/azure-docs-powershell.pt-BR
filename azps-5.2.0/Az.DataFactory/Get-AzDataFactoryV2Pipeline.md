...
## DESCRITIVO
O cmdlet Get-AzDataFactoryV2Pipeline Obtém informações sobre pipelines no Azure data Factory.
...
Você pode usar qualquer um dos comandos de exemplo a seguir.
...
### Exemplo 4: obter informações sobre as entradas para a primeira atividade
```powershell
...
Esse comando obtém informações para o pipeline chamado DPWikisample na fábrica de dados chamado WikiADF e, em seguida, usa a notação de ponto padrão para exibir a propriedade Activities associada a esse pipeline.
O comando exibe a propriedade inputs do primeiro elemento da matriz Activities usando o cmdlet Format-List.
