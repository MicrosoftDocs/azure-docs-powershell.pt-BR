...
## DESCRITIVO
O cmdlet Get-AzureBatchPoolNodeCounts permite que os clientes obtenham contagens de nós back por estado de nó agrupadas por pool. Estados de nó possíveis são: Creating, Idle, leavingPool, offline, preempção, reinicialização, recriação de imagens, execução, início, startTaskFailed, desconhecido, inutilizável e waitingForStartTask. O cmdlet usa poolid ou parâmetro de pool para filtrar somente o pool com ID de pool especificado. 
