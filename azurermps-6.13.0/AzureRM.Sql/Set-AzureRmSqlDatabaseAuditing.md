...
## DESCRITIVO
O cmdlet **set-AzureRmSqlDatabaseAuditing** altera as configurações de auditoria de um banco de dados SQL do Azure.
...
Se o cmdlet for bem-sucedido e você usar o parâmetro *PassThru* , ele retornará um objeto que descreve a política de auditoria de blob atual, além dos identificadores de banco de dados.
Os identificadores de banco de dados incluem, entre outros, **ResourceGroupName** , **nomedoservidor** e **DatabaseName**.

## EXEMPLOS

### Exemplo 1: habilitar a política de auditoria de um banco de dados SQL do Azure
```
PS C:\>Set-AzureRmSqlDatabaseAuditing -State Enabled -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -StorageAccountName "Storage22" -DatabaseName "Database01"
```

### Exemplo 2: desabilitar a política de auditoria de blob de um banco de dados SQL do Azure
```
PS C:\>Set-AzureRmSqlDatabaseAuditing -State Disabled -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -DatabaseName "Database01"
```

### Exemplo 3: habilitar a política de auditoria de um banco de dados SQL do Azure usando uma conta de armazenamento de uma assinatura diferente
```
...
As ações com suporte a auditoria são: selecionar atualizar inserir excluir executar como referências o formato geral para definir uma ação a ser auditada é: [ação] em [objeto] por [principal] Observe que [objeto] no formato acima pode se referir a um objeto, como uma tabela, um modo de exibição ou um procedimento armazenado ou um esquema ou banco de dados inteiro. Para os últimos casos, o banco de dados de formulários:: [dbname] e esquema:: [SchemaName] são usados, respectivamente.
Por exemplo: selecionar em dbo. myTable por público selecionar no banco de dados:: MyDatabase por público selecione em esquema:: MySchema by Public para obter mais informações, consulte https://docs.microsoft.com/en-us/sql/relational-databases/security/auditing/sql-server-audit-action-groups-and-actions#database-level-audit-actions .
