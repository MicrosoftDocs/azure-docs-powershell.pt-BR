...
## DESCRITIVO
O cmdlet **Remove-AzDelegation** leva em uma sub-rede com delegações e remove a delegação nomeada dessa sub-rede.
...
Neste exemplo, a primeira metade (encontrada em _"Adicionar delegação a uma sub-rede existente"_) é idêntica a [Add-AzDelegation](./Add-AzDelegation.md). Na segunda metade, os dois primeiros cmdlets recuperam a sub-rede do interesse, atualizando a cópia local com o que há no servidor. O terceiro cmdlet Remove a delegação que foi criada na primeira metade de _mysubnet_ e armazena a sub-rede atualizada em _$subnet_. O cmdlet final atualiza o servidor com a delegação removida.
...
### -Subnet
A sub-rede da qual você deseja remover a delegação
