...
## DESCRITIVO
O cmdlet **Get-AzIntegrationAccountAssembly** cria um novo assembly em uma conta de integração.

## EXEMPLOS

### Exemplo 1: criar um novo assembly usando um arquivo local
```powershell
...
Cria um novo assembly usando o arquivo local localizado no caminho do arquivo contido em "$localAssemblyFilePath".

### Exemplo 2: criar um novo assembly usando dados de bytes
```powershell
...
Cria um novo assembly usando a matriz de bytes contida em "$assemblyContent".

### Exemplo 3: criar um novo assembly usando um link de conteúdo
```powershell
...
Cria um novo assembly usando os dados de bytes localizados na URL "$assemblyUrl". Esse é o método sugerido para a criação de conjuntos de grandes tamanhos.
...
### -ParentResourceId
A ID do recurso da conta de integração.
