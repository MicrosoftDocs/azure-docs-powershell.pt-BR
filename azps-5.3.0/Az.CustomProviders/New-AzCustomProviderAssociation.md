...
## Sinopse
Criar ou atualizar uma associação.
...
### Exemplo 1: criar uma associação de provedor personalizado
```powershell
...
Criar uma associação de provedor personalizado, o destino associado provioder deve ser configurado corretamente com uma rota para "associações"
...
O escopo pode ser qualquer instância de recurso REST válida.
Por exemplo, use "/subscriptions/{subscription-id}/resourceGroups/{resource-group-name}/providers/Microsoft.Compute/virtualMachines/{vm-name}" para um recurso de máquina virtual.
...
### -TargetResourceId
A instância do recurso REST do recurso de destino para essa associação.
