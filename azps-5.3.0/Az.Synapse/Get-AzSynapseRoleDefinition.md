...
### GetByWorkspaceNameAndIdParameterSet (padrão)
```
...
## DESCRITIVO
O cmdlet **Get-AzSynapseRoleDefinition** Obtém uma definição de função de análise do Azure Synapse.
Se você não especificar um nome de função ou uma ID de função, esse cmdlet obterá todas as definições de função.
...
Esse comando obtém a definição de função em espaço de trabalho ContosoWorkspace com o nome ContosoRole.
...
Esse comando obtém todas as definições de função em um espaço de trabalho por pipeline.
