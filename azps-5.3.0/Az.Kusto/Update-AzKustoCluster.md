...
O comando acima atualiza a SKU do cluster Kusto "testnewkustocluster" encontrada no grupo de recursos "testrg".
...
O comando acima atualiza o cluster "testnewkustocluster" localizado no grupo de recursos "testrg" com uma chave gerenciada pelo cliente.
...
Um valor booliano que indica se os discos do cluster estão criptografados.
...
Um valor booliano que indica se a inclusão de streaming está habilitada.
...
### -IdentityType
O tipo de identidade gerenciada usado.
O tipo ' SystemAssigned, userassigned ' inclui uma identidade criada implicitamente e um conjunto de identidades atribuídas pelo usuário.
O tipo ' nenhum ' vai remover todas as identidades.
...
A lista de identidades de usuário associadas ao cluster Kusto.
As referências de chave de dicionário de identidade do usuário serão ARM IDs de recurso no formulário: '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName} '.
...
A identidade atribuída ao usuário (ID do recurso ARM) que tem acesso à chave.
...
Para construir, consulte a seção notas para propriedades LANGUAGEEXTENSIONVALUE e crie uma tabela de hash.
...
Um valor booliano que indica se o recurso de autoescala otimizado está habilitado ou não.
...
Contagem máxima permitida de instâncias permitidas.
...
Contagem mínima permitida de instâncias permitidas.
...
Locatários externos do cluster.
Para construir, consulte a seção notas para propriedades TRUSTEDEXTERNALTENANT e crie uma tabela de hash.
...
ID do recurso do endereço IP público do serviço de gerenciamento de dados.
...
ID do recurso do endereço IP público do serviço de mecanismo.
...
LANGUAGEEXTENSIONVALUE <ILanguageExtension [] >: a lista de extensões de idioma.
  - `[Name <LanguageExtensionName?>]`: O nome da extensão do idioma.

TRUSTEDEXTERNALTENANT <ITrustedExternalTenant [] >: os locatários externos do cluster.
  - `[Value <String>]`: GUID que representa um locatário externo.
