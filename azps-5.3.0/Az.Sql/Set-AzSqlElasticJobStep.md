...
## DESCRITIVO
O cmdlet Set-AzSqlElasticJobStep atualiza uma etapa do trabalho
...
### Exemplo 2: atualiza um script T-SQL de etapa do trabalho para um trabalho
```powershell
PS C:\> $jobStep = Get-AzSqlElasticJobStep -ResourceGroupName rg -ServerName elasticjobserver -AgentName agent -JobName job1 -StepName step1
...
Atualiza uma etapa de trabalho a partir de um trabalho
...
O intervalo inicial de segundos de repetição
...
O intervalo de repetição máximo de segundos
...
A ID do recurso de banco de dados de saída
...
### -ResourceId
A ID do recurso etapa do trabalho
...
O multiplicador de retrocesso do intervalo de repetição
...
### -StepId
O texto da identificação da etapa
