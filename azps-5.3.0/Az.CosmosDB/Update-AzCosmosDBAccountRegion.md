...
## DESCRITIVO
Atualize regiões de uma conta do CosmosDB. O local pode ser fornecido como um objeto do tipo PSLocation ou como cadeias de caracteres do nome da localização ordenada pela prioridade de failover.
O parâmetro locationobject espera a lista de locais atuais (failover prioritiies incluída) acrescentada pela nova LocationObjects correspondente a novos locais a serem adicionados.
O parâmetro Location espera a lista de localização atual (ordenada pela prioridade de failover) e os novos locais. Observe que só há suporte para a adição de regiões. Forneça o Location ou o Locationobject.
...
### -Local
Nome do local a ser adicionado.
...
### -Locationobject
Adicione um local para a conta de banco de dados do cosmos DB. Matriz de objetos PSLocation.
