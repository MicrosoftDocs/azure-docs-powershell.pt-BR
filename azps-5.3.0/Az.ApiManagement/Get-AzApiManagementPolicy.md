...
## DESCRITIVO
O cmdlet **Get-AzApiManagementPolicy** Obtém a política de escopo especificada.
...
### Exemplo 5: obter a política de escopo do locatário no formato RawXml
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Este comando obtém a política de escopo de locatário em formato de saída não XML.
...
### -ApiRevision
Identificador da revisão da API. Este parâmetro é opcional. Se não for especificado, a política será recuperada da revisão de API ativa no momento.
...
O valor padrão para esse parâmetro é "XML".
