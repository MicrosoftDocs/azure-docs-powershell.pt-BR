...
## DESCRITIVO
O cmdlet New-AzDataFactoryV2LinkedServiceEncryptedCredential criptografar credenciais no serviço vinculado com tempo de execução de integração especificado.

Certifique-se de que os seguintes pré-requisitos são atendidos:
* A opção de **acesso remoto** está habilitada no tempo de execução de integração de hospedagem automática.
* O PowerShell 7,0 ou posterior é usado para executar o cmdlet.

## EXEMPLOS

### Exemplo 1: criptografar credenciais em um serviço vinculado

Esse comando criptografa a credencial no arquivo C:\samples\WikiSample\TaxiDemo1.jscom o tempo de execução de integração chamado Test-selfhost-IV.
