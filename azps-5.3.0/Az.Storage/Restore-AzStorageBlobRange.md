...
O intervalo inicial é incluído, e o intervalo final é excluído na restauração de BLOB.
...
Esse comando primeiro cria 2 intervalos de BLOB e, em seguida, inicia a restauração de BLOBs em uma conta de armazenamento com os 2 intervalos de blob de 1 dia atrás. O usuário pode usar Get-AzStorageAccount para rastrear o status da restauração mais tarde.
...
Esse comando restaura todos os BLOBs em uma conta de armazenamento de 30 minutos atrás e aguarde até que a restauração seja concluída. Como os blobs de restauração podem levar muito tempo, execute-o no back-end com o parâmetro AsJob e aguarde a conclusão do trabalho e mostre o resultado.

### Exemplo 3: restaura os blobs por entrada de intervalos de blob diretamente e aguardar conclusão
```powershell
...
Esse comando restaura os BLOBs em uma conta de armazenamento a partir de um dia atrás, por entrada 2 intervalos de blob diretamente para o cmdlet Restore-AzStorageBlobRange. Esse comando irá aguardar a restauração ser concluída.
...
### -BlobRestoreRange
O intervalo de BLOBs a ser restaurado.
Se você não especificar esse parâmetro, restaurará todos os BLOBs.
...
### -WaitForComplete
Aguarde a conclusão da tarefa de restauração
