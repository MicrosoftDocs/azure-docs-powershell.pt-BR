...
### Exemplo 5: atualizar um imposiçãomode
```
PS C:\> $ResourceGroup = Get-AzResourceGroup -Name 'ResourceGroup11'
PS C:\> $PolicyAssignment = Get-AzPolicyAssignment -Name 'PolicyAssignment' -Scope $ResourceGroup.ResourceId
...
O segundo comando obtém a atribuição de política chamada PolicyAssignment usando o cmdlet Get-AzPolicyAssignment.
O comando armazena esse objeto na variável $PolicyAssignment.
O comando final atualiza a propriedade imposiçãomode na atribuição de política no grupo de recursos identificado pela propriedade **ResourceId** de $ResourceGroup.
...
### -InputObject
O objeto de atribuição de política para atualizar que tinha saída de outro cmdlet.
...
### -PolicyParameter
O novo caminho de arquivo de parâmetros de política ou a cadeia de caracteres da atribuição de política.
