...
### Exemplo 1: criar um modelo de imagem de máquina virtual
```powershell
...
Este comando cria um modelo de imagem de máquina virtual.
...
Duração máxima a esperar durante a criação do modelo de imagem.
Omita ou especifique 0 para usar o padrão (4 horas).
...
### -Personalizar
Especifica as propriedades usadas para descrever as etapas de personalização da imagem, como origem da imagem, etc. Para construir, consulte a seção de anotações para personalizar Propriedades e criar uma tabela de hash.
...
### -DefaultProfile
região HideParameter as credenciais, conta, locatário e assinatura usadas para comunicação com o Azure.
...
### -Distribuir
Os destinos de distribuição para os quais a saída da imagem precisa ir.
Para construir, consulte a seção de anotações para distribuir Propriedades e criar uma tabela de hash.
...
Para construir, consulte a seção de notas para propriedades de origem e crie uma tabela de hash.
...
A ID da identidade atribuída pelo usuário.
...
Tamanho do disco do sistema operacional em GB.
Omita ou especifique 0 para usar o tamanho do disco do sistema operacional padrão do Azure.
...
### -VMProfileVmSize
Tamanho da máquina virtual usada para criar, personalizar e capturar imagens.
Omita ou especifique uma cadeia de caracteres vazia para usar o padrão (Standard_D1_v2).
...
Personalizar <IImageTemplateCustomizer [] >: especifica as propriedades usadas para descrever as etapas de personalização da imagem, como origem da imagem, etc.
  - `Type <String>`: O tipo de ferramenta de personalização que você deseja usar na imagem. Por exemplo, "Shell" pode ser personalizador do Shell
  - `[Name <String>]`: Nome amigável para fornecer contexto sobre o que essa etapa de personalização faz

DISTRIBUIR <IImageTemplateDistributor [] >: os destinos de distribuição onde a saída da imagem precisa ir.
  - `RunOutputName <String>`: O nome a ser usado para o RunOutput associado.
  - `Type <String>`: Tipo de distribuição.
  - `[ArtifactTag <IImageTemplateDistributorArtifactTags>]`: Marcas que serão aplicadas ao artefato após a criação/atualização do distribuidor.
    - `[(Any) <String>]`: Isso indica que qualquer propriedade pode ser adicionada a esse objeto.

FONTE <IImageTemplateSource> : descreve uma fonte de imagem de máquina virtual para construir, personalizar e distribuir.
  - `Type <String>`: Especifica o tipo de imagem de origem com a qual você deseja começar.
