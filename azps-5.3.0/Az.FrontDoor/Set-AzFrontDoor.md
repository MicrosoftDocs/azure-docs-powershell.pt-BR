...
## DESCRITIVO
O cmdlet **set-AzFrontDoor** atualiza um balanceador de carga de porta frontal. Se os parâmetros de entrada não forem fornecidos, os parâmetros antigos da porta frontal existente serão usados.

## EXEMPLOS

### Exemplo 1: Atualize uma porta frontal existente com o FrontDoorName e o ResourceGroupName.
```powershell
...
### Exemplo 2: Atualize uma porta frontal existente com objeto PSFrontDoor.
```powershell
...
### Exemplo 3: atualizar uma porta frontal existente com ResourceId
```powershell
...
### Exemplo 4: atualizar a EnforceCertificateNameCheck de propriedade BackendPoolSetting de uma porta frontal existente com parâmetro de opção DisableCertificateNameCheck
A porta frontal a ser atualizada pode ser identificada usando FrontoorName e ResourceGroupName, objeto PSFrontDoor ou ResourceId. (Consulte acima de 3 exemplos, por exemplo) O exemplo a seguir usa o objeto PSFrontDoor.
...
### -BackendPool
Backendpools disponível para a regra de roteamento.
...
Se a verificação de nome do certificado deve ser desabilitada em solicitações HTTPS para todos os pools back-end. Nenhum efeito em solicitações não HTTPS.
...
### -Enabledstate
Status operacional do balanceador de carga da porta frontal.
...
### -HealthProbeSetting
Configurações de investigação de integridade associadas a esta instância de porta frontal.
...
### -InputObject
O objeto da porta frontal a ser atualizado.
...
Configurações de balanceamento de carga associadas a esta instância de porta frontal.
...
### -Nome
O nome da porta frontal a ser atualizada.
...
### -ResourceId
ID do recurso da porta frontal a ser atualizada
