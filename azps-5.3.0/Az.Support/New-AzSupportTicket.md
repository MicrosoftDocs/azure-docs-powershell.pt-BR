...
## DESCRITIVO
Este cmdlet pode ser usado para criar um tíquete de suporte para cobrança, gerenciamento de assinaturas, cotas ou problemas técnicos. Use cmdlets Get-AzSupportService e Get-AzSupportProblemClassification para identificar o serviço do Azure e ele tem classificações de problemas correspondentes, respectivamente para as quais você deseja solicitar suporte. Você deve especificar os seguintes parâmetros: 
...
Você pode usar o cmdlet do auxiliar New-AzSupportContactProfileObject para criar o objeto CustomerContactDetail.

Os provedores de soluções na nuvem podem criar um tíquete de suporte para as assinaturas do cliente conectando ao locatário do cliente e especificando sua ID de locatário inicial usando o parâmetro *CSPHomeTenantId* .

__Para tíquetes técnicos:__

Para especificar o nome do recurso, especifique a ID do recurso ARM do recurso usando o parâmetro *TechnicalTicketResourceId* . Veja um exemplo abaixo. 

__Para tíquetes de cota:__

Para solicitar aumento de cota para **núcleos de VM de computação**, **lote**, banco de dados **SQL** e **SQL data warehouse**, forneça detalhes adicionais em objeto *QuotaTicketDetail* . O objeto QuotaTicketDetail consiste em 3 propriedades conforme descrito abaixo. Para obter documentação detalhada, [clique aqui.](https://aka.ms/supportrpquotarequestpayload)
...
Para obter documentação detalhada sobre como construir carga para vários tipos de cotas, [clique aqui](https://aka.ms/supportrpquotarequestpayload) .

## EXEMPLOS

### Exemplo 1: criar um tíquete de suporte para gerenciamento de faturamento ou assinatura. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos para a classificação de problemas de gerenciamento de assinaturas ou cobrança para a qual você deseja solicitar suporte 
```powershell
...
### Exemplo 2: criar um tíquete de suporte técnico para o recurso da máquina virtual para Windows. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos da classificação de problemas para a máquina virtual para Windows para as quais você deseja solicitar suporte 
```powershell
...
### Exemplo 3: criar um tíquete de suporte de cotas para aumentar a cota de núcleos de máquina virtual para uma família específica de VM. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos para a classificação de problemas de núcleos de VM de computação de cota.
```powershell
...
### Exemplo 4: criar um tíquete de suporte de cotas para aumentar a cota de núcleos de baixa prioridade para uma conta em lotes. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos para classificação de problemas em lotes de cota.
```powershell
...
### Exemplo 5: criar um tíquete de suporte de cotas para aumentar a cota de núcleos de VM para uma família específica de VMs para uma conta em lotes. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos para classificação de problemas em lotes de cota.
```powershell
...
### Exemplo 6: criar um tíquete de suporte de cotas para aumentar a cota de grupos para uma conta em lotes. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos para classificação de problemas em lotes de cota.
```powershell
...
### Exemplo 7: criar um tíquete de suporte de cotas para aumentar trabalhos ativos e cota de cronogramas de trabalho para uma conta em lotes. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos para classificação de problemas em lotes de cota.
```powershell
...
### Exemplo 8: criar um tíquete de suporte de cotas para aumentar o número de contas em lotes para uma assinatura. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos para classificação de problemas em lotes de cota.
```powershell
...
### Exemplo 9: criar um tíquete de suporte de cotas para aumentar a cota de DTUs para o banco de dados SQL. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos da classificação de problemas do banco de dados SQL de cota.
```powershell
...
### Exemplo 10: criar um tíquete de suporte de cotas para aumentar a cota de servidores para o banco de dados SQL. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos da classificação de problemas do banco de dados SQL de cota.
```powershell
...
### Exemplo 11: criar um tíquete de suporte de cotas para aumentar a cota de DTUs para SQL data warehouse. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos da classificação de problemas de depósito de data SQL da cota.
```powershell
...
### Exemplo 12: criar um tíquete de suporte de cotas para aumentar a cota de servidores para SQL data warehouse. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos da classificação de problemas de data warehouse do SQL de cota.
```powershell
PS C:\> New-AzSupportTicket -Name "test1" -Title "Test" -Description "Test" -Severity "minimal" -ProblemClassificationId "/providers/Microsoft.Support/services/{quota_service_guid}/problemClassifications/{sql_datawarehouse_problemClassification_guid}" -QuotaTicketDetail @{QuotaChangeRequestVersion = "1.0" ; QuotaChangeRequestSubType = "Servers" ; QuotaChangeRequests = (@{Region = "westus"; Payload = "{`"NewLimit`":200}"})} -CustomerContactDetail @{FirstName = "first" ; LastName = "last" ; PreferredTimeZone = "pacific standard time" ; PreferredSupportLanguage = "en-us" ; Country = "USA" ; PreferredContactMethod = "Email" ; PrimaryEmailAddress = "user@contoso.com"}
...
### Exemplo 13: criar um tíquete de suporte de cotas para aumentar a cota de núcleos de baixa prioridade para o serviço de aprendizagem de máquina. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos da classificação do problema do serviço de aprendizagem da máquina de cota.
```powershell
...
### Exemplo 14: criar um tíquete de suporte de cotas para aumentar a cota de núcleos de VM para uma família específica de VMs para serviço de aprendizagem de computador. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos da classificação do problema do serviço de aprendizagem da máquina de cota.
```powershell
...
### Exemplo 15: criar um tíquete de suporte de cotas para aumentar a cota de instância gerenciada do Azure SQL. Use Get-AzSupportService e Get-AzSupportProblemClassification para recuperar os GUIDs corretos da classificação de problemas do serviço de instância gerenciada SQL de cota.
```powershell
...
### Exemplo 17: criar um tíquete de suporte com solicitação de resposta 24 x 7 do Azure. 
```powershell
...
### Exemplo 18: criar um tíquete de suporte em nome de seu cliente se você for um provedor de soluções de nuvem (CSP). O CSP deve primeiro fazer login no locatário dele e, em seguida, fazer logon no locatário do cliente, conforme mostrado no exemplo a seguir. Eles devem usar o parâmetro-CSPHomeTenantId para especificar sua ID de locatário inicial no momento da criação de um tíquete de suporte.  
```powershell
...
### -CSPHomeTenantId
Esta é a ID do locatário inicial do usuário do provedor de soluções na nuvem que está tentando criar um tíquete de suporte para o cliente.
...
### -CustomerCountry
País do cliente.
Deve ser um código de país Alfa-3 ISO válido (ISO 3166).
...
Isso será necessário se o método de contato preferencial for o telefone.
...
Idioma Peferred do personalizado.
Deve ser um código de conta de idioma válido para um dos idiomas com suporte listados aqui https://azure.microsoft.com/en-us/support/faq/ .
...
Deve ser um valor System.TimeZoneInfo.Id válido.
...
### -Descrição
Descrição detalhada da pergunta ou do problema.
...
### -Nome
Nome da permissão de suporte que este cmdlet cria.
...
### -ProblemClassificationId
Cada serviço do Azure tem seu próprio conjunto de categorias de problemas chamado classificação de problemas que corresponde ao tipo de problema que você está enfrentando.
Esse parâmetro é a ID do recurso ARM do recurso ProblemClassification.
...
### -ProblemStartTime
Data e hora em que o problema foi iniciado.
...
### -QuotaTicketDetail
Detalhes adicionais de um tíquete de suporte à cota.
...
### -Require24X7Response
Indica se esse tíquete de suporte requer uma resposta 24x7 do Azure.
...
Esta é a ID do recurso do serviço do Azure (por exemplo: um recurso de máquina virtual ou um recurso HDInsight) para o qual o tíquete de suporte é criado.
