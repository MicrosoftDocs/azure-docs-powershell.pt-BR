...
## Sinopse
Remove extensões de recursos de uma máquina virtual.

## SYNTAX

### RemoveByExtensionName (padrão)
```
...
### Exemplo 1: remover uma extensão usando um nome específico e o Publisher
```
PS C:\> $VM = Remove-AzureVMExtension -VM $VM -ExtensionName $EXT -Publisher $PUB;
```

Este comando Remove uma extensão com o nome e o fornecedor especificados.

### Exemplo 2: remover todas as extensões de uma determinada máquina virtual
```
PS C:\> $VM = Remove-AzureVMExtension -VM $VM -RemoveAll;
```

Esse comando Remove todas as extensões da máquina virtual especificada, conforme armazenadas na variável $VM.
...
### -RemoveAll
Indica que esse cmdlet Remove todas as extensões de recursos da máquina virtual.
