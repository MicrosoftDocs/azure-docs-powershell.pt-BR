...
## Sinopse
Adiciona uma credencial de acesso de armazenamento do Azure.
...
## DESCRITIVO
O cmdlet **New-AzureStorSimpleStorageAccountCredential** adiciona uma credencial de acesso de armazenamento do Azure ao StorSimple Manager para uso por cmdlets do storsimple OneSDK.
A maioria dos cmdlets do StorSimple OneSDK lida com entidades que, por fim, estão associadas a uma conta de armazenamento específica, como volumes, contêineres de volume, backups e políticas de backup.
Para alguns cmdlets, você deve fornecer as credenciais da conta de armazenamento em uso.
Uma credencial de conta de armazenamento é um objeto do Access criado no OneSDK que aponta para uma conta de armazenamento existente do Azure.
Você fornece o nome e a chave de acesso de uma conta de armazenamento existente para criar uma credencial de conta de armazenamento.
Em seguida, você pode usar esse objeto de credenciais com outros cmdlets.

Esse cmdlet usa a chave de registro fornecida quando você seleciona o recurso usando o cmdlet **Select-AzureStorSimpleResource** .
Certifique-se de que o valor está correto para evitar falha na criptografia.
Para modificar a chave de registro para um valor correto, use **Select-AzureStorSimpleResource**.
...
Esse comando cria uma credencial de acesso de armazenamento para a conta de armazenamento especificada.
Esse comando especifica o parâmetro *WaitForComplete* e, portanto, o cmdlet aguarda até que a tarefa termine para retornar o controle ao console.

### Exemplo 2: criar uma credencial e consultar o status da tarefa
```
...
O primeiro comando cria uma credencial de acesso de armazenamento para a conta de armazenamento especificada.
O comando retorna uma ID de tarefa.

O segundo comando consulta o status da tarefa usando o cmdlet **Get-AzureStorSimpleTask** .
O comando especifica a identificação da tarefa do primeiro comando.

### Exemplo 3: criar uma credencial para usar com outro cmdlet
```
...
Esse comando cria uma credencial de conta de armazenamento.
Em seguida, o comando passa essa credencial para o cmdlet **New-AzureStorSimpleDeviceVolumeContainer** usando o operador pipeline.
Esse cmdlet cria um novo contêiner de volume usando a credencial.
...
Esse cmdlet retorna uma lista de objetos **StorageAccountCredentialResponse** , se você especificar o parâmetro *WaitForComplete* .
