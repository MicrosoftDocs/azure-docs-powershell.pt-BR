...
## DESCRITIVO
O cmdlet **New-AzureVMConfig** cria um objeto de configuração de máquina virtual do Azure.
Você pode usar esse objeto para executar uma nova implantação e adicionar uma nova máquina virtual a uma implantação existente.
...
Esse comando cria uma configuração de máquina virtual do Windows com disco do sistema operacional, disco de dados e configuração do provisionamento.
Essa configuração é usada para criar uma nova máquina virtual.

### Exemplo 2: criar uma configuração de máquina virtual do Linux
```
...
Esse comando cria uma nova configuração de máquina virtual Linux com disco do sistema operacional, configuração de disco de dados e provisionamento.
Essa configuração é usada para criar uma nova máquina virtual.
...
Indica que a configuração desabilita o diagnóstico de inicialização.
Por padrão, o diagnóstico de inicialização está habilitado na máquina virtual.
...
### -LicenseType
Especifica o tipo de licença para uma imagem ou um disco licenciado no local.
Os valores aceitáveis para esse parâmetro são:

- Windows_Client
- Windows_Server 

Especifique esse parâmetro somente para imagens que contenham o sistema operacional Windows Server.
