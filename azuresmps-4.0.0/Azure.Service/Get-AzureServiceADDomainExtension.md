...
## Sinopse
Obtém a extensão de domínio do serviço de nuvem do Active Directory (AD) que se aplica a todas as funções ou às funções nomeadas em um slot de implantação especificado.
...
## DESCRITIVO
O cmdlet **Get-AzureServiceADDomainExtension** Obtém a extensão de domínio do AD do serviço em nuvem que se aplica a todas as funções ou funções nomeadas em um slot de implantação especificado.

## EXEMPLOS

### Exemplo 1: obter a extensão de domínio do AD para um serviço especificado
```
PS C:\> Get-AzureServiceADDomainExtension -ServiceName $Svc
```

Esse comando obtém a extensão de domínio do AD para o serviço especificado em $Svc.
