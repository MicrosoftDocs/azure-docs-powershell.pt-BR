...
## DESCRITIVO
O cmdlet **Update-AzVpnConnection** atualiza uma conexão VPN.  
A conexão VPN cria uma conexão IPsec que conecta um gateway VPN a uma ramificação de cliente remoto representada no Azure como um site de VPN.
...
A conexão é atualizada para ter um novo IpSecPolicy usando o comando Set-AzVpnConnection.
...
A conexão é atualizada para ter uma nova chave compartilhada usando a construção de cadeia de caracteres segura.
...
### -InputObject
O objeto VpnConnection a ser atualizado.
...
### -ResourceId
A ID do recurso do objeto VpnConnection a ser excluído.
...
A lista de VpnSiteLinkConnections que esse VpnConnection precisa ter.
