...
## DESCRITIVO
O cmdlet **set-AzSqlInstancePool** modifica as propriedades de um pool de instâncias do SQL do Azure.
...
Esse comando define o tipo de licença e/ou as marcas de um pool de instâncias chamado instancePool0.
...
Esse comando define o tipo de licença e/ou as marcas para um pool de instâncias usando um objeto de pool de instâncias.

### Exemplo 3: definir um tipo de licença de pool de instâncias usando uma ID de recurso do pool de instâncias
```powershell
...
Os valores possíveis são BasePrice (com desconto AHB) e LicenseIncluded (sem desconto AHB).
