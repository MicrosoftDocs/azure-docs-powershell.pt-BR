...
Salvamento automático de contexto é o armazenamento de informações de logon do Azure que pode ser usado entre novas e diferentes sessões do PowerShell. Para obter mais informações sobre o salvamento automático de contexto, consulte [este documento](https://docs.microsoft.com/en-us/powershell/azure/context-persistence).

Anteriormente, por padrão, o salvamento automático de contexto foi desabilitado, o que significa que as informações de logon do usuário do Azure não foram armazenadas entre sessões até o `Enable-AzureRmContextAutosave` cmdlet ter sido executado para ativar a persistência de contexto. Mais adiante, o salvamento automático de contexto será habilitado por padrão, o que significa que os usuários _sem nenhuma configuração salva de salvamento automático_ terão seu contexto armazenado na próxima vez que efetuarem logon. Os usuários podem recusar essa funcionalidade usando o cmdlet `Disable-AzureRmContextAutosave`.
...
- O parâmetro `Certificate` se tornou obrigatório.
