...
O Azure PowerShell foi projetado para gerenciar e administrar os recursos do Azure da linha de comando e para a compilação de scripts de automação que funcionam no Azure Resource Manager. Você pode usá-lo em seu navegador com o [Azure Cloud Shell](/azure/cloud-shell/overview), ou pode instalá-lo em seu computador local. Este artigo ajuda você a começar com o Azure PowerShell, além de ensinar os conceitos básicos por trás dele.
...
1. [Instale o Azure PowerShell](install-az-ps.md).

2. Para verificar se a instalação foi bem-sucedida, execute `Get-InstalledModule Az -AllVersions` na linha de comando.
...
1. Digite `Connect-AzAccount`. O argumento `-Environment` permite autenticar-se para uma região ou nuvem diferente. Por exemplo, para se conectar ao Azure China:

    ```powershell-interactive
    Connect-AzAccount -Environment AzureChinaCloud
    ```

2. você receberá um token a ser usado em https://microsoft.com/devicelogin. Abra essa página no seu navegador e insira o token para entrar com suas credenciais do Azure e autorizar o Azure PowerShell. 

Depois de entrar em uma conta do Azure, use os cmdlets do Azure PowerShell para acessar e gerenciar os recursos em sua assinatura. Para saber mais sobre o processo de entrada e os métodos de autenticação disponíveis, confira [Entrar com o Azure PowerShell](authenticate-azureps.md).
...
O cmdlet `New-AzVM` fornece uma sintaxe simplificada, facilitando a criação de uma nova máquina virtual. Há apenas dois valores de parâmetro que você deve fornecer: o nome da VM e um conjunto de credenciais para a conta de administrador local na VM.
...
Você deve estar imaginando o que mais é criado e como a VM é configurada. Primeiro, vamos examinar nossos grupos de recursos.
...
O grupo de recursos **cloud-shell-storage-westus** é criado na primeira vez que você usar o Cloud Shell. O grupo de recursos **SampleVM** foi criado pelo cmdlet `New-AzVM`.
...
Vamos criar um grupo de recursos chamado "MyResourceGroup" na região uswest2 do Azure. Para fazer isso, digite o seguinte comando:
...
Agora que a VM foi criada, entre em sua nova VM do Linux usando SSH com o endereço IP público da VM criada:
...
Usar o cmdlet `Get-AzResource` para listar os recursos em execução no Azure. O exemplo a seguir mostra os recursos que criamos no novo grupo de recursos.
...
Para limpar sua conta do Azure, convém remover os recursos que criamos neste exemplo. Usar os cmdlets `Remove-Az*` para excluir os recursos desnecessários. Para remover a VM do Windows que criamos, use o seguinte comando:
...
Também é possível excluir vários recursos ao mesmo tempo. Por exemplo, o comando a seguir exclui o grupo de recursos "MyResourceGroup" que usamos para todos os exemplos até agora.
Todos os recursos no grupo também são excluídos.
...
A tarefa pode demorar vários minutos para ser concluída, dependendo do número e do tipo de recursos.
