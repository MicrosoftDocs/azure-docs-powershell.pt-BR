...
### Exemplo 5: obter todos os BLOBs em um contêiner incluir versão de BLOB
```
...
Esse comando obtém uma única versão de BLOBs com VersionId.

### Exemplo 7: obter um instantâneo de blob único
```
...
Esse comando obtém um instantâneo de BLOBs único com Instantâneotime.
...
Se um valor for especificado para esse parâmetro, o cmdlet listará todos os BLOBs com nomes correspondentes a esse parâmetro. Esse parâmetro dá suporte a caracteres curinga em qualquer lugar na cadeia de caracteres.
...
### -IncludeVersion
As versões de blob serão listadas apenas se esse parâmetro estiver presente, por padrão se a obtenção de BLOB não incluirá versões de BLOB.
