...
## DESCRITIVO
Esse cmdlet permite a criação de serviços sem monitoração ou sem monitoração de estado no aplicativo especificado. O serviço deve ser encerrado no manifesto do aplicativo e o tipo deve ser igual ao do manifesto. O nome do aplicativo deve ser um prefixo do nome do serviço.
...
Este exemplo criará um novo serviço sem monitoração de estado "testApp ~ testService1" com a contagem de instância-1 (em todos os nós).
...
Este exemplo criará um novo serviço stateful "testApp ~ testService2" com um destino de 5 nós.
...
### -DefaultMoveCost
Especificar o custo padrão de uma movimentação.
Custos mais altos tornam menos provável que o Gerenciador de recursos de cluster mova a réplica ao tentar balancear o cluster
...
### -MinReplicaSetSize
Especificar o tamanho mínimo do conjunto de réplicas do serviço
...
Os serviços que usam esse modelo geralmente têm dados que podem ser emissodos em um conjunto associado.
...
Geralmente, as partições singleton são usadas quando o serviço não requer roteamento adicional.
...
Isso significa que cada partição possui um intervalo de chaves Int64.
...
Especificar a duração de espera de perda de quorum para o serviço
...
Especificar a duração de espera de reinício de réplica para o serviço
...
Especificar a duração da réplica em espera para o serviço
...
Usar para serviço sem monitoração de estado
...
### -Digite
Especifique o nome do tipo de serviço do aplicativo, que deve existir no manifesto do aplicativo.
