...
### Exemplo 3: Adicionar/atualizar marcas para um gateway de rede virtual existente
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
O primeiro comando obtém um gateway de rede virtual chamado Gateway01 que pertence à ResourceGroup001 do grupo de recursos e o armazena na variável chamada $Gateway o segundo comando atualiza o gateway de rede virtual Gateway01 com as marcas @ {testtagKey = "SomeTagKey"; testtagValue = "SomeKeyValue"}.

### Exemplo 4: Adicionar/atualizar a configuração de autenticação do AAD para o VpnClient de um gateway de rede virtual existente
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
### Exemplo 5: Adicionar/atualizar o IpConfigurationBgpPeeringAddresses a um gateway de rede virtual existente
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
O primeiro comando obtém um gateway de rede virtual chamado Gateway01 que pertence à ResourceGroup001 do grupo de recursos e o armazena na variável chamada $Gateway o segundo comando atribui o valor do gateway de rede virtual Gateway01 a ID da IpConfiguration à variável ipconfigurationId1.
O terceiro comando atribui a lista de endereços ao AddressList1.
O quarto comando criou um objeto PSIpConfigurationBgpPeeringAddress.
O quinto comando define esse novo PSIpConfigurationBgpPeeringAddress criado para IpConfigurationBgpPeeringAddresses e atualizar o gateway.
...
### -ASN
O ASN do gateway de rede virtual, usado para configurar sessões BGP dentro de túneis IPsec
...
Sinalizar para desabilitar o recurso ativo ativo no gateway de rede virtual
...
### -GatewayDefaultSite
O site padrão a ser usado para o tunelamento forçado.
...
### -GatewaySku
O SKU do gateway de rede virtual
...
### -RadiusServerList
P2S vários servidores RADIUS externos.
...
Sinalizador para remover a autenticação AAD do cliente P2S do gateway de rede virtual.
...
O objeto de gateway de rede virtual para basear as alterações a partir de.
Isso pode ser recuperado usando Get-AzVirtualNetworkGateway
...
### -VpnClientAddressPool
O espaço de endereço do qual os endereços IP do cliente VPN são alocados.
...
Um cliente VPN apresentando um certificado que corresponda a um deles será instruído a ficar ausente.
