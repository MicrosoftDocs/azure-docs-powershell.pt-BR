...
## DESCRITIVO
O cmdlet **New-AzNetworkInterface** cria uma interface de rede do Azure.
...
O primeiro comando recupera uma rede virtual especificada existente usada para atribuir a sub-rede no segundo comando.
O segundo comando cria uma configuração de IP de interface de rede chamada IPConfig1 e armazena a configuração na variável chamada $IPconfig.
O terceiro comando cria uma interface de rede chamada NetworkInterface1 que usa a configuração de IP da interface de rede armazenada na variável chamada $IPconfig.

### Exemplo 3

Cria uma interface de rede. (gerado automaticamente)
