...
### Exemplo 1: criar um novo trabalho do ImportExport
```powershell
...
Esses cmdlets criam um novo trabalho do ImportExport.
...
### -BlobListBlobPath
Uma coleção de cadeias de caracteres de caminho BLOB.
...
### -ClientTenantId
A ID do locatário do cliente que está realizando a solicitação.
...
### -DiagnosticsPath
O diretório de blob virtual ao qual a cópia dos arquivos de manifesto de unidade (se habilitada) será armazenada.
...
A lista de unidades é um elemento obrigatório para um trabalho de importação; Não está especificado para trabalhos de exportação.
...
O URI relativo para o blob de bloco que contém a lista de caminhos BLOB ou prefixos de caminho BLOB, conforme definido acima, começando com o nome do contêiner.
...
Um caminho de BLOB que aponta para um blob de bloco que contém uma lista de nomes de BLOB que não foram exportados devido a espaço insuficiente na unidade.
Se todos os BLOBs forem exportados com êxito, esse elemento não será incluído na resposta.
...
### -Local
Especifica o local do Azure com suporte no qual o trabalho deve ser criado
...
Indica se o log de erros ou o log detalhado será habilitado.
...
### -StorageAccountId
O identificador de recurso da conta de armazenamento na qual os dados serão importados ou exportados.
...
DRIVElist <IDriveStatus [] >: lista de até dez unidades que compõem o trabalho. A lista de unidades é um elemento obrigatório para um trabalho de importação; Não está especificado para trabalhos de exportação.
