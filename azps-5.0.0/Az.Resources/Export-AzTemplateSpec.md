...
### ExportByNameParameterSet (padrão)
```
...
## DESCRITIVO
Exporta uma versão de especificação de modelo específica para o sistema de arquivos local.

## EXEMPLOS

### Exemplo 1: exportar por nome
```powershell
PS C:\> Export-AzTemplateSpec -ResourceGroupName 'myRG' -Name 'MyTemplateSpec' -Version 'v1.0' -OutputFolder 'v1'
```

Exporta a versão "v 1.0" da especificação do modelo chamada "MyTemplateSpec" no grupo de recursos "myRG" para a pasta de saída local "v1".

### Exemplo 2: exportar por ID do recurso
```powershell
...
Exporta a versão "v 1.0" da especificação do modelo chamada "MyTemplateSpec" no grupo de recursos "myRG" da assinatura \{ subId \} para a pasta de saída local "v1".
...
### -OutputFolder
O caminho para a pasta em que a versão de especificação do modelo será impressa.
...
### -Versão
A versão da especificação do modelo a ser exportada.
