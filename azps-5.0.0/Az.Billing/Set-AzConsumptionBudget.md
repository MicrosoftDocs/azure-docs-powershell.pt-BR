...
### Exemplo 1: atualizar um orçamento por um novo valor com um nome de orçamento no nível de assinatura
```powershell
...
### Exemplo 2: atualizar um orçamento com uma notificação quando o custo ou o uso alcançar um limite de 90% do valor no nível de assinatura
```powershell
...
### Exemplo 3: atualizar um orçamento por um novo valor com um nome de orçamento no nível de grupo de recursos
```powershell
...
### -Categoria
Categoria do orçamento pode ser custo ou uso.
...
### -ContactEmail
Endereços de email para enviar a notificação de orçamento para quando o limite for excedido.
...
### -ContactRole
Funções de contato para enviar a notificação de orçamento para quando o limite for excedido.
...
### -MeterFilter
Lista separada por vírgula de medidores para filtrar.
Obrigatório se a categoria for o uso.
...
Se não for especificado, a notificação será desabilitada por padrão.
...
### -NotificationKey
Chave de uma notificação associada a um orçamento, necessária para criar uma notificação com uma chave habilitada para notificação, um limite de notificação, emails de contato, grupos de contatos ou funções de contato.
...
Valor limite associado a uma notificação.
A notificação é enviada quando o custo ou o uso excedeu o limite.
É sempre porcentagem e deve estar entre 0 e 1000.
...
### -ResourceFilter
Lista separada por vírgula de instâncias de recurso para filtrar.
...
### -ResourceGroupFilter
Lista separada por vírgula de grupos de recursos para filtrar.
...
Não antes do mês atual para o intervalo de tempo mensal.
Não antes de três meses para o intervalo trimestral de tempo.
Não antes de 12 meses por um intervalo de tempo anual.
Data de início futuro não tem mais de três meses.
...
### -Timegranular
O intervalo de tempo do orçamento pode ser mensal, trimestral ou anualmente.
