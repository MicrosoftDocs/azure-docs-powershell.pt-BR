...
Esse comando cria uma revisão `2` de API da `echo-api` API.

### Exemplo 2: criar uma revisão de API a partir de uma API existente e copiar todas as operações, marcas e políticas
```powershell
...
Esse comando cria uma revisão `5` de API da `echo-api` API.

## OS

### -ApiId
Identificador de API cuja revisão será criada.
...
### -ServiceUrl
Uma URL do serviço Web que expõe a API no serviço back-end. Essa URL será usada apenas pelo gerenciamento de APIs do Azure e não será publicada. Deve ter de 1 a 2000 caracteres de comprimento. Esse parâmetro é obrigatório.
