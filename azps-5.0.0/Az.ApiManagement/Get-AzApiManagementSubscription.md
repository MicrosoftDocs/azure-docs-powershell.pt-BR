...
## DESCRITIVO
O cmdlet **Get-AzApiManagementSubscription** Obtém uma assinatura especificada, ou todas as assinaturas, se nenhuma assinatura for especificada.
As teclas não serão incluídas nos detalhes do resultado. Para obter as chaves, use **Get-AzApiManagementSubscriptionKey**.
...
### Exemplo 5: obter todas as assinaturas de um escopo
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
Este comando obtém todas as assinaturas configuradas para o escopo da API global

### Exemplo 6: obter todas as assinaturas para um produto e um escopo de usuário
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
### -Escopo
Identificador de escopo. O escopo da assinatura, seja o escopo da API/apis/{apiId} ou o escopo do produto/products/{productId} ou o escopo da API global/APIs ou escopo global/.
