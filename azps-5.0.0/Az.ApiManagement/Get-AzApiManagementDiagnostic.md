...
## DESCRITIVO
**Get-AzApiManagementDiagnostic** Obtém detalhes dos diagnósticos configurados no serviço de gerenciamento de API em um determinado escopo.

## EXEMPLOS

### Exemplo 1: Obtenha todos os diagnósticos configurados no escopo do locatário.
```powershell
...
Esse comando obtém todos os diagnósticos configurados no serviço de gerenciamento de API.

### Exemplo 2: obter todos os diagnósticos configurados no escopo da API
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Esse comando obtém todos os diagnósticos configurados no `echo-api` escopo da API

### Exemplo 3: obter o diagnóstico de escopo de API especificado por uma ID
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Esse comando obtém o `applicationinsights` diagnóstico configurado na API `echo-api` .

## OS

### -ApiId
Identificador de API existente.
Se especificado, retornará o diagnóstico de escopo de API.
...
### -Diagnosticid
Identificador de diagnóstico existente.
Se especificado, retornará a política de escopo do produto.
...
### -ResourceId
Identificador de recursos ARM de um diagnóstico ou diagnóstico de API. Se especificado, tenta encontrar o diagnóstico pelo identificador. Esse parâmetro é obrigatório.
...
### Microsoft. Azure. Commands. ApiManagement. onmanagement. Models. PsApiManagementDiagnostic
