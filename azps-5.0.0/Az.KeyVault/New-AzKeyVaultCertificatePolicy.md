...
### Exemplo 3: criar um certificado alternativo de nome (ou SAN) de assunto
...
Este exemplo cria um certificado de SAN com 3 nomes DNS.

## OS

### -CertificateTransparency
Indica se a transparência do certificado está habilitada para este certificado/emissor; Se não for especificado, o padrão será ' true '
...
### -DnsName
Especifica os nomes DNS no certificado. Nomes alternativos de entidades (SANs) podem ser especificados como nomes DNS.
...
> [!NOTE]
> Se você deve usar uma vírgula (,) ou um ponto (.) dentro de uma propriedade no `SubjectName` parâmetro, você deve colocar o campo de propriedade entre aspas. Por exemplo, você pode usar O = "contoso, Ltd." no campo nome da organização.
