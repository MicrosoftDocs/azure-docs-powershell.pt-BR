...
### Exemplo 1: validar parâmetros de conexão de dados do EventHub
```powershell
...
O comando acima valida a conexão de dados do EventHub chamada "myeventhubdc" para o banco de dados "mykustodatabase" no cluster "testnewkustocluster".

### Exemplo 2: validar parâmetros de conexão de dados do EventGrid
```powershell
...
O comando acima valida a conexão de dados do EventGrid chamada "myeventgriddc" para o banco de dados "mykustodatabase" no cluster "testnewkustocluster".

### Exemplo 3: validar parâmetros de conexão de dados do IotHub
```powershell
...
O comando acima valida a conexão de dados do IotHub chamada "myiothubdc" para o banco de dados "mykustodatabase" no cluster "testnewkustocluster".

### Exemplo 4: validar parâmetros de conexão de dados do EventHub via identidade
```powershell
...
### Exemplo 5: validar parâmetros de conexão de dados EventGrid via identidade
```powershell
PS C:\> $database = Get-AzKustoDatabase -ResourceGroupName "testrg" -ClusterName "testnewkustocluster" -DatabaseName "mykustodatabase"
...
### Exemplo 6: validar parâmetros de conexão de dados IotHub via identidade
```powershell
PS C:\> $database = Get-AzKustoDatabase -ResourceGroupName "testrg" -ClusterName "testnewkustocluster" -DatabaseName "mykustodatabase" 
...
O nome do tipo de evento de armazenamento de blob a processar.
...
### -EventHubResourceId
A ID do recurso do hub de eventos a ser usado para criar uma conexão de dados/grade de eventos está configurada para enviar eventos.
...
### -IgnoreFirstRecord
Se definido como true, indica que a inclusão deve ignorar o primeiro registro de cada arquivo.
...
### -Kind
Tipo de ponto de extremidade para conexão de dados
...
### -MappingRuleName
A regra de mapeamento a ser usada para absorver os dados.
...
A ID do recurso da conta de armazenamento na qual os dados residem.
...
### -TableName
A tabela na qual os dados devem ser incluídos.
