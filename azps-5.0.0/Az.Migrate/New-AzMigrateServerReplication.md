...
### Exemplo 1: quando há apenas o disco do so
```powershell
...
Isso se destina ao cenário, quando há apenas um único disco que está protegido.
...
Isso se destina ao cenário, quando há vários discos que devem ser protegidos.
...
Para construir, consulte a seção notas para propriedades DISKTOINCLUDE e crie uma tabela de hash.
...
O objeto do servidor pode ser recuperado usando o cmdlet Get-AzMigrateServer.
...
### -LicenseType
Especifica se o benefício híbrido do Azure é aplicável para o servidor de origem ser migrado.
...
### -MachineID
Especifica a ID da máquina do servidor descoberto a ser migrado.
...
### -OSDiskID
Especifica o disco do sistema operacional para o qual o servidor de origem será migrado.
...
### -PerformAutoResync
Especifica se a replicação será reparada automaticamente para o controle de alterações de maiúsculas e minúsculas ser perdida para o servidor de origem em replicação.
...
Especifica o conjunto de disponibilidade a ser usado para a VM creationSpecifies o conjunto de disponibilidade a ser usado para a criação de VM.
...
Especifica a conta de armazenamento a ser usada para o diagnóstico de inicialização.
...
### -TargetNetworkId
Especifica a ID de rede virtual na assinatura de destino do Azure à qual o servidor precisa ser migrado.
...
Especifica a ID do grupo de recursos na assinatura de destino do Azure à qual o servidor precisa ser migrado.
...
### -TargetSubnetName
Especifica o nome da sub-rede dentro do Netowk virtual de destino para o qual o servidor precisa ser migrado.
...
### -TargetVMSize
Especifica a SKU da máquina virtual do Azure a ser criada.
...
DISKTOINCLUDE <IVMwareCbtDiskInput [] >: especifica os discos no servidor de origem a serem incluídos para replicação.
  - `DiskId <String>`: A ID de disco.
  - `IsOSDisk <String>`: Um valor que indica se o disco é o disco do sistema operacional.
  - `LogStorageAccountId <String>`: A ID do braço da conta de armazenamento de log.
  - `LogStorageAccountSasSecretName <String>`: O nome do segredo do cofre de chaves da conta de armazenamento de log.
  - `[DiskEncryptionSetId <String>]`: A ID do braço DiskEncryptionSet.
  - `[DiskType <DiskAccountType?>]`: O tipo de disco.

INPUTobject <IVMwareMachine> : especifica o servidor descoberto a ser migrado. O objeto do servidor pode ser recuperado usando o cmdlet Get-AzMigrateServer.
